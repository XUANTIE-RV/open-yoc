...
#define ERR_READ_REG -1
#define ERR_WRITE_REG -2
#define ERR_RESET -3
#define ERR_TIMEOUT -4
#define ERR_SEND -5
#define ERR_SEND_FRAMELEN -6
#define ERR_RECV_FRAMELEN -7
#define ERR_RX_STATUS -8
#define ERR_TX_STATUS -9

#define TAG "28j60"
#define ETH_LOG_ERR(err) LOGE(TAG, "%d", err)
#define ETH_LOG_ERR2(err, subtype) LOGE(TAG, "%d,%d", err, subtype)
...
static gpio_pin_handle_t   pgpio_pin_handle1 = NULL;
...
extern int32_t eth_spi_init(int32_t idx, spi_handle_t *spi_hdl, gpio_pin_handle_t *gpio_hdl);
...
uint32_t eth_get_event(int32_t idx)
{
    ...
    if (int_stat & EIR_RXERIF) {
        ...
        if (ptkcnt > (RXSTOP_INIT-RXSTART_INIT)/60) {
            LOGW(TAG, "rx:%d free:%d\n", ptkcnt, freespace);
            eth_event |= 0x80;
        } else {
    ...
    if (int_stat & EIR_TXERIF) {
        ...
        if ((estat & ESTAT_TXABRT) || (estat & ESTAT_LATECOL)) {
            ETH_LOG_ERR2(ERR_TX_STATUS, estat);
...
eth_mac_handle_t csi_eth_mac_initialize(int32_t idx, eth_event_cb_t cb_event)
{
    ...
    ret = eth_spi_init(idx, &g_net_spi_hd, &pgpio_pin_handle1);

    if (ret == 0) {
        if (enc28j60ResetInit(RST_ENC28J60_ALL) == 0) {
            //ETH_LOG_DBG("enc28j60ResetInit\n");
...
int32_t csi_eth_mac_control(eth_mac_handle_t handle, uint32_t control, uint32_t arg)
{
    ...
    if (control == CSI_ETH_MAC_CONFIGURE) {
        ...
        enc28j60WriteOp(arg ? ENC28J60_BIT_FIELD_SET : ENC28J60_BIT_FIELD_CLR, EIE, EIE_INTIE);
    }
...
int32_t csi_eth_mac_send_frame(eth_mac_handle_t handle, const uint8_t *frame, uint32_t len, uint32_t flags)
{
    ...
    if (len > MAX_FRAMELEN) {
        ETH_LOG_ERR2(ERR_SEND_FRAMELEN, len);
        return 0;
    }
...
int32_t csi_eth_mac_read_frame(eth_mac_handle_t handle, uint8_t *frame, uint32_t len)
{
    ...
    if (rlen > MAX_FRAMELEN - 1) {
        ETH_LOG_ERR2(ERR_RECV_FRAMELEN, rlen);
        rlen = MAX_FRAMELEN - 1;
        return -1;
    }
    ...
    /*   need to check this. */
    if ((rxstat & 0x80) == 0) {
        ETH_LOG_ERR2(ERR_RX_STATUS, rxstat);
        rlen = 0; // invalid
        return -1;
    }
...
int32_t csi_eth_mac_ex_read_frame_begin(eth_mac_handle_t handle)
{
    ...
    if (len > MAX_FRAMELEN - 1) {
        ETH_LOG_ERR2(ERR_RECV_FRAMELEN, len);
        len = MAX_FRAMELEN - 1;
        return -1;
    }
...
#endif/* CONFIG_TCPIP_ENC28J60_USE_PBUF */
