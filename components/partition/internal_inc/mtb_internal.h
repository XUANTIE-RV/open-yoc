...
#define GET_FIRST_IMG_SCN_ADDR(bm_addr, scn_addr)                                                  \
    get_section_addr((uint32_t)bm_addr, &(g_scn_img_type[SCN_TYPE_IMG]), NULL,                     \
                     (uint32_t *)scn_addr, SEACH_MODE_FIRST_TYPE)
#define GET_PUBLICKEY_SCN_ADDR(bm_addr, scn_addr)                                                  \
    get_section_addr((uint32_t)bm_addr, &(g_scn_key_type[SCN_SUB_TYPE_KEY_BOOT]), NULL,            \
                     (uint32_t *)scn_addr, SEACH_MODE_EXTEND_TYPE)
#define GET_PARTION_SCN_ADDR(bm_addr, scn_addr)                                                    \
    get_section_addr((uint32_t)bm_addr, &(g_scn_part_type[SCN_SUB_TYPE_IMG_PART]), NULL,           \
                     (uint32_t *)scn_addr, SEACH_MODE_EXTEND_TYPE)
...
int get_section_addr(uint32_t bm_addr, const scn_type_t *seach_type, uint8_t *seach_name, uint32_t *scn_addr, uint32_t mode);
