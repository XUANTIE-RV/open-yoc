...
#define BT_TASK_DEFAULT_PRIORITIES  (35)
#define BT_BTC_TASK_STACK_SIZE      UC_BTC_TASK_STACK_SIZE
#define BT_TASK_EXTRA_STACK_SIZE    2048
...
// Mapping between YOC_LOG_LEVEL and BT_TRACE_LEVEL
...
#if 1
#define BT_PRINT_E(tag, format, ...)   {LOGE(tag, format, ##__VA_ARGS__); }
#define BT_PRINT_W(tag, format, ...)   {LOGW(tag, format, ##__VA_ARGS__); }
#define BT_PRINT_I(tag, format, ...)   {LOGI(tag, format, ##__VA_ARGS__); }
#define BT_PRINT_D(tag, format, ...)   {LOGD(tag, format, ##__VA_ARGS__); }
#define BT_PRINT_V(tag, format, ...)   {LOGD(tag, format, ##__VA_ARGS__); }
#else
#define BT_PRINT_E(tag, format, ...)   printf("[E: "tag"]BT_PRT:"format"\n", ##__VA_ARGS__)
#define BT_PRINT_W(tag, format, ...)   printf("[W: "tag"]BT_PRT:"format"\n", ##__VA_ARGS__)
#define BT_PRINT_I(tag, format, ...)   printf("[I: "tag"]BT_PRT:"format"\n", ##__VA_ARGS__)
#define BT_PRINT_D(tag, format, ...)   printf("[D: "tag"]BT_PRT:"format"\n", ##__VA_ARGS__)
#define BT_PRINT_V(tag, format, ...)   printf("[V: "tag"]BT_PRT:"format"\n", ##__VA_ARGS__)
#endif
...
/* relate to YOC_BT_STATUS_xxx in yoc_bt_defs.h */
typedef enum {
    BT_STATUS_SUCCESS = 0,
    BT_STATUS_FAIL,
    BT_STATUS_NOT_READY,
