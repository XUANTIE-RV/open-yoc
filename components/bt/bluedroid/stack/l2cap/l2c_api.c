...
BOOLEAN L2CA_CheckIsCongest_by_conn_id(UINT16 fixed_cid, UINT16 conn_id)
{
    tL2C_LCB *p_lcb;
    p_lcb = l2cu_find_lcb_by_conn_id(conn_id);
...
UINT8 L2CA_PTS_DataWrite (BD_ADDR p_bd_addr, BT_HDR *p_data)
{
    tL2C_LCB  *p_lcb = NULL;
    tL2C_CCB  *p_ccb = NULL, *p_next_ccb;
    /* We assume the upper layer will call this function only when the link is established. */
    if ((p_lcb = l2cu_find_lcb_by_bd_addr (p_bd_addr, BT_TRANSPORT_BR_EDR)) == NULL) {
        L2CAP_TRACE_ERROR ("L2CA_Echo ERROR : link not established");
        return FALSE;
    ...
    for (p_ccb = p_lcb->ccb_queue.p_first_ccb; p_ccb; p_ccb = p_next_ccb) {
        ...
        //only send once ,if resend should malloc then copy data.
        l2c_data_write (p_ccb->local_cid, p_data, L2CAP_FLUSHABLE_CH_BASED);
        return TRUE;
    }
...
BOOLEAN L2CA_PTS_DisconnectReq (BD_ADDR p_bd_addr)
{
    tL2C_LCB  *p_lcb = NULL;
    tL2C_CCB  *p_ccb = NULL, *p_next_ccb;
    /* We assume the upper layer will call this function only when the link is established. */
    ...
    for (p_ccb = p_lcb->ccb_queue.p_first_ccb; p_ccb; p_ccb = p_next_ccb) {
        p_next_ccb = p_ccb->p_next_ccb;
        //only send once ,if resend should malloc then copy data.
        L2CA_DisconnectReq (p_ccb->local_cid);
        return TRUE;
    }
...
UINT16 L2CA_FlushChannel (UINT16 lcid, UINT16 num_to_flush)
{
    ...
    if (p_ccb->peer_cfg.fcr.mode != L2CAP_FCR_ERTM_MODE) {
        ...
        for (const list_node_t *node = osi_list_begin(p_lcb->link_xmit_data_q);
                (num_to_flush > 0) && node != osi_list_end(p_lcb->link_xmit_data_q);) {
            ...
            if ((p_buf->layer_specific == 0) && (p_buf->event == lcid)) {
                ...
                osi_list_remove(p_lcb->link_xmit_data_q, p_buf);
                osi_free(p_buf);
            }
        }
    }
    ...
    for (const list_node_t *node = osi_list_begin(p_lcb->link_xmit_data_q);
            node != osi_list_end(p_lcb->link_xmit_data_q);
            node = osi_list_next(node)) {
