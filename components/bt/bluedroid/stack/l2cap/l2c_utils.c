...
tL2C_LCB *l2cu_allocate_lcb (BD_ADDR p_bd_addr, BOOLEAN is_bonding, tBT_TRANSPORT transport)
{
    ...
    for (xx = 0; xx < MAX_L2CAP_LINKS; xx++, p_lcb++) {
        if (!p_lcb->in_use) {
            ...
            p_lcb->link_xmit_data_q = osi_list_new(NULL);
...
void l2cu_release_lcb (tL2C_LCB *p_lcb)
{
    ...
    if (p_lcb->link_xmit_data_q) {
        while (!osi_list_is_empty(p_lcb->link_xmit_data_q)) {
            BT_HDR *p_buf = osi_list_front(p_lcb->link_xmit_data_q);
            osi_list_remove(p_lcb->link_xmit_data_q, p_buf);
            osi_free(p_buf);
        }
        osi_list_free(p_lcb->link_xmit_data_q);
...
#endif
    {
        UINT16_TO_STREAM (p, L2CAP_INFO_RYOC_RESULT_SUCCESS);
#if (BLE_INCLUDED == TRUE)
        if (p_lcb->transport == BT_TRANSPORT_LE) {
        ...
        UINT16_TO_STREAM (p, L2CAP_INFO_RYOC_RESULT_NOT_SUPPORTED);  /* 'not supported' */
    }
...
tL2C_LCB  *l2cu_find_lcb_by_conn_id (UINT16 id)
{
    tL2C_LCB    *p_lcb = l2cb.lcb_pool + id;
