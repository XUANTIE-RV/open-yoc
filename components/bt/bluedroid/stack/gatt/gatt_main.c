...
void gatt_bqb_setFlag(int32_t flag);
int32_t gatt_bqb_getFlag(void);

#if (CLASSIC_BT_INCLUDED == TRUE)
#define BT_GATT_CONN_MAX 20
typedef struct {
    int conn_state;
    UINT16 lcid;
} _bt_gatt_conn_state;
typedef struct {
    _bt_gatt_conn_state bt_gatt_conn_state[BT_GATT_CONN_MAX];
    int conn_num;
} _bt_gatt_info;
_bt_gatt_info bt_gatt_info;
#endif
...
void gatt_init (void)
{
    ...
    #if (CLASSIC_BT_INCLUDED == TRUE)
    memset (&bt_gatt_info, 0, sizeof(_bt_gatt_info));
    #endif
...
/*******************************************************************************
**
** Function         bt_gatt_disconnectall
**
** Description      This function is called to disconnect to all ATT device.
**
** Parameter        void
**
** Returns          TRUE: if connection found and to be disconnected; otherwise
...
**
*******************************************************************************/
BOOLEAN bt_gatt_disconnectall(void)
{
    BOOLEAN             ret = TRUE;
#if (CLASSIC_BT_INCLUDED == TRUE)
    BOOLEAN             ret_tmp = FALSE;
    GATT_TRACE_ERROR("bt_gatt_disconnectall");
    for(int i=0;i<BT_GATT_CONN_MAX; i++){
        if(bt_gatt_info.bt_gatt_conn_state[i].conn_state == 1) {
            ret_tmp = L2CA_DisconnectReq(bt_gatt_info.bt_gatt_conn_state[i].lcid);
            ret &= ret_tmp;
            GATT_TRACE_ERROR("disconnect cid = %d", bt_gatt_info.bt_gatt_conn_state[i].lcid);
        }
    }
#endif
    return ret;
...
static void gatt_l2cif_connect_ind_cback (BD_ADDR  bd_addr, UINT16 lcid, UINT16 psm, UINT8 id)
{
    ...
    int i = 0;
    for(;i<BT_GATT_CONN_MAX; i++){
        if(bt_gatt_info.bt_gatt_conn_state[i].lcid == lcid){
            bt_gatt_info.bt_gatt_conn_state[i].conn_state = 1;
            break;
        }
    }
    if(i == BT_GATT_CONN_MAX) {
        bt_gatt_info.bt_gatt_conn_state[bt_gatt_info.conn_num].conn_state = 1;
        bt_gatt_info.bt_gatt_conn_state[bt_gatt_info.conn_num].lcid = lcid;
        bt_gatt_info.conn_num++;
        GATT_TRACE_ERROR("add new connection cid = %d", lcid);
    }
...
void gatt_l2cif_disconnect_ind_cback(UINT16 lcid, BOOLEAN ack_needed)
{
    ...
    if ((p_tcb = gatt_find_tcb_by_cid(lcid)) != NULL) {
        ...
        for(;i<BT_GATT_CONN_MAX; i++){
            if(bt_gatt_info.bt_gatt_conn_state[i].lcid == lcid){
                bt_gatt_info.bt_gatt_conn_state[i].conn_state = 0;
                GATT_TRACE_ERROR("find connect and disconnect cid = %d", lcid);
                break;
            }
        }
        if(i == BT_GATT_CONN_MAX) {
            GATT_TRACE_ERROR("cannot find connection cid = %d", lcid);
        }
    }
}
...
void gatt_data_process (tGATT_TCB *p_tcb, BT_HDR *p_buf)
{
    ...
    if (gatt_bqb_getFlag()) {
        ...
        if (pseudo_op_code < GATT_OP_CODE_MAX) {
            error = GATT_REQ_NOT_SUPPORTED;
        } else {
            error = GATT_REQ_NOT_SUPPORTED;
        }
        gatt_bqb_setFlag(0);
        gatt_send_error_rsp(p_tcb, error, op_code, 0, false);
        return;
    }
...
/* PTS: GATT/SR/UNS/BI-01-C */
uint32_t g_gatt_bqb_flag;
void gatt_bqb_setFlag(int32_t flag)
{
    g_gatt_bqb_flag = flag;
}

int32_t gatt_bqb_getFlag(void)
{
    return g_gatt_bqb_flag;
}
