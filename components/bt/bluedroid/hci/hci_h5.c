...
#ifdef RTK_COEX
static struct rtl_btpf btpf_handle;
#endif
...
static volatile int num_hci_cmd_pkts = 1;
...
static uint16_t h5_wake_up()
{
    ...
    while (NULL != (skb = h5_dequeue())) {
        ...
        bytes_sent = hci_send(hci_dev, data, data_len);
...
#ifdef RTK_COEX
        hci_process_evt(&btpf_handle, hci_skb_get_data(skb), hci_skb_get_data_length(skb));
#endif

    } else if (pkt_type == HCI_ACLDATA_PKT) {
#ifdef RTK_COEX
        l2_process_frame(&btpf_handle, (uint8_t *)hci_skb_get_data(skb), hci_skb_get_data_length(skb), 0);
#endif
    }
...
void hci_event(hci_event_t event, uint32_t size, void *priv)
{
...
static int hci_driver_send_cmd_cb(uint16_t opcode, uint8_t* send_data, uint32_t send_len, uint8_t* resp_data, uint32_t *resp_len)
{
    ...
    BT_HDR *response;
    BT_HDR *command = (BT_HDR *)osi_calloc(sizeof(BT_HDR) + send_len + HCI_COMMAND_PREAMBLE_SIZE);
    assert(command);

    command->event = 0;
    command->offset = 0;
    command->layer_specific = 0;
    command->len = send_len + HCI_COMMAND_PREAMBLE_SIZE;
    p = command->data;

    UINT16_TO_STREAM(p, opcode);
    UINT8_TO_STREAM(p, send_len);

    if (send_data > 0 && send_data != NULL) {
        ARRAY_TO_STREAM(p, send_data, send_len);
    }

    response = future_await(hci->transmit_command_futured(command));
    ...
    if (*resp_len < response->len - HCI_COMMAND_PREAMBLE_SIZE) {
        err = -1;
        goto end;
    }

    memcpy(resp_data, response->data + HCI_COMMAND_PREAMBLE_SIZE + 2, response->len - HCI_COMMAND_PREAMBLE_SIZE - 2);

    *resp_len = response->len - HCI_COMMAND_PREAMBLE_SIZE - 2;
...
#ifdef RTK_COEX
void rtk_vendor_cmd_to_fw(uint16_t opcode, uint32_t send_len, uint8_t* send_data)
{
    uint8_t *p;
    BT_HDR *command = (BT_HDR *)osi_calloc(sizeof(BT_HDR) + send_len + HCI_COMMAND_PREAMBLE_SIZE);
    assert(command);
    ...
    hci->transmit_command(command, NULL, NULL, NULL);
}
#endif
...
void hci_h5_int_init(hci_h5_hal_callbacks_t *h5_callbacks)
{
    ...
    hci_dev = hci_open("hci");

    aos_check(hci_dev, EIO);

    hci_set_event(hci_dev, hci_event, NULL);

    hci_start(hci_dev, hci_driver_send_cmd_cb);
}
...
uint16_t  hci_h5_parse_msg(uint8_t *byte, uint16_t count)
{
    ...
    return h5_recv(&rtk_h5, &h5_byte, count);
}
...
{
    uint32_t read_len = hci_recv(hci_dev, data_buffer, sizeof(data_buffer));
...
#ifdef RTK_COEX
    l2_process_frame(&btpf_handle, data, length, 1);
#endif
    skb = hci_skb_alloc_and_init(type, data, length);
