...
static void uart_event(aos_dev_t *dev, int event_id, void *priv)
{
    if (event_id == USART_EVENT_READ) {
        hci_hal_h4_task_post(100);
    }
}

static aos_dev_t *dev_uart_open()
{
    uart_config_t config;
    aos_dev_t *uart_dev = uart_open_id("uart", CONFIG_HCI_UART_ID);
...
void event_uart_has_bytes(fixed_queue_t *queue)
{
    BT_HDR *packet;
