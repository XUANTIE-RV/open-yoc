...
#define HCI_HOST_TASK_PRIO              (BT_TASK_DEFAULT_PRIORITIES - 4)
...
osi_thread_t *hci_host_thread = NULL;
...
int hci_start_up(void)
{
    ...
    HCI_TRACE_DEBUG("%s start", __func__);
    ...
    ret = hal->open(&hal_callbacks, hci_host_thread);
    if (ret == false)
        goto error;

    hci_host_startup_flag = true;
    HCI_TRACE_DEBUG("%s end", __func__);
    return 0;
error:
    HCI_TRACE_DEBUG("%s error ret = %d", __func__, ret);
...
bool hci_host_startup_flag_get(void)
{
    return hci_host_startup_flag;
}
...
static int hci_layer_init_env(void)
{
    ...
    cmd_wait_q->commands_pending_response = osi_list_new(NULL);
...
static void hci_layer_deinit_env(void)
{
    ...
    cmd_wait_q = &hci_host_env.cmd_waiting_q;
    osi_list_free(cmd_wait_q->commands_pending_response);
...
static void event_command_ready(fixed_queue_t *queue)
{
    ...
    HCI_TRACE_DEBUG("%s start: wait_entry opcode = %x", __func__, wait_entry->opcode);
    ...
    osi_list_append(cmd_wait_q->commands_pending_response, wait_entry);
...
static void restart_command_waiting_response_timer(command_waiting_response_t *cmd_wait_q)
{
    ...
    if (!osi_list_is_empty(cmd_wait_q->commands_pending_response)) {
...
static void command_timed_out(void *context)
{
    ...
    osi_mutex_lock(&cmd_wait_q->commands_pending_response_lock, OSI_MUTEX_MAX_TIMEOUT);
    wait_entry = (osi_list_is_empty(cmd_wait_q->commands_pending_response) ?
                  NULL : osi_list_front(cmd_wait_q->commands_pending_response));
    osi_mutex_unlock(&cmd_wait_q->commands_pending_response_lock);
...
static waiting_command_t *get_waiting_command(command_opcode_t opcode)
{
    ...
    for (const list_node_t *node = osi_list_begin(cmd_wait_q->commands_pending_response);
            node != osi_list_end(cmd_wait_q->commands_pending_response);
            node = osi_list_next(node)) {
        waiting_command_t *wait_entry = osi_list_node(node);
