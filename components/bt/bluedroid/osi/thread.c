...
struct osi_thread {
  aos_task_t thread_handle;                  /*!< Store the thread object */
...
static void osi_thread_stop(osi_thread_t *thread)
{
    ...
    //join
    osi_thread_join(thread, 10000000); //wait 1000ms
   
    // //if join failed, delete the task here
    // if (ret != 0 && thread->thread_handle) {
    //     aos_(thread->thread_handle);
    // }
}
...
osi_thread_t *osi_thread_create(const char *name, size_t stack_size, int priority, osi_thread_core_t core, uint8_t work_queue_num)
{
    ...
    if (aos_task_new_ext(&thread->thread_handle, name, osi_thread_run,
                         &start_arg, stack_size, priority) != 0) {
        goto _err;
    }
...
_err:
    ...
    if (thread) {
        if (osi_sem_is_valid(&start_arg.start_sem)) {
            osi_sem_free(&start_arg.start_sem);
        }
...
const char *osi_thread_name(osi_thread_t *thread)
{
    ...
    return aos_task_get_name(&thread->thread_handle);
}
