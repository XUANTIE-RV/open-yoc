...
fixed_queue_t *fixed_queue_new(size_t capacity)
{
    ...
    if (!osi_sem_is_valid(&ret->enqueue_sem)) {
        goto error;
    }

    osi_sem_new(&ret->dequeue_sem, capacity, 0);
    if (!osi_sem_is_valid(&ret->dequeue_sem)) {
        goto error;
    }
...
bool fixed_queue_enqueue(fixed_queue_t *queue, void *data, uint32_t timeout)
{
    ...
    ret = osi_sem_take(&queue->enqueue_sem, timeout);
    if (ret != 0) {
        OSI_TRACE_ERROR("%s, sem take failed %d", __func__, ret);
        return false;
    }

    ret = osi_mutex_lock(&queue->lock, OSI_MUTEX_MAX_TIMEOUT);
    if (ret != 0) {
        OSI_TRACE_ERROR("%s error in mutex lock %x", __func__, ret);
    }

    osi_list_append(queue->list, data);
    osi_mutex_unlock(&queue->lock);
...
void *fixed_queue_dequeue(fixed_queue_t *queue, uint32_t timeout)
{
    ...
    ret1 = osi_mutex_lock(&queue->lock, OSI_MUTEX_MAX_TIMEOUT);
    if (ret1 != 0) {
        OSI_TRACE_ERROR("%s error in mutex unlock %x", __func__, ret1);
    }
