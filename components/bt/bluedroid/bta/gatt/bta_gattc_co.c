...
static bool cacheOpen(BD_ADDR bda, bool to_save, UINT8 *index)
{
    ...
    if (((*index = bta_gattc_co_find_addr_in_cache(bda)) != INVALID_ADDR_NUM) ||
        ...
        if (cache_env->cache_addr[*index].is_open) {
           ...
           // if ((status = nvs_open(fname, NVS_READWRITE, &cache_env->cache_addr[*index].cache_fp)) == BT_OK) {
    ...
    return ((status == BT_OK) ? true : false);
}
    ...
    if ((index = bta_gattc_co_find_addr_in_cache(bda)) != INVALID_ADDR_NUM) {
        ...
        if(cache_env->num_addr > 0) {
            ...
            if (cache_env->is_open) {
                //if (nvs_set_blob(cache_env->addr_fp, cache_key, p_buf, length) != BT_OK) {
...
tBTA_GATT_STATUS bta_gattc_co_cache_load(tBTA_GATTC_NV_ATTR *attr, UINT8 index)
{
   ...
   // bt_err_t err_code = nvs_get_blob(cache_env->cache_addr[index].cache_fp, cache_key, attr, &length);
#if (!UC_BT_STACK_NO_LOG)
    num_attr = length / sizeof(tBTA_GATTC_NV_ATTR);
#endif
    //status = (err_code == BT_OK && length != 0) ? BTA_GATT_OK : BTA_GATT_ERROR;
                              ...
                              tBTA_GATTC_NV_ATTR *p_attr_list)
{
    ...
    //    bt_err_t err_code = nvs_set_blob(cache_env->cache_addr[index].cache_fp, cache_key,
       //                                   p_attr_list, sizeof(tBTA_GATTC_NV_ATTR)*num_attr)//;
       // status = (err_code == BT_OK) ? BTA_GATT_OK : BTA_GATT_ERROR;
    } else {
        status = BTA_GATT_ERROR;
    }
...
/*
    if ((err_code = nvs_open(cache_addr, NVS_READWRITE, &fp)) == BT_OK) {
        cache_env->addr_fp = fp;
        cache_env->is_open = TRUE;
        // Read previously saved blob if available
        if ((err_code = nvs_get_blob(fp, cache_key, p_buf, &length)) != BT_OK) {
            if(err_code != YOC_ERR_NVS_NOT_FOUND) {
        ...
        if(addr_info) {
           // nvs_close(addr_info->cache_fp);
            ...
            if(addr_info->assoc_addr) {
                osi_list_free(addr_info->assoc_addr);
            }
        }
    }
...
/* 
    ...
    if (cache_env->is_open) {
//if ((err_code = nvs_set_blob(cache_env->addr_fp, cache_key, p_buf, length)) != BT_OK) {
...
BOOLEAN bta_gattc_co_cache_new_assoc_list(BD_ADDR src_addr, UINT8 index)
{
    ...
    addr_info->assoc_addr = osi_list_new(osi_free_func);
...
BOOLEAN bta_gattc_co_cache_append_assoc_addr(BD_ADDR src_addr, BD_ADDR assoc_addr)
{
    ...
    if ((addr_index = bta_gattc_co_find_addr_in_cache(src_addr)) != INVALID_ADDR_NUM) {
        ...
        if (addr_info->assoc_addr == NULL) {
            addr_info->assoc_addr = osi_list_new(NULL);
        }
        return osi_list_append(addr_info->assoc_addr, p_assoc_buf);
    }
...
BOOLEAN bta_gattc_co_cache_remove_assoc_addr(BD_ADDR src_addr, BD_ADDR assoc_addr)
{
    ...
    if ((addr_index = bta_gattc_co_find_addr_in_cache(src_addr)) != INVALID_ADDR_NUM) {
        addr_info = &cache_env->cache_addr[addr_index];
        if (addr_info->assoc_addr != NULL) {
            for (list_node_t *sn = osi_list_begin(addr_info->assoc_addr);
             sn != osi_list_end(addr_info->assoc_addr); sn = osi_list_next(sn)) {
                void *addr = osi_list_node(sn);
                if (!memcmp(addr, assoc_addr, sizeof(BD_ADDR))) {
                    return osi_list_remove(addr_info->assoc_addr, addr);
                }
            }
            //return osi_list_remove(addr_info->assoc_addr, assoc_addr);
        } else {
            return FALSE;
        }
    }
...
UINT8* bta_gattc_co_cache_find_src_addr(BD_ADDR assoc_addr, UINT8 *index)
{
    ...
    for (int i = 0; i < num; i++) {
       for (const list_node_t *node = osi_list_begin(addr_info->assoc_addr); node != osi_list_end(addr_info->assoc_addr);
            node = osi_list_next(node)) {
            addr_data = (UINT8 *)osi_list_node(node);
...
BOOLEAN bta_gattc_co_cache_clear_assoc_addr(BD_ADDR src_addr)
{
    ...
    if ((addr_index = bta_gattc_co_find_addr_in_cache(src_addr)) != INVALID_ADDR_NUM) {
        addr_info = &cache_env->cache_addr[addr_index];
        if (addr_info->assoc_addr != NULL) {
            osi_list_clear(addr_info->assoc_addr);
        } else {
            return FALSE;
        }
        return TRUE;
