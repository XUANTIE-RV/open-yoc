...
unsigned char bta_pts_abort_flag = false;
...
#else /* #if 0*/
    if(bta_pts_abort_flag){
        AVDT_AbortReq(p_scb->avdt_handle);
        bta_pts_abort_flag = false;
    }
#endif
}
...
void bta_av_security_ind (tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    if (bta_av_cb.features & BTA_AV_FEAT_PROTECT) {
        ...
        */
        ...
        AVDT_SecurityRsp(p_scb->avdt_handle, p_scb->avdt_label, 0,  protect_req.p_data, protect_req.len);
    }
...
void bta_av_str_stopped (tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    if (BTA_AV_CHNL_AUDIO == p_scb->chnl) {
        while (!osi_list_is_empty(p_scb->a2d_list)) {
            p_buf = (BT_HDR *)osi_list_front(p_scb->a2d_list);
            osi_list_remove(p_scb->a2d_list, p_buf);
            osi_free(p_buf);
        }
...
void bta_av_data_path (tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    */
    ...
    if (!osi_list_is_empty(p_scb->a2d_list)) {
        p_buf = (BT_HDR *)osi_list_front(p_scb->a2d_list);
        osi_list_remove(p_scb->a2d_list, p_buf);
        ...
        if (p_scb->l2c_bufs < (BTA_AV_QUEUE_DATA_CHK_NUM)) {
            ...
            */
            ...
            if (new_buf) {
                 ...
                 * put it in queue */
                osi_list_append(p_scb->a2d_list, p_buf);
            } else {
                /* just dequeue it from the a2d_list */
                if (osi_list_length(p_scb->a2d_list) < 3) {
                    /* put it back to the queue */
                    osi_list_prepend(p_scb->a2d_list, p_buf);
                } else {
