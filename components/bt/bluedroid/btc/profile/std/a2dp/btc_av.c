...
{
    yoc_a2d_cb_t btc_a2d_cb = (yoc_a2d_cb_t)btc_profile_cb_get(BTC_PID_A2DP);
    if (btc_a2d_cb) {
...
**  Static functions
******************************************************************************/
static void btc_report_connection_state(yoc_a2d_connection_state_t state, bt_bdaddr_t *bd_addr, int disc_rsn)
{
    // todo: add callback for SRC
    yoc_a2d_cb_param_t param;
    memset(&param, 0, sizeof(yoc_a2d_cb_param_t));

    param.conn_stat.state = state;
    if (bd_addr) {
        memcpy(param.conn_stat.remote_bda, bd_addr, sizeof(yoc_bd_addr_t));
    }
    if (state == YOC_A2D_CONNECTION_STATE_DISCONNECTED) {
        param.conn_stat.disc_rsn = (disc_rsn == 0) ? YOC_A2D_DISC_RSN_NORMAL :
                                   YOC_A2D_DISC_RSN_ABNORMAL;
    }
    btc_a2d_cb_to_app(YOC_A2D_CONNECTION_STATE_EVT, &param);
}

static void btc_report_audio_state(yoc_a2d_audio_state_t state, bt_bdaddr_t *bd_addr)
{
    // todo: add callback for SRC
    yoc_a2d_cb_param_t param;
    memset(&param, 0, sizeof(yoc_a2d_cb_param_t));
    ...
    if (bd_addr) {
        memcpy(param.audio_stat.remote_bda, bd_addr, sizeof(yoc_bd_addr_t));
    }
    btc_a2d_cb_to_app(YOC_A2D_AUDIO_STATE_EVT, &param);
}
...
static BOOLEAN btc_av_state_opening_handler(btc_sm_event_t event, void *p_data)
{
    ...
    case BTC_SM_ENTER_EVT:
        ...
        btc_report_connection_state(YOC_A2D_CONNECTION_STATE_CONNECTING, &(btc_av_cb.peer_bda), 0);
        break;
    ...
    case BTA_AV_OPEN_EVT: {
        ...
        if (p_bta_data->open.status == BTA_AV_SUCCESS) {
            ...
            BTC_TRACE_WARNING("BTA_AV_OPEN_EVT::FAILED status: %d\n",
                     p_bta_data->open.status );
            state = YOC_A2D_CONNECTION_STATE_DISCONNECTED;
    ...
    case BTC_AV_SINK_CONFIG_REQ_EVT: {
        if (btc_av_cb.peer_sep == AVDT_TSEP_SRC) {
            yoc_a2d_cb_param_t param;
            memcpy(param.audio_cfg.remote_bda, &btc_av_cb.peer_bda, sizeof(yoc_bd_addr_t));
            memcpy(&param.audio_cfg.mcc, p_data, sizeof(yoc_a2d_mcc_t));
            btc_a2d_cb_to_app(YOC_A2D_AUDIO_CFG_EVT, &param);
        }
    } break;
    ...
    case BTC_AV_CONNECT_REQ_EVT:
        ...
        if (memcmp ((bt_bdaddr_t *)p_data, &(btc_av_cb.peer_bda),
            ...
            btc_report_connection_state(YOC_A2D_CONNECTION_STATE_DISCONNECTED, (bt_bdaddr_t *)p_data, 0);
            btc_queue_advance();
            break;
        }
...
static BOOLEAN btc_av_state_closing_handler(btc_sm_event_t event, void *p_data)
{
    ...
    case BTA_AV_CLOSE_EVT: {
        ...
        btc_report_connection_state(YOC_A2D_CONNECTION_STATE_DISCONNECTED, &(btc_av_cb.peer_bda),
                                    close->disc_rsn);
...
static BOOLEAN btc_av_state_opened_handler(btc_sm_event_t event, void *p_data)
{
    ...
    case BTC_AV_DISCONNECT_REQ_EVT:
        ...
        /* inform the application that we are disconnecting */
        btc_report_connection_state(YOC_A2D_CONNECTION_STATE_DISCONNECTING, &(btc_av_cb.peer_bda), 0);
        break;
    ...
    case BTA_AV_CLOSE_EVT: {
        ...
        if (btc_av_cb.flags & BTC_AV_FLAG_PENDING_START) {
            btc_a2dp_control_command_ack(YOC_A2D_MEDIA_CTRL_ACK_FAILURE);
            /* pending start flag will be cleared when exit current state */
        }
        btc_sm_change_state(btc_av_cb.sm_handle, BTC_AV_STATE_IDLE);
        break;
    ...
    case BTC_AV_CONNECT_REQ_EVT:
        if (memcmp (&((btc_av_connect_req_t *)p_data)->target_bda, &(btc_av_cb.peer_bda),
            ...
            btc_report_connection_state(YOC_A2D_CONNECTION_STATE_DISCONNECTED,
                                        (bt_bdaddr_t *)p_data, YOC_A2D_DISC_RSN_NORMAL);
        }
        btc_queue_advance();
        break;
...
static BOOLEAN btc_av_state_started_handler(btc_sm_event_t event, void *p_data)
{
    ...
    case BTC_SM_ENTER_EVT:
        ...
        btc_report_audio_state(YOC_A2D_AUDIO_STATE_STARTED, &(btc_av_cb.peer_bda));
    ...
    case BTA_AV_SUSPEND_EVT:
        ...
        if (p_av->suspend.initiator != TRUE) {
            ...
            btc_report_audio_state(YOC_A2D_AUDIO_STATE_REMOTE_SUSPEND, &(btc_av_cb.peer_bda));
        } else {
            btc_report_audio_state(YOC_A2D_AUDIO_STATE_STOPPED, &(btc_av_cb.peer_bda));
        }
...
static void bte_av_media_callback(tBTA_AV_EVT event, tBTA_AV_MEDIA *p_data)
{
    ...
    if (event == BTA_AV_MEDIA_SINK_CFG_EVT) {
        ...
        if (a2d_status == A2D_SUCCESS) {
            ...
            arg.mcc.type = YOC_A2D_MCT_SBC;
            memcpy(arg.mcc.cie.sbc, (uint8_t *)p_data + 3, YOC_A2D_CIE_LEN_SBC);
...
bt_status_t btc_av_execute_service(BOOLEAN b_enable, UINT8 tsep)
{
    if (b_enable) {
        ...
        BTA_AvEnable(BTA_SEC_AUTHENTICATE, (BTA_AV_FEAT_NO_SCO_SSPD)
                     ...
                     | BTA_AV_FEAT_RCCT | BTA_AV_FEAT_ADV_CTRL | BTA_AV_FEAT_PROTECT,
                     bte_av_callback);
