...
#define BTC_TASK_PRIO                   (BT_TASK_DEFAULT_PRIORITIES - 3)
...
#if GATTS_INCLUDED == TRUE && GATT_DYNAMIC_MEMORY == TRUE
    if ((btc_creat_tab_env_ptr = (yoc_btc_creat_tab_t *)osi_malloc(sizeof(yoc_btc_creat_tab_t))) == NULL) {
        goto error_exit;
    }
    memset((void *)btc_creat_tab_env_ptr, 0, sizeof(yoc_btc_creat_tab_t));
