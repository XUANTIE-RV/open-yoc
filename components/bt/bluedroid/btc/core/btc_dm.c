...
static void btc_dm_auth_cmpl_evt (tBTA_DM_AUTH_CMPL *p_auth_cmpl)
{
    ...
    memcpy(param.auth_cmpl.bda, p_auth_cmpl->bd_addr, BT_BD_ADDR_LEN);
    memcpy(param.auth_cmpl.device_name, p_auth_cmpl->bd_name, YOC_BT_GAP_MAX_BDNAME_LEN + 1);

    ret = btc_transfer_context(&msg, &param,
                               sizeof(yoc_bt_gap_cb_param_t), NULL);
...
static void btc_dm_pin_req_evt(tBTA_DM_PIN_REQ *p_pin_req)
{
    ...
    memcpy(param.pin_req.bda, p_pin_req->bd_addr, BT_BD_ADDR_LEN);
...
static void btc_dm_sp_cfm_req_evt(tBTA_DM_SP_CFM_REQ *p_cfm_req)
{
    ...
    memcpy(param.cfm_req.bda, p_cfm_req->bd_addr, BT_BD_ADDR_LEN);
...
static void btc_dm_sp_key_notif_evt(tBTA_DM_SP_KEY_NOTIF *p_key_notif)
{
    ...
    memcpy(param.key_notif.bda, p_key_notif->bd_addr, BT_BD_ADDR_LEN);
...
static void btc_dm_sp_key_req_evt(tBTA_DM_SP_KEY_REQ *p_key_req)
{
    ...
    memcpy(param.key_req.bda, p_key_req->bd_addr, BT_BD_ADDR_LEN);
...
void btc_dm_sec_cb_handler(btc_msg_t *msg)
{
    ...
    yoc_ble_gap_cb_param_t param = {0};
    btc_msg_t ble_msg = {0};
    bool rsp_app = false;
    ...
    case BTA_DM_ENABLE_EVT: {
        ...
        if (p_data->enable.status == BTA_SUCCESS) {
            const char *initial_device_name = "T-HEAD";
    ...
    case BTA_DM_BLE_DEV_UNPAIRED_EVT: {
        ...
        memcpy(bd_addr.address, p_data->link_down.bd_addr, sizeof(BD_ADDR));
        btm_set_bond_type_dev(p_data->link_down.bd_addr, BOND_TYPE_UNKNOWN);
        param.remove_bond_dev_cmpl.status = YOC_BT_STATUS_FAIL;
        ...
        ble_msg.act = YOC_GAP_BLE_REMOVE_BOND_DEV_COMPLETE_EVT;
    ...
    case BTA_DM_BLE_AUTH_CMPL_EVT: {
        rsp_app = true;
        ble_msg.act = YOC_GAP_BLE_AUTH_CMPL_EVT;
    ...
    case BTA_DM_BLE_KEY_EVT: {
        rsp_app = true;
        ble_msg.act = YOC_GAP_BLE_KEY_EVT;
    ...
    case BTA_DM_BLE_PASSKEY_REQ_EVT: {
        rsp_app = true;
        ble_msg.act = YOC_GAP_BLE_PASSKEY_REQ_EVT;
        memcpy(param.ble_security.ble_req.bd_addr, p_data->ble_req.bd_addr, BD_ADDR_LEN);
        break;
    }
    case BTA_DM_BLE_OOB_REQ_EVT: {
        rsp_app = true;
        ble_msg.act = YOC_GAP_BLE_OOB_REQ_EVT;
        memcpy(param.ble_security.ble_req.bd_addr, p_data->ble_req.bd_addr, BD_ADDR_LEN);
        break;
    }
    case BTA_DM_BLE_LOCAL_IR_EVT: {
        rsp_app = true;
        ble_msg.act = YOC_GAP_BLE_LOCAL_IR_EVT;
    ...
    case BTA_DM_BLE_LOCAL_ER_EVT: {
        rsp_app = true;
        ble_msg.act = YOC_GAP_BLE_LOCAL_ER_EVT;
        memcpy(&param.ble_security.ble_id_keys, &p_data->ble_id_keys, sizeof(tBTA_BLE_LOCAL_ID_KEYS));
    ...
    case BTA_DM_BLE_NC_REQ_EVT: {
        rsp_app = true;
        ble_msg.act = YOC_GAP_BLE_NC_REQ_EVT;
    ...
    if (rsp_app) {
        ret = btc_transfer_context(&ble_msg, &param,
                                   sizeof(yoc_ble_gap_cb_param_t), NULL);
