...
#ifndef _BLE_MESH_BLE_MESH_NETWORKING_API_H_
#define _BLE_MESH_BLE_MESH_NETWORKING_API_H_
...
typedef void (* ble_mesh_model_cb_t)(ble_mesh_model_cb_event_t event,
 ...
 *                  application layer;
 *                - If users call ble_mesh_server/client_model_send, this callback notifies the
 ...
 *                For example, ble_mesh_register_config_client_callback is the register
 ...
 *                      the user_data(defined using ble_mesh_client_t_) of the client model
 ...
 *                publication message (@ref ble_mesh_model_pub_t.msg) contains a valid
 *                message to be sent. And if users want to update the publishing message,
 *                this API should be called in BLE_MESH_BLE_MESH_MODEL_PUBLISH_UPDATE_EVT
 ...
 * @param[in]    length: Message length (exclude the message opcode).
 * @param[in]    data: Parameters of the Access Payload (exclude the message opcode) to be sent.
 * @param[in]    device_role: Role of the device (node/provisioner) publishing the message of the type ble_mesh_dev_role_t.
 *
 * @return       BT_OK on success or error code otherwise.
 *
 */
 ...
 *                needs to be reprovisioned. The API function ble_mesh_node_prov_enable()
 ...
 *
 * @note         index is obtained from the parameters of BLE_MESH_BLE_MESH_PROVISIONER_PROV_COMPLETE_EVT.
 *
 * @return       BT_OK on success or error code otherwise.
 *
 */
 ...
 *
 */
bt_err_t ble_mesh_provisioner_add_local_app_key(const uint8_t app_key[16], uint16_t net_idx, uint16_t app_idx);
...
#endif /* _BLE_MESH_BLE_MESH_NETWORKING_API_H_ */
