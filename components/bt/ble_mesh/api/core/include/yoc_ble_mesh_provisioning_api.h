...
#ifndef _BLE_MESH_BLE_MESH_PROVISIONING_API_H_
#define _BLE_MESH_BLE_MESH_PROVISIONING_API_H_

#include "yoc_ble_mesh_defs.h"

/** @brief: event, event code of provisioning events; param, parameters of provisioning events */
typedef void (* ble_mesh_prov_cb_t)(ble_mesh_prov_cb_event_t event,
 ...
 *
 */
bt_err_t ble_mesh_register_prov_callback(ble_mesh_prov_cb_t callback);
 ...
 *
 */
bt_err_t ble_mesh_node_prov_enable(ble_mesh_prov_bearer_t bearers);
 ...
 *
 */
bt_err_t ble_mesh_node_prov_disable(ble_mesh_prov_bearer_t bearers);
 ...
 * @return       BT_OK on success or error code otherwise.
 */
bt_err_t ble_mesh_node_set_oob_pub_key(uint8_t pub_key_x[32], uint8_t pub_key_y[32],
                uint8_t private_key[32]);

/**
 * @brief        Provide provisioning input OOB number.
 *
 * @note         This is intended to be called if the user has received BLE_MESH_BLE_MESH_NODE_PROV_INPUT_EVT
 *               with BLE_MESH_BLE_MESH_ENTER_NUMBER as the action.
 *
 ...
 *
 */
bt_err_t ble_mesh_node_input_number(uint32_t number);

/**
 * @brief        Provide provisioning input OOB string.
 *
 * @note         This is intended to be called if the user has received BLE_MESH_BLE_MESH_NODE_PROV_INPUT_EVT
 *               with BLE_MESH_BLE_MESH_ENTER_STRING as the action.
 *
 ...
 *
 */
bt_err_t ble_mesh_node_input_string(const char *string);
 ...
 *
 */
bt_err_t ble_mesh_set_unprovisioned_device_name(const char *name);
 ...
 * @return       BT_OK on success or error code otherwise.
 */
bt_err_t ble_mesh_provisioner_read_oob_pub_key(uint8_t link_idx, uint8_t pub_key_x[32],
            uint8_t pub_key_y[32]);

/**
 * @brief        Provide provisioning input OOB string.
 *
 *               This is intended to be called after the ble_mesh_prov_t prov_input_num
 *               callback has been called with BLE_MESH_BLE_MESH_ENTER_STRING as the action.
 *
 ...
 *
 */
bt_err_t ble_mesh_provisioner_input_string(const char *string, uint8_t link_idx);

/**
 * @brief        Provide provisioning input OOB number.
 *
 *               This is intended to be called after the ble_mesh_prov_t prov_input_num
 *               callback has been called with BLE_MESH_BLE_MESH_ENTER_NUMBER as the action.
 *
 ...
 *
 */
bt_err_t ble_mesh_provisioner_input_number(uint32_t number, uint8_t link_idx);
 ...
 *
 */
bt_err_t ble_mesh_provisioner_prov_enable(ble_mesh_prov_bearer_t bearers);
 ...
 *
 */
bt_err_t ble_mesh_provisioner_prov_disable(ble_mesh_prov_bearer_t bearers);
 ...
 *                    BLE_MESH_BLE_MESH_PROV_ADV and BLE_MESH_BLE_MESH_PROV_GATT enabled, and cannot set flags
 ...
 *                  ble_mesh_register_prov_callback. And in the callback, the Provisioner
 ...
 *                    device address can be added, bearers shall be set with BLE_MESH_BLE_MESH_PROV_ADV
 ...
 *                    address need to be added, bearers shall be set with BLE_MESH_BLE_MESH_PROV_GATT enabled,
 ...
 *                    of BLE_MESH_BLE_MESH_PROV_ADV and BLE_MESH_BLE_MESH_PROV_GATT enabled (recommend to enable the
 ...
 *                  - Note: BLE_MESH_BLE_MESH_PROV_ADV, BLE_MESH_BLE_MESH_PROV_GATT and ADD_DEV_START_PROV_NOW_FLAG
 *                          can not be enabled at the same time.
 *
 */
bt_err_t ble_mesh_provisioner_add_unprov_dev(ble_mesh_unprov_dev_add_t *add_dev,
        ble_mesh_dev_add_flag_t flags);
 ...
 *
 */
bt_err_t ble_mesh_provisioner_delete_dev(ble_mesh_device_delete_t *del_dev);
 ...
 *
 */
typedef void (*ble_mesh_prov_adv_cb_t)(const yoc_bd_addr_t addr, const yoc_ble_addr_type_t addr_type,
        const uint8_t adv_type, const uint8_t *dev_uuid,
        uint16_t oob_info, ble_mesh_prov_bearer_t bearer);
 ...
 *
 */
bt_err_t ble_mesh_provisioner_set_dev_uuid_match(const uint8_t *match_val, uint8_t match_len,
 ...
 *
 */
bt_err_t ble_mesh_provisioner_set_prov_data_info(ble_mesh_prov_data_info_t *prov_data_info);
 ...
 *
 */
bt_err_t ble_mesh_set_fast_prov_info(ble_mesh_fast_prov_info_t *fast_prov_info);
 ...
 *
 */
bt_err_t ble_mesh_set_fast_prov_action(ble_mesh_fast_prov_action_t action);

#endif /* _BLE_MESH_BLE_MESH_PROVISIONING_API_H_ */
