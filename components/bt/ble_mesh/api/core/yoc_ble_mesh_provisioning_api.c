...
bt_err_t ble_mesh_register_prov_callback(ble_mesh_prov_cb_t callback)
{
    ...
    return (btc_profile_cb_set(BTC_PID_PROV, callback) == 0 ? BT_OK : BT_FAIL);
}
...
bt_err_t ble_mesh_set_unprovisioned_device_name(const char *name)
{
    ...
    if (!name || strlen(name) > BLE_MESH_BLE_MESH_DEVICE_NAME_MAX_LEN) {
        return BT_ERR_INVALID_ARG;
    }
        ...
        ble_mesh_dev_add_flag_t flags)
{
    ...
    memcpy(arg.provisioner_dev_add.add_dev.addr, add_dev->addr, sizeof(yoc_bd_addr_t));
...
bt_err_t ble_mesh_provisioner_delete_dev(ble_mesh_device_delete_t *del_dev)
{
    ...
    if (del_dev->flag & DEL_DEV_ADDR_FLAG) {
        ...
        memcpy(arg.provisioner_dev_del.del_dev.addr, del_dev->addr, sizeof(yoc_bd_addr_t));
