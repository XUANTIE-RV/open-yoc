...
#define XACT_SEG_DATA(_idx, _seg) (&_link[_idx].rx.buf->data[20 + ((_seg - 1) * 23)])
#define XACT_SEG_RECV(_idx, _seg) (_link[_idx].rx.seg &= ~(1 << (_seg)))
...
/** Provisioner _link structure allocation
 * |--------------------------------------------------------|
...
static struct prov_link _link[BLE_MESH_PROV_SAME_TIME];
...
{                                       \
    if (_link[_idx].member) {            \
        osi_free(_link[_idx].member);    \
    }                                   \
}
...
void provisioner_clear_link_conn_info(const u8_t addr[6])
{
    ...
    for (i = CONFIG_BLE_MESH_PBA_SAME_TIME; i < BLE_MESH_PROV_SAME_TIME; i++) {
        if (!memcmp(_link[i].addr.val, addr, BLE_MESH_ADDR_LEN)) {
            _link[i].connecting = false;
            _link[i].conn = NULL;
            _link[i].oob_info = 0x0;
            memset(_link[i].uuid, 0, 16);
            memset(&_link[i].addr, 0, sizeof(bt_mesh_addr_t));
            bt_mesh_atomic_test_and_clear_bit(_link[i].flags, LINK_ACTIVE);
            if (bt_mesh_atomic_test_and_clear_bit(_link[i].flags, TIMEOUT_START)) {
                k_delayed_work_cancel(&_link[i].timeout);
            }
            return;
        }
    }
...
static bool is_unprov_dev_being_provision(const u8_t uuid[16])
{
    ...
    for (i = 0U; i < BLE_MESH_PROV_SAME_TIME; i++) {
        ...
        if (_link[i].linking || _link[i].connecting ||
            bt_mesh_atomic_test_bit(_link[i].flags, LINK_ACTIVE)) {
#elif defined(CONFIG_BLE_MESH_PB_ADV) && !defined(CONFIG_BLE_MESH_PB_GATT)
        if (_link[i].linking || bt_mesh_atomic_test_bit(_link[i].flags, LINK_ACTIVE)) {
#else
        if (_link[i].connecting || bt_mesh_atomic_test_bit(_link[i].flags, LINK_ACTIVE)) {
#endif
            if (!memcmp(_link[i].uuid, uuid, 16)) {
                // BT_ERR("%s, Device is being provisioned", __func__);
                return true;
            }
        }
    }
...
static int provisioner_check_unprov_dev_info(const u8_t uuid[16])
{
    ...
    if (is_unprov_dev_uuid_match(uuid) == false) {
        BT_ERR("%s, Device uuid is not matched", __func__);
        return -EIO;
    }
     ...
     * Here we check both PB-GATT and PB-ADV _link status.
     */
                ...
                const bt_mesh_addr_t *addr, u16_t oob_info)
{
    ...
    for (i = 0U; i < CONFIG_BLE_MESH_PBA_SAME_TIME; i++) {
        if (!bt_mesh_atomic_test_bit(_link[i].flags, LINK_ACTIVE) && !_link[i].linking) {
            memcpy(_link[i].uuid, uuid, 16);
            _link[i].oob_info = oob_info;
            if (addr_cmp && (addr->type <= BLE_MESH_ADDR_RANDOM)) {
                _link[i].addr.type = addr->type;
                memcpy(_link[i].addr.val, addr->val, BLE_MESH_ADDR_LEN);
            }
            send_link_open(i);
            osi_mutex_unlock(&prov_ctx.pb_adv_lock);
            return 0;
    ...
    BT_ERR("%s, No PB-ADV _link is available", __func__);
    osi_mutex_unlock(&prov_ctx.pb_adv_lock);
    return -ENOMEM;
}
                ...
                const bt_mesh_addr_t *addr, u16_t oob_info)
{
    ...
    for (i = CONFIG_BLE_MESH_PBA_SAME_TIME; i < BLE_MESH_PROV_SAME_TIME; i++) {
        if (!_link[i].connecting && !bt_mesh_atomic_test_bit(_link[i].flags, LINK_ACTIVE)) {
            ...
            if (bt_mesh_gattc_conn_create(&_link[i].addr, BLE_MESH_UUID_MESH_PROV_VAL)) {
                memset(_link[i].uuid, 0, 16);
                _link[i].oob_info = 0x0;
                memset(&_link[i].addr, 0, sizeof(bt_mesh_addr_t));
                osi_mutex_unlock(&prov_ctx.pb_gatt_lock);
            ...
            _link[i].connecting = true;
            osi_mutex_unlock(&prov_ctx.pb_gatt_lock);
            return 0;
        }
    }

    BT_ERR("%s, No PB-GATT _link is available", __func__);
    osi_mutex_unlock(&prov_ctx.pb_gatt_lock);
    return -ENOMEM;
}
...
start:
    ...
    /* Check if current provisioned node count + active _link reach max limit */
    if (prov_ctx.node_count + prov_ctx.pba_count + \
        prov_ctx.pbg_count >= ARRAY_SIZE(prov_nodes)) {
        BT_WARN("%s, Node count + active _link count reach max limit", __func__);
        return -EIO;
    }
...
int bt_mesh_provisioner_delete_device(struct bt_mesh_device_delete *del_dev)
{
     ...
     * 2. device is being provisioned, need to close _link & remove from device queue.
    ...
    for (i = 0U; i < ARRAY_SIZE(_link); i++) {
        if (addr_cmp && (del_dev->addr_type <= BLE_MESH_ADDR_RANDOM)) {
            if (!memcmp(_link[i].addr.val, del_dev->addr, BLE_MESH_ADDR_LEN) &&
                    _link[i].addr.type == del_dev->addr_type) {
                addr_match = true;
            }
        }
        if (uuid_cmp) {
            if (!memcmp(_link[i].uuid, del_dev->uuid, 16)) {
                uuid_match = true;
            }
        }
...
static void buf_sent(int err, void *user_data)
{
    ...
    if (!_link[idx].tx.buf[0]) {
        return;
    }

    k_delayed_work_submit(&_link[idx].tx.retransmit, RETRANSMIT_TIMEOUT);
}
...
static void free_segments(const u8_t idx)
{
    ...
    for (i = 0U; i < ARRAY_SIZE(_link[idx].tx.buf); i++) {
        struct net_buf *buf = _link[idx].tx.buf[i];

        if (!buf) {
            break;
        }

        _link[idx].tx.buf[i] = NULL;
        /* Mark as canceled */
...
static void prov_clear_tx(const u8_t idx)
{
    ...
    k_delayed_work_cancel(&_link[idx].tx.retransmit);
...
static void reset_link(const u8_t idx, u8_t reason)
{
    ...
    if (bt_mesh_atomic_test_and_clear_bit(_link[idx].flags, TIMEOUT_START)) {
        k_delayed_work_cancel(&_link[idx].timeout);
    }
    ...
    /* Remove the _link id from exceptional list */
    bt_mesh_update_exceptional_list(BLE_MESH_EXCEP_LIST_REMOVE,
        BLE_MESH_EXCEP_INFO_MESH_LINK_ID, &_link[idx].link_id);
#endif

    /* Clear everything except the retransmit delayed work config */
    memset(&_link[idx], 0, offsetof(struct prov_link, tx.retransmit));

    _link[idx].pending_ack = XACT_NVAL;
    _link[idx].rx.prev_id  = XACT_NVAL;

    if (bt_mesh_pub_key_get()) {
        bt_mesh_atomic_set_bit(_link[idx].flags, LOCAL_PUB_KEY);
    }

    _link[idx].rx.buf = bt_mesh_pba_get_buf(idx);
...
static void ack_complete(u16_t duration, int err, void *user_data)
{
    ...
    BT_DBG("xact %u complete", _link[idx].pending_ack);
...
static void gen_prov_ack_send(const u8_t idx, u8_t xact_id)
{
    ...
    if (_link[idx].pending_ack == xact_id) {
    ...
    if (_link[idx].pending_ack == XACT_NVAL) {
        _link[idx].pending_ack = xact_id;
        complete = &cb;
    } else {
        complete = NULL;
    }

    net_buf_add_be32(buf, _link[idx].link_id);
...
static void send_reliable(const u8_t idx)
{
    ...
    _link[idx].tx.start = k_uptime_get();
    ...
    for (i = 0U; i < ARRAY_SIZE(_link[idx].tx.buf); i++) {
        ...
        if (i + 1 < ARRAY_SIZE(_link[idx].tx.buf) && _link[idx].tx.buf[i + 1]) {
...
static void send_link_open(const u8_t idx)
{
    ...
    /** Generate _link ID, and may need to check if this id is
     *  currently being used, which may will not happen ever.
     */
    bt_mesh_rand(&_link[idx].link_id, sizeof(u32_t));
    while (1) {
        for (j = 0U; j < CONFIG_BLE_MESH_PBA_SAME_TIME; j++) {
            if (bt_mesh_atomic_test_bit(_link[j].flags, LINK_ACTIVE) || _link[j].linking) {
                if (_link[idx].link_id == _link[j].link_id) {
                    bt_mesh_rand(&_link[idx].link_id, sizeof(u32_t));
                    break;
                }
            }
...
#if defined(CONFIG_BLE_MESH_USE_DUPLICATE_SCAN)
    /* Add the _link id into exceptional list */
    bt_mesh_update_exceptional_list(BLE_MESH_EXCEP_LIST_ADD,
        BLE_MESH_EXCEP_INFO_MESH_LINK_ID, &_link[idx].link_id);
#endif

    bearer_ctl_send(idx, LINK_OPEN, _link[idx].uuid, 16);
     ...
     */
    _link[idx].linking = true;

    /* Set LINK_ACTIVE just to be in compatibility with current Zephyr code */
    bt_mesh_atomic_set_bit(_link[idx].flags, LINK_ACTIVE);
...
{
    if (_link[idx].tx.trans_id > 0x7F) {
        _link[idx].tx.trans_id = 0x0;
    }
    return _link[idx].tx.trans_id++;
}
...
static int prov_send_adv(const u8_t idx, struct net_buf_simple *msg)
{
    ...
    net_buf_add_be32(start, _link[idx].link_id);
    ...
    _link[idx].tx.buf[0] = start;
    /* Changed by Espressif, get message type */
    _link[idx].tx_pdu_type = msg->data[0];
    ...
    for (seg_id = 1; msg->len > 0; seg_id++) {
        if (seg_id >= ARRAY_SIZE(_link[idx].tx.buf)) {
...
#if (CONFIG_BLE_MESH_FAST_PROV)
    if (_link[idx].tx_pdu_type >= PROV_DATA) {
        timeout = K_SECONDS(60);
    }
#endif
    if (!bt_mesh_atomic_test_and_set_bit(_link[idx].flags, TIMEOUT_START)) {
        k_delayed_work_submit(&_link[idx].timeout, timeout);
    }
...
static int prov_send_gatt(const u8_t idx, struct net_buf_simple *msg)
{
    ...
    err = provisioner_proxy_send(_link[idx].conn, BLE_MESH_PROXY_PROV, msg);
    if (err) {
    ...
    if (!bt_mesh_atomic_test_and_set_bit(_link[idx].flags, TIMEOUT_START)) {
        k_delayed_work_submit(&_link[idx].timeout, PROVISION_TIMEOUT);
    }
...
static inline int prov_send(const u8_t idx, struct net_buf_simple *buf)
{
    ...
    BT_ERR("%s, Invalid _link index %d", __func__, idx);
    return -EINVAL;
}
...
static void send_invite(const u8_t idx)
{
    ...
    _link[idx].conf_inputs[0] = prov->prov_attention;
    ...
    _link[idx].expect = PROV_CAPABILITIES;
}
...
static void prov_capabilities(const u8_t idx, const u8_t *data)
{
    ...
    _link[idx].element_num = element_num;
    ...
    memcpy(&_link[idx].conf_inputs[1], data, 11);
    ...
    memcpy(&_link[idx].conf_inputs[12], &buf.data[1], 5);
    ...
    _link[idx].auth_method = auth_method;
    _link[idx].auth_action = auth_action;
    _link[idx].auth_size   = auth_size;
...
#if defined(CONFIG_BLE_MESH_PB_ADV)
    if (idx < CONFIG_BLE_MESH_PBA_SAME_TIME) {
        _link[idx].expect_ack_for = PROV_START;
        return;
    }
#endif /* CONFIG_BLE_MESH_PB_ADV */
...
static int prov_auth(const u8_t idx, u8_t method, u8_t action, u8_t size)
{
    ...
    _link[idx].auth = (u8_t *)osi_calloc(PROV_AUTH_VAL_SIZE);
    if (!_link[idx].auth) {
        BT_ERR("%s, Failed to allocate memory", __func__);
        close_link(idx, CLOSE_REASON_FAILED);
        return -ENOMEM;
    ...
    case AUTH_METHOD_STATIC:
        ...
        memcpy(_link[idx].auth + 16 - prov->prov_static_oob_len,
               prov->prov_static_oob_val, prov->prov_static_oob_len);
        memset(_link[idx].auth, 0, 16 - prov->prov_static_oob_len);
        return 0;
    ...
    case AUTH_METHOD_INPUT:
        ...
        _link[idx].expect = PROV_INPUT_COMPLETE;
        ...
        if (input == BLE_MESH_ENTER_STRING) {
            ...
            memcpy(_link[idx].auth, str, size);
            memset(_link[idx].auth + size, 0, sizeof(_link[idx].auth) - size);
            ...
            sys_put_be32(num, &_link[idx].auth[12]);
            memset(_link[idx].auth, 0, 12);
...
static void send_confirm(const u8_t idx)
{
    ...
    BT_DBG("ConfInputs[0]   %s", bt_hex(_link[idx].conf_inputs, 64));
    BT_DBG("ConfInputs[64]  %s", bt_hex(_link[idx].conf_inputs + 64, 64));
    BT_DBG("ConfInputs[128] %s", bt_hex(_link[idx].conf_inputs + 128, 17));

    _link[idx].conf_salt = (u8_t *)osi_calloc(PROV_CONF_SALT_SIZE);
    if (!_link[idx].conf_salt) {
        BT_ERR("%s, Failed to allocate memory", __func__);
        goto fail;
    }

    _link[idx].conf_key = (u8_t *)osi_calloc(PROV_CONF_KEY_SIZE);
    if (!_link[idx].conf_key) {
        BT_ERR("%s, Failed to allocate memory", __func__);
        goto fail;
    }

    if (bt_mesh_prov_conf_salt(_link[idx].conf_inputs, _link[idx].conf_salt)) {
        BT_ERR("%s, Failed to generate confirmation salt", __func__);
        goto fail;
    }

    BT_DBG("ConfirmationSalt: %s", bt_hex(_link[idx].conf_salt, 16));

    if (bt_mesh_prov_conf_key(_link[idx].dhkey, _link[idx].conf_salt, _link[idx].conf_key)) {
        BT_ERR("%s, Failed to generate confirmation key", __func__);
        goto fail;
    }

    BT_DBG("ConfirmationKey: %s", bt_hex(_link[idx].conf_key, 16));
    ...
    if (!(prov_ctx.rand_gen_done & BIT(0))) {
        ...
        _link[idx].rand = prov_ctx.random;
    ...
    BT_DBG("LocalRandom: %s", bt_hex(_link[idx].rand, 16));

    prov_buf_init(&buf, PROV_CONFIRM);

    if (bt_mesh_prov_conf(_link[idx].conf_key, _link[idx].rand, _link[idx].auth,
...
int bt_mesh_prov_set_oob_input_data(const u8_t idx, const u8_t *val, bool num_flag)
{
    ...
    if (num_flag) {
        ...
        memcpy(_link[idx].auth + 12, val, sizeof(u32_t));
    } else {
        /* Provisioner inputs string */
        memcpy(_link[idx].auth, val, _link[idx].auth_size);
    }
...
int bt_mesh_prov_set_oob_output_data(const u8_t idx, u8_t *num, u8_t size, bool num_flag)
{
     ...
     */
    if (!_link[idx].auth) {
        BT_ERR("%s, _link auth is NULL", __func__);
        return -EINVAL;
    }
    ...
    if (num_flag) {
        ...
        memset(_link[idx].auth, 0, 16);
        memcpy(_link[idx].auth + 16 - size, num, size);
    } else {
        /* Provisioner output string */
        memset(_link[idx].auth, 0, 16);
        memcpy(_link[idx].auth, num, size);
    }
...
{
    if (!_link[idx].conf_inputs) {
        BT_ERR("%s, Link conf_inputs is NULL", __func__);
        return -EINVAL;
    }

    /* Swap X and Y halves independently to big-endian */
    sys_memcpy_swap(&_link[idx].conf_inputs[81], pub_key_x, 32);
    sys_memcpy_swap(&_link[idx].conf_inputs[81] + 32, pub_key_y, 32);

    bt_mesh_atomic_set_bit(_link[idx].flags, REMOTE_PUB_KEY);

    if (bt_mesh_atomic_test_and_clear_bit(_link[idx].flags, WAIT_GEN_DHKEY)) {
        prov_gen_dh_key(idx);
    }
...
static void prov_dh_key_cb(const u8_t key[32], const u8_t idx)
{
    ...
    _link[idx].dhkey = (u8_t *)osi_calloc(PROV_DH_KEY_SIZE);
    if (!_link[idx].dhkey) {
        BT_ERR("%s, Failed to allocate memory", __func__);
        goto fail;
    }
    sys_memcpy_swap(_link[idx].dhkey, key, 32);

    BT_DBG("DHkey: %s", bt_hex(_link[idx].dhkey, 32));

    bt_mesh_atomic_set_bit(_link[idx].flags, HAVE_DHKEY);
     ...
     */
    if (prov_auth(idx, _link[idx].auth_method,
            _link[idx].auth_action, _link[idx].auth_size) < 0) {
        BT_ERR("%s, Failed to authenticate", __func__);
        goto fail;
    }
    if (_link[idx].auth_method == AUTH_METHOD_OUTPUT ||
        _link[idx].auth_method == AUTH_METHOD_INPUT) {
        return;
    }

    if (_link[idx].expect != PROV_INPUT_COMPLETE) {
        send_confirm(idx);
    }
    return;
...
static void prov_gen_dh_key(const u8_t idx)
{
     ...
     */
    sys_memcpy_swap(&pub_key[0], &_link[idx].conf_inputs[81], 32);
    sys_memcpy_swap(&pub_key[32], &_link[idx].conf_inputs[113], 32);
...
static void send_pub_key(const u8_t idx, u8_t oob)
{
    ...
    memcpy(&_link[idx].conf_inputs[17], &buf.data[1], 64);
    ...
    if (!oob) {
...
#if defined(CONFIG_BLE_MESH_PB_ADV)
        if (idx < CONFIG_BLE_MESH_PBA_SAME_TIME) {
            _link[idx].expect_ack_for = PROV_PUB_KEY;
            return;
        }
#endif /* CONFIG_BLE_MESH_PB_ADV */
         ...
         */
        if (bt_mesh_atomic_test_bit(_link[idx].flags, REMOTE_PUB_KEY)) {
            prov_gen_dh_key(idx);
        } else {
            bt_mesh_atomic_set_bit(_link[idx].flags, WAIT_GEN_DHKEY);
        }
    }
}
...
static void prov_pub_key(const u8_t idx, const u8_t *data)
{
    ...
    memcpy(&_link[idx].conf_inputs[81], data, 64);

    if (!bt_mesh_atomic_test_bit(_link[idx].flags, LOCAL_PUB_KEY)) {
...
static void prov_confirm(const u8_t idx, const u8_t *data)
{
    ...
    _link[idx].conf = (u8_t *)osi_calloc(PROV_CONFIRM_SIZE);
    if (!_link[idx].conf) {
        BT_ERR("%s, Failed to allocate memory", __func__);
        close_link(idx, CLOSE_REASON_FAILED);
        return;
    ...
    memcpy(_link[idx].conf, data, 16);

    if (!bt_mesh_atomic_test_bit(_link[idx].flags, HAVE_DHKEY)) {
#if defined(CONFIG_BLE_MESH_PB_ADV)
        prov_clear_tx(idx);
#endif
        bt_mesh_atomic_set_bit(_link[idx].flags, SEND_CONFIRM);
    }

    prov_buf_init(&buf, PROV_RANDOM);

    net_buf_simple_add_mem(&buf, _link[idx].rand, 16);
...
static void send_prov_data(const u8_t idx)
{
    ...
    err = bt_mesh_session_key(_link[idx].dhkey, _link[idx].prov_salt, session_key);
    if (err) {
    ...
    err = bt_mesh_prov_nonce(_link[idx].dhkey, _link[idx].prov_salt, nonce);
    if (err) {
    ...
    for (j = 0U; j < ARRAY_SIZE(prov_ctx.already_prov); j++) {
        if (!memcmp(_link[idx].uuid, prov_ctx.already_prov[j].uuid, 16)) {
            if (_link[idx].element_num <= prov_ctx.already_prov[j].element_num) {
                already_flag = true;
                sys_put_be16(prov_ctx.already_prov[j].unicast_addr, &pdu[23]);
                _link[idx].unicast_addr = prov_ctx.already_prov[j].unicast_addr;
                break;
            } else {
    ...
    if (!already_flag) {
        ...
        if (prov_ctx.current_addr + _link[idx].element_num - 1 > max_addr) {
            BT_ERR("%s, Not enough unicast address for the device", __func__);
            goto fail;
        }

        sys_put_be16(prov_ctx.current_addr, &pdu[23]);
        _link[idx].unicast_addr = prov_ctx.current_addr;
    }
    ...
    if (!already_flag) {
        for (j = 0U; j < ARRAY_SIZE(prov_ctx.already_prov); j++) {
            if (!prov_ctx.already_prov[j].element_num) {
                memcpy(prov_ctx.already_prov[j].uuid, _link[idx].uuid, 16);
                prov_ctx.already_prov[j].element_num  = _link[idx].element_num;
                prov_ctx.already_prov[j].unicast_addr = _link[idx].unicast_addr;
                break;
            }
        }
         ...
         */
        prov_ctx.current_addr += _link[idx].element_num;
    ...
    if (FAST_PROV_FLAG_GET()) {
        _link[idx].ki_flags = fast_prov_info.flags;
        _link[idx].iv_index = fast_prov_info.iv_index;
    } else {
        _link[idx].ki_flags = prov_ctx.curr_flags;
        _link[idx].iv_index = prov_ctx.curr_iv_index;
    }

    _link[idx].expect = PROV_COMPLETE;
    return;
...
static void prov_random(const u8_t idx, const u8_t *data)
{
    ...
    if (bt_mesh_prov_conf(_link[idx].conf_key, data, _link[idx].auth, conf_verify)) {
        BT_ERR("%s, Failed to calculate confirmation verification", __func__);
        goto fail;
    }

    if (memcmp(conf_verify, _link[idx].conf, 16)) {
        BT_ERR("%s, Invalid confirmation value", __func__);
        BT_DBG("Received:   %s", bt_hex(_link[idx].conf, 16));
     ...
     */
    _link[idx].prov_salt = (u8_t *)osi_calloc(PROV_PROV_SALT_SIZE);
    if (!_link[idx].prov_salt) {
        BT_ERR("%s, Failed to allocate memory", __func__);
        goto fail;
    }

    if (bt_mesh_prov_salt(_link[idx].conf_salt, _link[idx].rand, data,
                          _link[idx].prov_salt)) {
        BT_ERR("%s, Failed to generate ProvisioningSalt", __func__);
        goto fail;
    }

    BT_DBG("ProvisioningSalt: %s", bt_hex(_link[idx].prov_salt, 16));
...
static void prov_complete(const u8_t idx, const u8_t *data)
{
     ...
     */
    err = bt_mesh_dev_key(_link[idx].dhkey, _link[idx].prov_salt, device_key);
    if (err) {
    ...
    for (j = 0U; j < ARRAY_SIZE(prov_nodes); j++) {
        if (!prov_nodes[j].provisioned) {
            ...
            prov_nodes[j].oob_info = _link[idx].oob_info;
            prov_nodes[j].element_num = _link[idx].element_num;
            prov_nodes[j].unicast_addr = _link[idx].unicast_addr;
            if (FAST_PROV_FLAG_GET()) {
                prov_nodes[j].net_idx = fast_prov_info.net_idx;
            } else {
                prov_nodes[j].net_idx = prov_ctx.curr_net_idx;
            }
            prov_nodes[j].flags = _link[idx].ki_flags;
            prov_nodes[j].iv_index = _link[idx].iv_index;
            prov_nodes[j].addr.type = _link[idx].addr.type;
            memcpy(prov_nodes[j].addr.val, _link[idx].addr.val, BLE_MESH_ADDR_LEN);
            memcpy(prov_nodes[j].uuid, _link[idx].uuid, 16);
            break;
        }
    }
    ...
    err = provisioner_dev_find(&_link[idx].addr, _link[idx].uuid, &rm);
    if (!err) {
...
static void close_link(const u8_t idx, u8_t reason)
{
    ...
    if (idx < BLE_MESH_PROV_SAME_TIME
        ...
        if (_link[idx].conn) {
            bt_mesh_gattc_disconnect(_link[idx].conn);
        }
        return;
    }
#endif
...
static void prov_retransmit(struct k_work *work)
{
    ...
    if (!bt_mesh_atomic_test_bit(_link[idx].flags, LINK_ACTIVE)) {
...
#endif
    if (k_uptime_get() - _link[idx].tx.start > timeout) {
        BT_WARN("Provisioner timeout, giving up transaction");
        reset_link(idx, CLOSE_REASON_TIMEOUT);
        return;
    }

    if (_link[idx].send_link_close & BIT(0)) {
        u8_t reason = (_link[idx].send_link_close >> 1) & BIT_MASK(2);
        u16_t count = (_link[idx].send_link_close >> 3);
        if (count >= 2) {
            reset_link(idx, reason);
            return;
        }
        _link[idx].send_link_close += BIT(3);
    }
...
static void link_ack(const u8_t idx, struct prov_rx *rx, struct net_buf_simple *buf)
{
    ...
    if (_link[idx].expect == PROV_CAPABILITIES) {
        BT_WARN("%s, Link ACK is already received", __func__);
        return;
    }

    _link[idx].conf_inputs = (u8_t *)osi_calloc(PROV_CONF_INPUTS_SIZE);
    if (!_link[idx].conf_inputs) {
        BT_ERR("%s, Failed to allocate memory", __func__);
        close_link(idx, CLOSE_REASON_FAILED);
        return;
...
{
    u8_t type = _link[idx].rx.buf->data[0];

    BT_DBG("type 0x%02x len %u", type, _link[idx].rx.buf->len);

    /**
     * Provisioner first checks information within the received
     * Provisioning PDU. If the check succeeds then check fcs.
     */
    if (type != PROV_FAILED && type != _link[idx].expect) {
        BT_ERR("%s, Unexpected msg 0x%02x != 0x%02x", __func__, type, _link[idx].expect);
        goto fail;
    }
    ...
    if (1 + prov_handlers[type].len != _link[idx].rx.buf->len) {
        BT_ERR("%s, Invalid length %u for type 0x%02x", __func__, _link[idx].rx.buf->len, type);
        goto fail;
    }

    if (!bt_mesh_fcs_check(_link[idx].rx.buf, _link[idx].rx.fcs)) {
        BT_ERR("%s, Incorrect FCS", __func__);
        goto fail;
    }

    gen_prov_ack_send(idx, _link[idx].rx.trans_id);
    _link[idx].rx.prev_id = _link[idx].rx.trans_id;
    _link[idx].rx.trans_id = 0;

    prov_handlers[type].func(idx, &_link[idx].rx.buf->data[1]);
    return;
...
static void gen_prov_cont(const u8_t idx, struct prov_rx *rx, struct net_buf_simple *buf)
{
    ...
    if (!_link[idx].rx.seg && _link[idx].rx.prev_id == rx->xact_id) {
        BT_WARN("%s, Resending ack", __func__);
        gen_prov_ack_send(idx, rx->xact_id);
        return;
    }

    if (rx->xact_id != _link[idx].rx.trans_id) {
        BT_WARN("%s, Data for unknown transaction (%u != %u)",
            __func__, rx->xact_id, _link[idx].rx.trans_id);
        /**
    ...
    if (seg > _link[idx].rx.last_seg) {
        BT_ERR("%s, Invalid segment index %u", __func__, seg);
        goto fail;
    } else if (seg == _link[idx].rx.last_seg) {
        u8_t expect_len;

        expect_len = (_link[idx].rx.buf->len - 20 -
                      (23 * (_link[idx].rx.last_seg - 1)));
...
static void gen_prov_ack(const u8_t idx, struct prov_rx *rx, struct net_buf_simple *buf)
{
    ...
    if (!_link[idx].tx.trans_id) {
        return;
    }

    if (rx->xact_id == (_link[idx].tx.trans_id - 1)) {
        prov_clear_tx(idx);

        ack_type = _link[idx].expect_ack_for;
        switch (ack_type) {
        case PROV_START:
            pub_key_oob = _link[idx].conf_inputs[13];
            send_pub_key(idx, pub_key_oob);
            break;
        case PROV_PUB_KEY:
            prov_gen_dh_key(idx);
...
static void gen_prov_start(const u8_t idx, struct prov_rx *rx, struct net_buf_simple *buf)
{
    ...
    if (_link[idx].rx.prev_id == rx->xact_id) {
        BT_WARN("%s, Resending ack", __func__);
        gen_prov_ack_send(idx, rx->xact_id);
        return;
    }

    _link[idx].rx.buf->len = net_buf_simple_pull_be16(buf);
    _link[idx].rx.trans_id = rx->xact_id;
    _link[idx].rx.fcs = net_buf_simple_pull_u8(buf);

    BT_DBG("len %u last_seg %u total_len %u fcs 0x%02x", buf->len,
           START_LAST_SEG(rx->gpc), _link[idx].rx.buf->len, _link[idx].rx.fcs);

    /* Provisioner can not receive zero-length provisioning pdu */
    if (_link[idx].rx.buf->len < 1) {
        BT_ERR("%s, Ignoring zero-length provisioning PDU", __func__);
        close_link(idx, CLOSE_REASON_FAILED);
        return;
    }

    if (_link[idx].rx.buf->len > _link[idx].rx.buf->size) {
        BT_ERR("%s, Too large provisioning PDU (%u bytes)",
            __func__, _link[idx].rx.buf->len);
        // close_link(i, CLOSE_REASON_FAILED);
        return;
    }

    if (START_LAST_SEG(rx->gpc) > 0 && _link[idx].rx.buf->len <= 20) {
        BT_ERR("%s, Too small total length for multi-segment PDU", __func__);
        close_link(idx, CLOSE_REASON_FAILED);
        return;
    }

    _link[idx].rx.seg = (1 << (START_LAST_SEG(rx->gpc) + 1)) - 1;
    _link[idx].rx.last_seg = START_LAST_SEG(rx->gpc);
    memcpy(_link[idx].rx.buf->data, buf->data, buf->len);
    XACT_SEG_RECV(idx, 0);
...
static void gen_prov_recv(const u8_t idx, struct prov_rx *rx, struct net_buf_simple *buf)
{
    ...
    /**
     * require_link can be used combining with _link[].linking flag to
    ...
    if (!bt_mesh_atomic_test_bit(_link[idx].flags, LINK_ACTIVE) &&
        ...
        BT_DBG("Ignoring message that requires active _link");
        return;
    }
...
static int find_link(u32_t link_id, u8_t *idx)
{
    ...
    /* _link for PB-ADV is from 0 to CONFIG_BLE_MESH_PBA_SAME_TIME */
    for (i = 0U; i < CONFIG_BLE_MESH_PBA_SAME_TIME; i++) {
        if (bt_mesh_atomic_test_bit(_link[i].flags, LINK_ACTIVE)) {
            if (_link[i].link_id == link_id) {
                if (idx) {
                    *idx = i;
                }
                return 0;
...
void provisioner_pb_adv_recv(struct net_buf_simple *buf)
{
    ...
    if (find_link(rx.link_id, &idx) < 0) {
        BT_DBG("%s, Data for unexpected _link", __func__);
        return;
    }
...
static struct bt_mesh_conn *find_conn(struct bt_mesh_conn *conn, u8_t *idx)
{
    ...
    /* _link for PB-GATT is from CONFIG_BLE_MESH_PBA_SAME_TIME to BLE_MESH_PROV_SAME_TIME */
    for (i = CONFIG_BLE_MESH_PBA_SAME_TIME; i < BLE_MESH_PROV_SAME_TIME; i++) {
        if (bt_mesh_atomic_test_bit(_link[i].flags, LINK_ACTIVE)) {
            if (_link[i].conn == conn) {
                if (idx) {
                    *idx = i;
                }
                return conn;
...
int provisioner_set_prov_conn(const u8_t addr[6], struct bt_mesh_conn *conn)
{
    ...
    for (i = CONFIG_BLE_MESH_PBA_SAME_TIME; i < BLE_MESH_PROV_SAME_TIME; i++) {
        if (!memcmp(_link[i].addr.val, addr, BLE_MESH_ADDR_LEN)) {
            _link[i].conn = bt_mesh_conn_ref(conn);
            return 0;
        }
    }
...
int provisioner_pb_gatt_open(struct bt_mesh_conn *conn, u8_t *addr)
{
    ...
    for (i = 0U; i < CONFIG_BLE_MESH_PBA_SAME_TIME; i++) {
        if (bt_mesh_atomic_test_bit(_link[i].flags, LINK_ACTIVE)) {
            if (!memcmp(_link[i].uuid, _link[idx].uuid, 16)) {
    ...
    bt_mesh_atomic_set_bit(_link[idx].flags, LINK_ACTIVE);
    _link[idx].conn = bt_mesh_conn_ref(conn);
...
int provisioner_pb_gatt_close(struct bt_mesh_conn *conn, u8_t reason)
{
    ...
    memset(&_link[idx], 0, offsetof(struct prov_link, timeout));
...
int provisioner_prov_init(const struct bt_mesh_prov *prov_info)
{
    ...
    for (i = 0U; i < CONFIG_BLE_MESH_PBA_SAME_TIME; i++) {
        ...
        _link[i].pending_ack = XACT_NVAL;
        k_delayed_work_init(&_link[i].tx.retransmit, prov_retransmit);
        _link[i].tx.retransmit.work.index = (int)i;
        _link[i].rx.prev_id = XACT_NVAL;
        _link[i].rx.buf = bt_mesh_pba_get_buf(i);
    }
#endif

    for (i = 0U; i < BLE_MESH_PROV_SAME_TIME; i++) {
        k_delayed_work_init(&_link[i].timeout, prov_timeout);
        _link[i].timeout.work.index = (int)i;
    }
...
u16_t provisioner_srv_uuid_recv(struct net_buf_simple *buf)
{
    ...
    // BT_ERR("Received adv pkt with service UUID: %d", uuid);
...
void provisioner_srv_data_recv(struct net_buf_simple *buf, const bt_mesh_addr_t *addr, u16_t uuid)
{
    ...
    case BLE_MESH_UUID_MESH_PROXY_VAL:
        ...
        // BT_ERR("Start to deal with Mesh Proxy Service Data");
...
static void provisioner_prov_srv_data_recv(struct net_buf_simple *buf, const bt_mesh_addr_t *addr)
{
    ...
    /* Provisioner will copy the device uuid, oob info, etc. into an unused _link
     * struct, and at this moment the _link has not been activated. Even if we
     ...
     * adv pkt into two _link structs which will has no impact on the provisioning
     * of this device, because no matter which _link among PB-GATT and PB-ADV is
     * activated first, the other one will be dropped finally and the _link struct
     * occupied by the dropped _link will be used by other devices (because the _link
     * is not activated).
