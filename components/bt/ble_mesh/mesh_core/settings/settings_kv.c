...
void bt_mesh_settings_foreach(void)
{
    ...
    for (i = 0; i < ARRAY_SIZE(settings_ctx); i++) {
        ...
        // err = nvs_open(ctx->name, NVS_READWRITE, &ctx->handle);
        // if (err != BT_OK) {
        //     BT_ERR("%s, Open nvs failed, name %s, err %d", __func__, ctx->name, err);
        //     continue;
        // }

        if (ctx->settings_init && ctx->settings_init()) {
            BT_ERR("%s, Init settings failed, name %s", __func__, ctx->name);
            continue;
        }

        if (ctx->settings_load && ctx->settings_load()) {
            BT_ERR("%s, Load settings failed, name %s", __func__, ctx->name);
            continue;
        }

        if (ctx->settings_commit && ctx->settings_commit()) {
            BT_ERR("%s, Commit settings failed, name %s", __func__, ctx->name);
            continue;
        }
    }
}
...
static int settings_save(const char *key, const u8_t *val, size_t len)
{
    ...
    if (val) {
        err = aos_kv_set(key, (void*)val, len, 1);
    } else {
        err = aos_kv_del(key);
    }
                         ...
                         u8_t *buf, size_t buf_len, bool *exist)
{
    ...
    err = aos_kv_get(key, buf, (int*)&buf_len);
    if (err != BT_OK) {
        if (err == BT_ERR_NOT_FOUND) {
            BT_ERR("%s, Settings %s is not found", __func__, key);
            *exist = false;
            return 0;
        }
...
static int settings_add_item(const char *key, const u16_t val)
{
    ...
    if (buf == NULL) {
        BT_ERR("%s, settings_get_item", __func__);
        return -ENOMEM;
    }
