...
static aos_queue_t xBleMeshQueue;
    ...
    while (1) {
        *buf = NULL;
        ...
        if (IS_ENABLED(CONFIG_BLE_MESH_PROXY)) {
            aos_queue_recv(&xBleMeshQueue, AOS_WAIT_FOREVER, &msg, &size);
            while (!(*buf)) {
                s32_t timeout;
                BT_DBG("Proxy advertising start");
                timeout = bt_mesh_proxy_adv_start();
                BT_DBG("Proxy Advertising up to %d ms", timeout);
                aos_queue_recv(&xBleMeshQueue, timeout, &msg, &size);
        ...
        if (BLE_MESH_ADV(*buf)->busy) {
            ...
            status = adv_send(*buf);
            if (status) {
                BT_ERR("%s, adv_send failed", __func__);
                if (aos_queue_send(&xBleMeshQueue, &msg, sizeof(bt_mesh_msg_t)) != 0) {
                      ...
                      void *cb_data)
{
    ...
    bt_mesh_task_post(&msg, AOS_WAIT_FOREVER);
}
                            ...
                            u8_t adv_type, struct net_buf_simple *buf)
{
    ...
    BT_DBG("%s, rssi %d, len %u: %s", __func__, rssi, buf->len, bt_hex(buf->data, buf->len));
    ...
    // if (type != 0xFF)
        // BT_DBG("%s, type: %02x, len: %d", __func__, type, len);
...
void bt_mesh_adv_init(void)
{
    ...
    static uint8_t buffer[QUEUE_LEN * sizeof(bt_mesh_msg_t)];
    aos_queue_new(&xBleMeshQueue, buffer, QUEUE_LEN * sizeof(bt_mesh_msg_t), sizeof(bt_mesh_msg_t));
    aos_task_new("BLE_Mesh_ADV_Task", adv_thread, NULL, 3072);
}
