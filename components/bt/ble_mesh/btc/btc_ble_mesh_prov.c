...
#include "yoc_ble_mesh_common_api.h"
#include "yoc_ble_mesh_provisioning_api.h"
...
void btc_ble_mesh_prov_arg_deep_copy(btc_msg_t *msg, void *p_dest, void *p_src)
{
    ...
    case BTC_BLE_MESH_ACT_CLIENT_MODEL_SEND: {
        BT_LOG_DEBUG("%s, BTC_BLE_MESH_ACT_MODEL_SEND, src->model_send.length = %d", __func__, src->model_send.length);
        dst->model_send.data = src->model_send.length ? (uint8_t *)osi_malloc(src->model_send.length) : NULL;
        dst->model_send.ctx = osi_malloc(sizeof(ble_mesh_msg_ctx_t));
        ...
        if (dst->model_send.ctx) {
            memcpy(dst->model_send.ctx, src->model_send.ctx, sizeof(ble_mesh_msg_ctx_t));
        } else {
            BT_LOG_ERROR("%s, Failed to allocate memory, act %d", __func__, msg->act);
        }
        break;
...
{
    ble_mesh_model_cb_param_t *p_dest_data = (ble_mesh_model_cb_param_t *)p_dest;
    ble_mesh_model_cb_param_t *p_src_data = (ble_mesh_model_cb_param_t *)p_src;
    ...
    switch (msg->act) {
    case BLE_MESH_BLE_MESH_MODEL_OPERATION_EVT: {
        if (p_src_data->model_operation.ctx && p_src_data->model_operation.msg) {
            p_dest_data->model_operation.ctx = osi_malloc(sizeof(ble_mesh_msg_ctx_t));
            ...
            if (p_dest_data->model_operation.ctx) {
                memcpy(p_dest_data->model_operation.ctx, p_src_data->model_operation.ctx, sizeof(ble_mesh_msg_ctx_t));
            } else {
                BT_LOG_ERROR("%s, Failed to allocate memory, act %d", __func__, msg->act);
            }
    ...
    case BLE_MESH_BLE_MESH_CLIENT_MODEL_RECV_PUBLISH_MSG_EVT: {
        if (p_src_data->client_recv_publish_msg.ctx && p_src_data->client_recv_publish_msg.msg) {
            p_dest_data->client_recv_publish_msg.ctx = osi_malloc(sizeof(ble_mesh_msg_ctx_t));
            ...
            if (p_dest_data->client_recv_publish_msg.ctx) {
                memcpy(p_dest_data->client_recv_publish_msg.ctx, p_src_data->client_recv_publish_msg.ctx, sizeof(ble_mesh_msg_ctx_t));
            } else {
                BT_LOG_ERROR("%s, Failed to allocate memory, act %d", __func__, msg->act);
            }
    ...
    case BLE_MESH_BLE_MESH_MODEL_SEND_COMP_EVT: {
        if (p_src_data->model_send_comp.ctx) {
            p_dest_data->model_send_comp.ctx = osi_malloc(sizeof(ble_mesh_msg_ctx_t));
            if (p_dest_data->model_send_comp.ctx) {
                memcpy(p_dest_data->model_send_comp.ctx, p_src_data->model_send_comp.ctx, sizeof(ble_mesh_msg_ctx_t));
            } else {
                BT_LOG_ERROR("%s, Failed to allocate memory, act %d", __func__, msg->act);
            }
        }
    ...
    case BLE_MESH_BLE_MESH_CLIENT_MODEL_SEND_TIMEOUT_EVT: {
        if (p_src_data->client_send_timeout.ctx) {
            p_dest_data->client_send_timeout.ctx = osi_malloc(sizeof(ble_mesh_msg_ctx_t));
            if (p_dest_data->client_send_timeout.ctx) {
                memcpy(p_dest_data->client_send_timeout.ctx, p_src_data->client_send_timeout.ctx, sizeof(ble_mesh_msg_ctx_t));
            } else {
                BT_LOG_ERROR("%s, Failed to allocate memory, act %d", __func__, msg->act);
            }
        }
...
{
    ble_mesh_model_cb_param_t *arg = NULL;
    ...
    arg = (ble_mesh_model_cb_param_t *)(msg->arg);
...
static inline void btc_ble_mesh_cb_to_app(ble_mesh_prov_cb_event_t event,
        ble_mesh_prov_cb_param_t *param)
{
    ble_mesh_prov_cb_t btc_mesh_cb = (ble_mesh_prov_cb_t)btc_profile_cb_get(BTC_PID_PROV);
    if (btc_mesh_cb) {
        btc_mesh_cb(event, param);
    }
}

static inline void btc_ble_mesh_model_cb_to_app(ble_mesh_model_cb_event_t event,
        ble_mesh_model_cb_param_t *param)
{
    ble_mesh_model_cb_t btc_mesh_cb = (ble_mesh_model_cb_t)btc_profile_cb_get(BTC_PID_MODEL);
    if (btc_mesh_cb) {
        btc_mesh_cb(event, param);
    }
}
...
{
    ble_mesh_model_cb_param_t mesh_param = {0};
    btc_msg_t msg = {0};
    bt_status_t ret;

    mesh_param.model_operation.opcode = mesh_opcode;
    mesh_param.model_operation.model = (ble_mesh_model_t *)model;
    mesh_param.model_operation.ctx = (ble_mesh_msg_ctx_t *)ctx;
    mesh_param.model_operation.length = buf->len;
    mesh_param.model_operation.msg = buf->data;

    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_MODEL;
    msg.act = BLE_MESH_BLE_MESH_MODEL_OPERATION_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_model_cb_param_t), btc_ble_mesh_copy_req_data);

    if (ret != BT_STATUS_SUCCESS) {
        BT_LOG_ERROR("%s btc_transfer_context failed", __func__);
    }
    return;
}
        ...
        struct net_buf_simple *buf)
{
    ...
    if (!data) {
        BT_LOG_ERROR("%s, Client internal_data is NULL", __func__);
        return;
    }

    node = bt_mesh_is_model_message_publish(model, ctx, buf, false);
    if (node == NULL) {
        msg.act = BLE_MESH_BLE_MESH_CLIENT_MODEL_RECV_PUBLISH_MSG_EVT;
        mesh_param.client_recv_publish_msg.opcode = mesh_opcode;
        mesh_param.client_recv_publish_msg.model = (ble_mesh_model_t *)model;
        mesh_param.client_recv_publish_msg.ctx = (ble_mesh_msg_ctx_t *)ctx;
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_MODEL;
    if (msg.act != BLE_MESH_BLE_MESH_CLIENT_MODEL_RECV_PUBLISH_MSG_EVT) {
...
static void btc_ble_mesh_model_send_comp_cb(ble_mesh_model_t *model, ble_mesh_msg_ctx_t *ctx, u32_t opcode, int err)
{
    ble_mesh_model_cb_param_t mesh_param = {0};
    btc_msg_t msg = {0};
    bt_status_t ret;
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_MODEL;
    msg.act = BLE_MESH_BLE_MESH_MODEL_SEND_COMP_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_model_cb_param_t), btc_ble_mesh_copy_req_data);
...
static void btc_ble_mesh_model_publish_comp_cb(ble_mesh_model_t *model, int err)
{
    ble_mesh_model_cb_param_t mesh_param = {0};
    btc_msg_t msg = {0};
    bt_status_t ret;
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_MODEL;
    msg.act = BLE_MESH_BLE_MESH_MODEL_PUBLISH_COMP_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_model_cb_param_t), NULL);
...
static void btc_oob_pub_key_cb(void)
{
    ...
    msg.act = BLE_MESH_BLE_MESH_NODE_PROV_OOB_PUB_KEY_EVT;
...
{
    ble_mesh_prov_cb_param_t mesh_param = {0};
    btc_msg_t msg = {0};
    bt_status_t ret;

    BT_LOG_DEBUG("%s", __func__);

    mesh_param.node_prov_output_num.action = (ble_mesh_output_action_t)act;
    mesh_param.node_prov_output_num.number = num;

    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_PROV;
    msg.act = BLE_MESH_BLE_MESH_NODE_PROV_OUTPUT_NUMBER_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_prov_cb_param_t), NULL);
...
static int btc_output_string_cb(const char *str)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_PROV;
    msg.act = BLE_MESH_BLE_MESH_NODE_PROV_OUTPUT_STRING_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_prov_cb_param_t), NULL);
...
static int btc_input_cb(bt_mesh_input_action_t act, u8_t size)
{
    ...
    mesh_param.node_prov_input.action = (ble_mesh_input_action_t)act;
    mesh_param.node_prov_input.size = size;

    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_PROV;
    msg.act = BLE_MESH_BLE_MESH_NODE_PROV_INPUT_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_prov_cb_param_t), NULL);
...
static void btc_link_open_cb(bt_mesh_prov_bearer_t bearer)
{
    ...
    mesh_param.node_prov_link_open.bearer = (ble_mesh_prov_bearer_t)bearer;

    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_PROV;
    msg.act = BLE_MESH_BLE_MESH_NODE_PROV_LINK_OPEN_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_prov_cb_param_t), NULL);
...
static void btc_link_close_cb(bt_mesh_prov_bearer_t bearer)
{
    ...
    mesh_param.node_prov_link_close.bearer = (ble_mesh_prov_bearer_t)bearer;

    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_PROV;
    msg.act = BLE_MESH_BLE_MESH_NODE_PROV_LINK_CLOSE_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_prov_cb_param_t), NULL);
...
static void btc_complete_cb(u16_t net_idx, u16_t addr, u8_t flags, u32_t iv_index)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_PROV;
    msg.act = BLE_MESH_BLE_MESH_NODE_PROV_COMPLETE_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_prov_cb_param_t), NULL);
...
static void btc_reset_cb(void)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_PROV;
    msg.act = BLE_MESH_BLE_MESH_NODE_PROV_RESET_EVT;
...
static void btc_prov_register_complete_cb(int err_code)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_PROV;
    msg.act = BLE_MESH_BLE_MESH_PROV_REGISTER_COMP_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_prov_cb_param_t), NULL);
...
static void btc_client_model_timeout_cb(struct k_work *work)
{
    ...
    mesh_param.client_send_timeout.model = (ble_mesh_model_t *)node->ctx.model;
    mesh_param.client_send_timeout.ctx = (ble_mesh_msg_ctx_t *)&node->ctx;

    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_MODEL;
    msg.act = BLE_MESH_BLE_MESH_CLIENT_MODEL_SEND_TIMEOUT_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_model_cb_param_t), btc_ble_mesh_copy_req_data);
...
static int btc_model_publish_update(struct bt_mesh_model *mod)
{
    ...
    mesh_param.model_publish_update.model = (ble_mesh_model_t *)mod;

    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_MODEL;
    msg.act = BLE_MESH_BLE_MESH_MODEL_PUBLISH_UPDATE_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_model_cb_param_t), NULL);
...
static void btc_prov_set_complete_cb(ble_mesh_prov_cb_param_t *param, uint8_t act)
{
    btc_msg_t msg = {0};
    bt_status_t ret;
                ...
                u16_t oob_info, bt_mesh_prov_bearer_t bearer)
{
    ...
    memcpy(mesh_param.provisioner_recv_unprov_adv_pkt.addr, addr, BT_BD_ADDR_LEN);
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_PROV;
    msg.act = BLE_MESH_BLE_MESH_PROVISIONER_RECV_UNPROV_ADV_PKT_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_prov_cb_param_t), NULL);
...
static int btc_provisioner_prov_read_oob_pub_key_cb(u8_t link_idx)
{
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_PROV;
    msg.act = BLE_MESH_BLE_MESH_PROVISIONER_PROV_READ_OOB_PUB_KEY_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_prov_cb_param_t), NULL);
...
static int btc_provisioner_prov_input_cb(u8_t method, bt_mesh_output_action_t act, u8_t size, u8_t link_idx)
{
    ...
    mesh_param.provisioner_prov_input.method = (ble_mesh_oob_method_t)method;
    mesh_param.provisioner_prov_input.action = (ble_mesh_output_action_t)act;
    mesh_param.provisioner_prov_input.size = size;
    mesh_param.provisioner_prov_input.link_idx = link_idx;

    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_PROV;
    msg.act = BLE_MESH_BLE_MESH_PROVISIONER_PROV_INPUT_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_prov_cb_param_t), NULL);
...
static int btc_provisioner_prov_output_cb(u8_t method, bt_mesh_input_action_t act, void *data, u8_t size, u8_t link_idx)
{
    ...
    mesh_param.provisioner_prov_output.method = (ble_mesh_oob_method_t)method;
    mesh_param.provisioner_prov_output.action = (ble_mesh_input_action_t)act;
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_PROV;
    msg.act = BLE_MESH_BLE_MESH_PROVISIONER_PROV_OUTPUT_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_prov_cb_param_t), NULL);
...
static void btc_provisioner_link_open_cb(bt_mesh_prov_bearer_t bearer)
{
    ...
    mesh_param.provisioner_prov_link_open.bearer = (ble_mesh_prov_bearer_t)bearer;

    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_PROV;
    msg.act = BLE_MESH_BLE_MESH_PROVISIONER_PROV_LINK_OPEN_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_prov_cb_param_t), NULL);
...
static void btc_provisioner_link_close_cb(bt_mesh_prov_bearer_t bearer, u8_t reason)
{
    ...
    mesh_param.provisioner_prov_link_close.bearer = (ble_mesh_prov_bearer_t)bearer;
    mesh_param.provisioner_prov_link_close.reason = reason;

    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_PROV;
    msg.act = BLE_MESH_BLE_MESH_PROVISIONER_PROV_LINK_CLOSE_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_prov_cb_param_t), NULL);
        ...
        u16_t netkey_idx)
{
    ...
    memcpy(mesh_param.provisioner_prov_complete.device_uuid, device_uuid, sizeof(ble_mesh_octet16_t));

    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_PROV;
    msg.act = BLE_MESH_BLE_MESH_PROVISIONER_PROV_COMPLETE_EVT;
    ret = btc_transfer_context(&msg, &mesh_param,
                               sizeof(ble_mesh_prov_cb_param_t), NULL);
...
int btc_ble_mesh_client_init(ble_mesh_model_t *model)
{
    __ASSERT(model && model->op, "%s, Invalid parameter", __func__);
    ble_mesh_model_op_t *op = model->op;
    while (op != NULL && op->opcode != 0) {
        op->param_cb = (ble_mesh_cb_t)btc_ble_mesh_client_model_op_cb;
        op++;
    }
    return bt_mesh_client_init((struct bt_mesh_model *)model);
}

int32_t btc_ble_mesh_model_pub_period_get(ble_mesh_model_t *mod)
{
...
uint16_t *btc_ble_mesh_model_find_group(ble_mesh_model_t *mod, uint16_t addr)
{
    return bt_mesh_model_find_group((struct bt_mesh_model *)mod, addr);
}

ble_mesh_elem_t *btc_ble_mesh_elem_find(u16_t addr)
{
    return (ble_mesh_elem_t *)bt_mesh_elem_find(addr);
}
...
ble_mesh_model_t *btc_ble_mesh_model_find_vnd(const ble_mesh_elem_t *elem,
        uint16_t company, uint16_t id)
{
    return (ble_mesh_model_t *)bt_mesh_model_find_vnd((struct bt_mesh_elem *)elem, company, id);
}

ble_mesh_model_t *btc_ble_mesh_model_find(const ble_mesh_elem_t *elem, uint16_t id)
{
    return (ble_mesh_model_t *)bt_mesh_model_find((struct bt_mesh_elem *)elem, id);
}

const ble_mesh_comp_t *btc_ble_mesh_comp_get(void)
{
    return (const ble_mesh_comp_t *)bt_mesh_comp_get();
}
...
static void btc_mesh_model_op_add(ble_mesh_model_t *model)
{
    ble_mesh_model_op_t *op = NULL;
    ...
    case BLE_MESH_MODEL_ID_CFG_SRV: {
        model->op = (ble_mesh_model_op_t *)bt_mesh_cfg_srv_op;
        break;
    }
    case BLE_MESH_MODEL_ID_CFG_CLI: {
        model->op = (ble_mesh_model_op_t *)bt_mesh_cfg_cli_op;
    ...
    case BLE_MESH_MODEL_ID_HEALTH_SRV: {
        model->op = (ble_mesh_model_op_t *)bt_mesh_health_srv_op;
        break;
    }
    case BLE_MESH_MODEL_ID_HEALTH_CLI: {
        model->op = (ble_mesh_model_op_t *)bt_mesh_health_cli_op;
    ...
    case BLE_MESH_MODEL_ID_GEN_ONOFF_CLI: {
        model->op = ((ble_mesh_model_op_t *)gen_onoff_cli_op);
    ...
    case BLE_MESH_MODEL_ID_GEN_LEVEL_CLI: {
        model->op = ((ble_mesh_model_op_t *)gen_level_cli_op);
    ...
    case BLE_MESH_MODEL_ID_GEN_DEF_TRANS_TIME_CLI: {
        model->op = ((ble_mesh_model_op_t *)gen_def_trans_time_cli_op);
    ...
    case BLE_MESH_MODEL_ID_GEN_POWER_ONOFF_CLI: {
        model->op = ((ble_mesh_model_op_t *)gen_power_onoff_cli_op);
    ...
    case BLE_MESH_MODEL_ID_GEN_POWER_LEVEL_CLI: {
        model->op = ((ble_mesh_model_op_t *)gen_power_level_cli_op);
    ...
    case BLE_MESH_MODEL_ID_GEN_BATTERY_CLI: {
        model->op = ((ble_mesh_model_op_t *)gen_battery_cli_op);
    ...
    case BLE_MESH_MODEL_ID_GEN_LOCATION_CLI: {
        model->op = ((ble_mesh_model_op_t *)gen_location_cli_op);
    ...
    case BLE_MESH_MODEL_ID_GEN_PROP_CLI: {
        model->op = ((ble_mesh_model_op_t *)gen_property_cli_op);
    ...
    case BLE_MESH_MODEL_ID_LIGHT_LIGHTNESS_CLI: {
        model->op = ((ble_mesh_model_op_t *)light_lightness_cli_op);
    ...
    case BLE_MESH_MODEL_ID_LIGHT_CTL_CLI: {
        model->op = ((ble_mesh_model_op_t *)light_ctl_cli_op);
    ...
    case BLE_MESH_MODEL_ID_LIGHT_HSL_CLI: {
        model->op = ((ble_mesh_model_op_t *)light_hsl_cli_op);
    ...
    case BLE_MESH_MODEL_ID_LIGHT_XYL_CLI: {
        model->op = ((ble_mesh_model_op_t *)light_xyl_cli_op);
    ...
    case BLE_MESH_MODEL_ID_LIGHT_LC_CLI: {
        model->op = ((ble_mesh_model_op_t *)light_lc_cli_op);
    ...
    case BLE_MESH_MODEL_ID_SENSOR_CLI: {
        model->op = ((ble_mesh_model_op_t *)sensor_cli_op);
    ...
    case BLE_MESH_MODEL_ID_TIME_CLI: {
        model->op = ((ble_mesh_model_op_t *)time_cli_op);
    ...
    case BLE_MESH_MODEL_ID_SCENE_CLI: {
        model->op = ((ble_mesh_model_op_t *)scene_cli_op);
        bt_mesh_time_scene_client_t *cli = (bt_mesh_time_scene_client_t *)model->user_data;
        if (cli != NULL) {
            cli->publish_status = btc_mesh_time_scene_client_publish_callback;
        }
    ...
    case BLE_MESH_MODEL_ID_SCHEDULER_CLI: {
        model->op = ((ble_mesh_model_op_t *)scheduler_cli_op);
        bt_mesh_time_scene_client_t *cli = (bt_mesh_time_scene_client_t *)model->user_data;
        if (cli != NULL) {
            cli->publish_status = btc_mesh_time_scene_client_publish_callback;
        }
...
add_model_op:
    if (model->pub) {
        model->pub->update = (ble_mesh_cb_t)btc_model_publish_update;
    }
    op = model->op;
    while (op != NULL && op->opcode != 0) {
        op->param_cb = (ble_mesh_cb_t)btc_ble_mesh_server_model_op_cb;
        op++;
    }
    return;
}
...
{
    ble_mesh_prov_cb_param_t param = {0};
    ...
    case BTC_BLE_MESH_ACT_MESH_INIT: {
        ...
        for (int i = 0; i < arg->mesh_init.comp->element_count; i++) {
            ble_mesh_elem_t *elem = &arg->mesh_init.comp->elements[i];
            /* For SIG models */
            for (int j = 0; j < elem->sig_model_count; j++) {
                ble_mesh_model_t *sig_model = &elem->sig_models[j];
            ...
            for (int k = 0; k < elem->vnd_model_count; k++) {
                ble_mesh_model_t *vnd_model = &elem->vnd_models[k];
...
#if CONFIG_BLE_MESH_NODE
        arg->mesh_init.prov->oob_pub_key_cb = (ble_mesh_cb_t)btc_oob_pub_key_cb;
        arg->mesh_init.prov->output_num_cb = (ble_mesh_cb_t)btc_output_number_cb;
        arg->mesh_init.prov->output_str_cb = (ble_mesh_cb_t)btc_output_string_cb;
        arg->mesh_init.prov->input_cb = (ble_mesh_cb_t)btc_input_cb;
        arg->mesh_init.prov->link_open_cb = (ble_mesh_cb_t)btc_link_open_cb;
        arg->mesh_init.prov->link_close_cb = (ble_mesh_cb_t)btc_link_close_cb;
        arg->mesh_init.prov->complete_cb = (ble_mesh_cb_t)btc_complete_cb;
        arg->mesh_init.prov->reset_cb = (ble_mesh_cb_t)btc_reset_cb;
#endif /* CONFIG_BLE_MESH_NODE */
#if (CONFIG_BLE_MESH_PROVISIONER)
        arg->mesh_init.prov->provisioner_prov_read_oob_pub_key = (ble_mesh_cb_t)btc_provisioner_prov_read_oob_pub_key_cb;
        arg->mesh_init.prov->provisioner_prov_input = (ble_mesh_cb_t)btc_provisioner_prov_input_cb;
        arg->mesh_init.prov->provisioner_prov_output = (ble_mesh_cb_t)btc_provisioner_prov_output_cb;
        arg->mesh_init.prov->provisioner_link_open = (ble_mesh_cb_t)btc_provisioner_link_open_cb;
        arg->mesh_init.prov->provisioner_link_close = (ble_mesh_cb_t)btc_provisioner_link_close_cb;
        arg->mesh_init.prov->provisioner_prov_comp = (ble_mesh_cb_t)btc_provisioner_prov_complete_cb;
        ...
        aos_sem_signal(arg->mesh_init.semaphore);
        btc_prov_register_complete_cb(err_code);
        return;
    }
#if CONFIG_BLE_MESH_NODE
    case BTC_BLE_MESH_ACT_PROV_ENABLE:
        BT_LOG_DEBUG("%s, BTC_BLE_MESH_ACT_PROV_ENABLE, bearers = %d", __func__, arg->node_prov_enable.bearers);
        act = BLE_MESH_BLE_MESH_NODE_PROV_ENABLE_COMP_EVT;
        param.node_prov_enable_comp.err_code = bt_mesh_prov_enable(arg->node_prov_enable.bearers);
        break;
    case BTC_BLE_MESH_ACT_PROV_DISABLE:
        BT_LOG_DEBUG("%s, BTC_BLE_MESH_ACT_PROV_DISABLE, bearers = %d", __func__, arg->node_prov_disable.bearers);
        act = BLE_MESH_BLE_MESH_NODE_PROV_DISABLE_COMP_EVT;
        param.node_prov_disable_comp.err_code = bt_mesh_prov_disable(arg->node_prov_disable.bearers);
        break;
    case BTC_BLE_MESH_ACT_NODE_RESET:
        BT_LOG_DEBUG("%s, BTC_BLE_MESH_ACT_NODE_RESET", __func__);
        bt_mesh_reset();
        return;
    case BTC_BLE_MESH_ACT_SET_OOB_PUB_KEY:
        act = BLE_MESH_BLE_MESH_NODE_PROV_SET_OOB_PUB_KEY_COMP_EVT;
    ...
    case BTC_BLE_MESH_ACT_SET_DEVICE_NAME:
        act = BLE_MESH_BLE_MESH_NODE_SET_UNPROV_DEV_NAME_COMP_EVT;
    ...
    case BTC_BLE_MESH_ACT_PROVISIONER_READ_OOB_PUB_KEY:
        act = BLE_MESH_BLE_MESH_PROVISIONER_PROV_READ_OOB_PUB_KEY_COMP_EVT;
    ...
    case BTC_BLE_MESH_ACT_PROVISIONER_INPUT_STR:
        act = BLE_MESH_BLE_MESH_PROVISIONER_PROV_INPUT_STRING_COMP_EVT;
    ...
    case BTC_BLE_MESH_ACT_PROVISIONER_INPUT_NUM:
        act = BLE_MESH_BLE_MESH_PROVISIONER_PROV_INPUT_NUMBER_COMP_EVT;
    ...
    case BTC_BLE_MESH_ACT_PROVISIONER_ENABLE:
        act = BLE_MESH_BLE_MESH_PROVISIONER_PROV_ENABLE_COMP_EVT;
    ...
    case BTC_BLE_MESH_ACT_PROVISIONER_DISABLE:
        act = BLE_MESH_BLE_MESH_PROVISIONER_PROV_DISABLE_COMP_EVT;
    ...
    case BTC_BLE_MESH_ACT_PROVISIONER_DEV_ADD: {
        ...
        act = BLE_MESH_BLE_MESH_PROVISIONER_ADD_UNPROV_DEV_COMP_EVT;
    ...
    case BTC_BLE_MESH_ACT_PROVISIONER_DEV_DEL: {
        ...
        act = BLE_MESH_BLE_MESH_PROVISIONER_DELETE_DEV_COMP_EVT;
    ...
    case BTC_BLE_MESH_ACT_PROVISIONER_SET_DEV_UUID_MATCH:
        act = BLE_MESH_BLE_MESH_PROVISIONER_SET_DEV_UUID_MATCH_COMP_EVT;
    ...
    case BTC_BLE_MESH_ACT_PROVISIONER_SET_PROV_DATA_INFO: {
        ...
        act = BLE_MESH_BLE_MESH_PROVISIONER_SET_PROV_DATA_INFO_COMP_EVT;
    ...
    case BTC_BLE_MESH_ACT_PROVISIONER_SET_NODE_NAME:
        act = BLE_MESH_BLE_MESH_PROVISIONER_SET_NODE_NAME_COMP_EVT;
        ...
        if (param.provisioner_set_node_name_comp.err_code) {
            param.provisioner_set_node_name_comp.node_index = BLE_MESH_BLE_MESH_INVALID_NODE_INDEX;
        } else {
    ...
    case BTC_BLE_MESH_ACT_PROVISIONER_SET_LOCAL_APP_KEY: {
        ...
        act = BLE_MESH_BLE_MESH_PROVISIONER_ADD_LOCAL_APP_KEY_COMP_EVT;
        ...
        if (param.provisioner_add_app_key_comp.err_code) {
            param.provisioner_add_app_key_comp.app_idx = BLE_MESH_BLE_MESH_KEY_UNUSED;
        } else {
    ...
    case BTC_BLE_MESH_ACT_PROVISIONER_BIND_LOCAL_MOD_APP:
        act = BLE_MESH_BLE_MESH_PROVISIONER_BIND_APP_KEY_TO_MODEL_COMP_EVT;
    ...
    case BTC_BLE_MESH_ACT_PROVISIONER_ADD_LOCAL_NET_KEY: {
        ...
        act = BLE_MESH_BLE_MESH_PROVISIONER_ADD_LOCAL_NET_KEY_COMP_EVT;
        ...
        if (param.provisioner_add_net_key_comp.err_code) {
            param.provisioner_add_net_key_comp.net_idx = BLE_MESH_BLE_MESH_KEY_UNUSED;
        } else {
    ...
    case BTC_BLE_MESH_ACT_SET_FAST_PROV_INFO:
        act = BLE_MESH_BLE_MESH_SET_FAST_PROV_INFO_COMP_EVT;
...
void btc_mesh_model_call_handler(btc_msg_t *msg)
{
    ...
    case BTC_BLE_MESH_ACT_SERVER_MODEL_SEND: {
        ...
        if (!buf) {
            BT_LOG_ERROR("%s, Failed to allocate memory", __func__);
            return;
        }
    ...
    default:
        BT_LOG_WARN("%s, Unknown msg->act %d", __func__, msg->act);
        break;
    }
...
{
    ble_mesh_model_cb_param_t *param = NULL;
    ...
    param = (ble_mesh_model_cb_param_t *)(msg->arg);

    if (msg->act < BLE_MESH_BLE_MESH_MODEL_EVT_MAX) {
...
{
    ble_mesh_prov_cb_param_t *param = NULL;
    ...
    param = (ble_mesh_prov_cb_param_t *)(msg->arg);

    if (msg->act < BLE_MESH_BLE_MESH_PROV_EVT_MAX) {
