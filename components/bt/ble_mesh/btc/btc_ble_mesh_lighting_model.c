...
static inline void btc_ble_mesh_cb_to_app(ble_mesh_light_client_cb_event_t event,
        ble_mesh_light_client_cb_param_t *param)
{
    ble_mesh_light_client_cb_t btc_mesh_cb = (ble_mesh_light_client_cb_t)btc_profile_cb_get(BTC_PID_LIGHT_CLIENT);
    if (btc_mesh_cb) {
...
void btc_ble_mesh_light_client_arg_deep_copy(btc_msg_t *msg, void *p_dest, void *p_src)
{
    ...
    case BTC_BLE_MESH_ACT_LIGHT_CLIENT_GET_STATE: {
        dst->light_client_get_state.params = (ble_mesh_client_common_param_t *)osi_malloc(sizeof(ble_mesh_client_common_param_t));
        dst->light_client_get_state.get_state = (ble_mesh_light_client_get_state_t *)osi_malloc(sizeof(ble_mesh_light_client_get_state_t));
        if (dst->light_client_get_state.params && dst->light_client_get_state.get_state) {
            ...
            memcpy(dst->light_client_get_state.get_state, src->light_client_get_state.get_state,
                   sizeof(ble_mesh_light_client_get_state_t));
        } else {
    ...
    case BTC_BLE_MESH_ACT_LIGHT_CLIENT_SET_STATE: {
        dst->light_client_set_state.params = (ble_mesh_client_common_param_t *)osi_malloc(sizeof(ble_mesh_client_common_param_t));
        dst->light_client_set_state.set_state = (ble_mesh_light_client_set_state_t *)osi_malloc(sizeof(ble_mesh_light_client_set_state_t));
        if (dst->light_client_set_state.params && dst->light_client_set_state.set_state) {
            ...
            memcpy(dst->light_client_set_state.set_state, src->light_client_set_state.set_state,
                   sizeof(ble_mesh_light_client_set_state_t));
        } else {
            BT_LOG_ERROR("%s, Failed to allocate memory, act %d", __func__, msg->act);
        }
        break;
...
{
    ble_mesh_light_client_cb_param_t *p_dest_data = (ble_mesh_light_client_cb_param_t *)p_dest;
    ble_mesh_light_client_cb_param_t *p_src_data = (ble_mesh_light_client_cb_param_t *)p_src;
    u32_t opcode;
    u16_t length;
    ...
    switch (msg->act) {
    case BLE_MESH_BLE_MESH_LIGHT_CLIENT_GET_STATE_EVT:
    case BLE_MESH_BLE_MESH_LIGHT_CLIENT_SET_STATE_EVT:
    case BLE_MESH_BLE_MESH_LIGHT_CLIENT_PUBLISH_EVT:
    ...
    case BLE_MESH_BLE_MESH_LIGHT_CLIENT_TIMEOUT_EVT:
        if (p_src_data->params) {
...
{
    ble_mesh_light_client_cb_param_t *arg = NULL;
    u32_t opcode;
    ...
    arg = (ble_mesh_light_client_cb_param_t *)(msg->arg);
...
static void btc_mesh_light_client_callback(ble_mesh_light_client_cb_param_t *cb_params, uint8_t act)
{
    btc_msg_t msg = {0};
    ...
    btc_transfer_context(&msg, cb_params,
                         sizeof(ble_mesh_light_client_cb_param_t), btc_ble_mesh_copy_req_data);
}
...
{
    ble_mesh_light_client_cb_param_t cb_params = {0};
    ...
    switch (evt_type) {
    case 0x00:
        act = BLE_MESH_BLE_MESH_LIGHT_CLIENT_GET_STATE_EVT;
        break;
    case 0x01:
        act = BLE_MESH_BLE_MESH_LIGHT_CLIENT_SET_STATE_EVT;
        break;
    case 0x02:
        act = BLE_MESH_BLE_MESH_LIGHT_CLIENT_PUBLISH_EVT;
        break;
    case 0x03:
        act = BLE_MESH_BLE_MESH_LIGHT_CLIENT_TIMEOUT_EVT;
        break;
    default:
        BT_LOG_ERROR("%s, Unknown lighting client event type", __func__);
        return;
    }
...
{
    ble_mesh_light_client_cb_param_t light_client_cb = {0};
...
{
    ble_mesh_light_client_cb_param_t *param = NULL;
    ...
    param = (ble_mesh_light_client_cb_param_t *)(msg->arg);

    if (msg->act < BLE_MESH_BLE_MESH_LIGHT_CLIENT_EVT_MAX) {
