...
#include "yoc_ble_mesh_config_model_api.h"

#define CID_NVAL 0xffff

extern s32_t config_msg_timeout;

static inline void btc_ble_mesh_cfg_client_cb_to_app(ble_mesh_cfg_client_cb_event_t event,
        ble_mesh_cfg_client_cb_param_t *param)
{
    ble_mesh_cfg_client_cb_t btc_mesh_cb = (ble_mesh_cfg_client_cb_t)btc_profile_cb_get(BTC_PID_CFG_CLIENT);
    if (btc_mesh_cb) {
        btc_mesh_cb(event, param);
    }
}

static inline void btc_ble_mesh_cfg_server_cb_to_app(ble_mesh_cfg_server_cb_event_t event,
        ble_mesh_cfg_server_cb_param_t *param)
{
    ble_mesh_cfg_server_cb_t btc_mesh_cb = (ble_mesh_cfg_server_cb_t)btc_profile_cb_get(BTC_PID_CFG_SERVER);
    if (btc_mesh_cb) {
        btc_mesh_cb(event, param);
    }
}
...
void btc_ble_mesh_cfg_client_arg_deep_copy(btc_msg_t *msg, void *p_dest, void *p_src)
{
    ...
    case BTC_BLE_MESH_ACT_CONFIG_CLIENT_GET_STATE: {
        dst->cfg_client_get_state.params = (ble_mesh_client_common_param_t *)osi_malloc(sizeof(ble_mesh_client_common_param_t));
        dst->cfg_client_get_state.get_state = (ble_mesh_cfg_client_get_state_t *)osi_malloc(sizeof(ble_mesh_cfg_client_get_state_t));
        if (dst->cfg_client_get_state.params && dst->cfg_client_get_state.get_state) {
            ...
            memcpy(dst->cfg_client_get_state.get_state, src->cfg_client_get_state.get_state,
                   sizeof(ble_mesh_cfg_client_get_state_t));
        } else {
    ...
    case BTC_BLE_MESH_ACT_CONFIG_CLIENT_SET_STATE: {
        dst->cfg_client_set_state.params = (ble_mesh_client_common_param_t *)osi_malloc(sizeof(ble_mesh_client_common_param_t));
        dst->cfg_client_set_state.set_state = (ble_mesh_cfg_client_set_state_t *)osi_malloc(sizeof(ble_mesh_cfg_client_set_state_t));
        if (dst->cfg_client_set_state.params && dst->cfg_client_set_state.set_state) {
            ...
            memcpy(dst->cfg_client_set_state.set_state, src->cfg_client_set_state.set_state,
                   sizeof(ble_mesh_cfg_client_set_state_t));
        } else {
            BT_LOG_ERROR("%s, Failed to allocate memory, act %d", __func__, msg->act);
        }
        break;
...
{
    ble_mesh_cfg_client_cb_param_t *p_dest_data = (ble_mesh_cfg_client_cb_param_t *)p_dest;
    ble_mesh_cfg_client_cb_param_t *p_src_data = (ble_mesh_cfg_client_cb_param_t *)p_src;
    u32_t opcode;
    u16_t length;
    ...
    switch (msg->act) {
    case BLE_MESH_BLE_MESH_CFG_CLIENT_GET_STATE_EVT:
    case BLE_MESH_BLE_MESH_CFG_CLIENT_SET_STATE_EVT:
    case BLE_MESH_BLE_MESH_CFG_CLIENT_PUBLISH_EVT:
        if (p_src_data->params) {
            ...
            case OP_DEV_COMP_DATA_STATUS:
                if (p_src_data->status_cb.comp_data_status.composition_data) {
                    ...
                    p_dest_data->status_cb.comp_data_status.composition_data = (struct net_buf_simple2*)bt_mesh_alloc_buf(length);
                    if (!p_dest_data->status_cb.comp_data_status.composition_data) {
                        BT_LOG_ERROR("%s, Failed to allocate memory, act %d", __func__, msg->act);
                        return;
                    }
                    net_buf_simple_add_mem((struct net_buf_simple*)p_dest_data->status_cb.comp_data_status.composition_data,
    ...
    case BLE_MESH_BLE_MESH_CFG_CLIENT_TIMEOUT_EVT:
        if (p_src_data->params) {
...
{
    ble_mesh_cfg_client_cb_param_t *arg = NULL;
    u32_t opcode;
    ...
    arg = (ble_mesh_cfg_client_cb_param_t *)(msg->arg);
    ...
    case BLE_MESH_BLE_MESH_CFG_CLIENT_PUBLISH_EVT:
        if (arg->params) {
            ...
            case OP_DEV_COMP_DATA_GET:
            case OP_DEV_COMP_DATA_STATUS:
                bt_mesh_free_buf((struct net_buf_simple*)arg->status_cb.comp_data_status.composition_data);
                break;
            case OP_MOD_SUB_GET:
            case OP_MOD_SUB_GET_VND:
            case OP_MOD_SUB_LIST:
...
static void btc_mesh_cfg_client_callback(ble_mesh_cfg_client_cb_param_t *cb_params, uint8_t act)
{
    btc_msg_t msg = {0};
    ...
    btc_transfer_context(&msg, cb_params,
                         sizeof(ble_mesh_cfg_client_cb_param_t), btc_ble_mesh_cfg_client_copy_req_data);
}
...
{
    ble_mesh_cfg_client_cb_param_t cb_params = {0};
    ...
    switch (evt_type) {
    case 0x00:
        act = BLE_MESH_BLE_MESH_CFG_CLIENT_GET_STATE_EVT;
        break;
    case 0x01:
        act = BLE_MESH_BLE_MESH_CFG_CLIENT_SET_STATE_EVT;
        break;
    case 0x02:
        act = BLE_MESH_BLE_MESH_CFG_CLIENT_PUBLISH_EVT;
        break;
    case 0x03:
        act = BLE_MESH_BLE_MESH_CFG_CLIENT_TIMEOUT_EVT;
        break;
    default:
        BT_LOG_ERROR("%s, Unknown config client event type %d", __func__, evt_type);
        return;
    }
...
{
    ble_mesh_cfg_client_cb_param_t cfg_client_cb = {0};
    btc_ble_mesh_cfg_client_args_t *arg = NULL;
    ...
    case BTC_BLE_MESH_ACT_CONFIG_CLIENT_GET_STATE: {
        ...
        if (cfg_client_cb.error_code) {
            btc_mesh_cfg_client_callback(&cfg_client_cb, BLE_MESH_BLE_MESH_CFG_CLIENT_GET_STATE_EVT);
        }
        break;
    }
    case BTC_BLE_MESH_ACT_CONFIG_CLIENT_SET_STATE: {
        ...
        btc_ble_mesh_config_client_set_state(arg->cfg_client_set_state.params,
                                             ...
                                             &cfg_client_cb);
        if (cfg_client_cb.error_code) {
            btc_mesh_cfg_client_callback(&cfg_client_cb, BLE_MESH_BLE_MESH_CFG_CLIENT_SET_STATE_EVT);
        }
        break;
    }
    default:
...
{
    ble_mesh_cfg_client_cb_param_t *param = NULL;
    ...
    param = (ble_mesh_cfg_client_cb_param_t *)(msg->arg);

    if (msg->act < BLE_MESH_BLE_MESH_CFG_CLIENT_EVT_MAX) {
...
int btc_ble_mesh_config_client_get_state(ble_mesh_client_common_param_t *params,
        ble_mesh_cfg_client_get_state_t *get_state,
        ble_mesh_cfg_client_cb_param_t *cfg_client_cb)
{
...
int btc_ble_mesh_config_client_set_state(ble_mesh_client_common_param_t *params,
        ...
        ble_mesh_cfg_client_cb_param_t *cfg_client_cb)
{
    ...
    case BLE_MESH_BLE_MESH_MODEL_OP_MODEL_PUB_SET: {
    ...
    case BLE_MESH_BLE_MESH_MODEL_OP_MODEL_SUB_ADD:
        return (cfg_client_cb->error_code =
    ...
    case BLE_MESH_BLE_MESH_MODEL_OP_MODEL_SUB_DELETE:
        return (cfg_client_cb->error_code =
    ...
    case BLE_MESH_BLE_MESH_MODEL_OP_MODEL_SUB_OVERWRITE:
        return (cfg_client_cb->error_code =
    ...
    case BLE_MESH_BLE_MESH_MODEL_OP_MODEL_SUB_VIRTUAL_ADDR_ADD:
        return (cfg_client_cb->error_code =
    ...
    case BLE_MESH_BLE_MESH_MODEL_OP_MODEL_SUB_VIRTUAL_ADDR_OVERWRITE:
        return (cfg_client_cb->error_code =
    ...
    case BLE_MESH_BLE_MESH_MODEL_OP_MODEL_SUB_VIRTUAL_ADDR_DELETE:
        return (cfg_client_cb->error_code =
    ...
    case BLE_MESH_BLE_MESH_MODEL_OP_MODEL_SUB_DELETE_ALL:
        return (cfg_client_cb->error_code =
    ...
    case BLE_MESH_BLE_MESH_MODEL_OP_KEY_REFRESH_PHASE_SET:
        return (cfg_client_cb->error_code =
                    bt_mesh_cfg_kr_phase_set(&ctx, set_state->kr_phase_set.net_idx, set_state->kr_phase_set.transition));
    case BLE_MESH_BLE_MESH_MODEL_OP_NETWORK_TRANSMIT_SET:
        return (cfg_client_cb->error_code =
...
static void btc_mesh_cfg_server_callback(ble_mesh_cfg_server_cb_param_t *cb_params, uint8_t act)
{
    btc_msg_t msg = {0};
    ...
    btc_transfer_context(&msg, cb_params, sizeof(ble_mesh_cfg_server_cb_param_t), NULL);
}
...
void bt_mesh_callback_cfg_server_event_to_btc(u8_t evt_type, struct bt_mesh_model *model,
        ...
        const u8_t *val, size_t len)
{
    ble_mesh_cfg_server_cb_param_t cb_params = {0};
    size_t  length;
    uint8_t act;
    ...
    switch (evt_type) {
    case 0x00:
        act = BLE_MESH_BLE_MESH_CFG_SERVER_RECV_MSG_EVT;
        break;
    default:
        BT_LOG_ERROR("%s, Unknown config server event type %d", __func__, evt_type);
        return;
    }

    cb_params.model = (ble_mesh_model_t *)model;
...
{
    ble_mesh_cfg_server_cb_param_t *param = NULL;
    ...
    param = (ble_mesh_cfg_server_cb_param_t *)(msg->arg);

    if (msg->act < BLE_MESH_BLE_MESH_CFG_SERVER_EVT_MAX) {
