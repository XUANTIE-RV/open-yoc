...
#define EVENT_PCM_DATA      (1<<4)
#define EVENT_PCM_LAST_DATA (1<<5)
#define EVENT_PCM_EMPTY     (1<<6)
#define EVENT_TASK_QUIT     (1<<7)
...
#define RINGBUFFER_SIZE (64 * 1024 + 1)
...
#define BUF_SIZE 1024
    ...
    cJSON_AddBoolToObject(payload, "enable_punctuation_prediction", true);
...
static rws_socket _aliyun_ws_new(struct aliyun_rasr_priv *priv)
{
    ...
    sock->token = (char *)aliyun_rasr_get_token();
...
#define SLEEP_MS 100
    ...
    if (!priv->sock) {
        ...
        flags = 0;
        aos_event_get(&priv->event, EVENT_ASR_SER_OK | EVENT_DISCONNECT, AOS_EVENT_OR_CLEAR,
                        &flags, -1);
...
out:
    ...
    aos_event_set(&priv->event, EVENT_TASK_QUIT, AOS_EVENT_OR);
    aos_task_exit(0);
}
...
int aliyun_rasr_source_pcm(aui_t *aui, char *data, int size)
{
    ...
    if (priv->ringbuffer.buffer == NULL) {
        ...
        aos_event_new(&priv->event, 0);
        priv->task_quit = 0;
        aos_task_new("aliyun-snd", _send_task, (void *)priv, 4096);
    }
    if (priv->status == 2) {
