...
static void wpa_tx_cb_(void *cb_arg, bool tx_ok)
{
    if (tx_ok) {
        eapol_txcb(NULL);
    }
            ...
            u8 *msg, size_t msg_len, u8 *key_mic)
{
    ...
    wpa_sm_ether_send(sm->own_addr, dest, proto, msg, msg_len);
out:
    return;
}
...
void wpa_sm_key_request(struct wpa_sm *sm, int error, int pairwise)
{
    ...
    rbuf = wpa_sm_alloc_eapol(WPA_SM_EAPOL_VERSION, IEEE802_1X_TYPE_EAPOL_KEY, NULL,
        ...
        const u8 *pmkid)
{
    ...
    if (pmkid && sm->cur_pmksa &&
        ...
        int res = 0, pmk_len;
        pmk_len = PMK_LEN;
        /* For BL_SUPPLICANT this is already set using wpa_set_pmk*/
        ...
        !wpa_key_mgmt_ft(sm->key_mgmt) && sm->key_mgmt != WPA_KEY_MGMT_OSEN)
    {
        ...
        buf = wpa_sm_alloc_eapol(WPA_SM_EAPOL_VERSION, IEEE802_1X_TYPE_EAPOL_START,
                     NULL, 0, &buflen, NULL);
        if (buf) {
            wpa_sm_ether_send(sm->own_addr, sm->bssid, ETH_P_EAPOL,
                      buf, buflen);
            os_free(buf);
            return -2;
        }
                   ...
                   struct wpa_ptk *ptk)
{
    ...
    rbuf = wpa_sm_alloc_eapol(WPA_SM_EAPOL_VERSION, IEEE802_1X_TYPE_EAPOL_KEY,
                         ...
                         struct wpa_gtk_data *gd)
{
    ...
    wpa_parse_kde_ies(keydata, keydatalen, &ie);
    if (ie.gtk && !(key_info & WPA_KEY_INFO_ENCR_KEY_DATA)) {
        #ifdef DEBUG_PRINT
        wpa_printf(MSG_DEBUG, "WPA: GTK IE in unencrypted key data");
      #endif
...
void wpa_eapol_key_dump(int level, const struct wpa_eapol_key *key)
{
    ...
    wpa_printf(MSG_DEBUG, "  key_info 0x%x (ver=%ld keyidx=%ld rsvd=%ld %s"
           "%s%s%s%s%s%s%s)\n",
...
void wpa_sm_set_state(enum wpa_states state)
{
    ...
    if(WPA_MIC_FAILURE==WPA_SM_STATE(sm))
        bl_wifi_timer_disarm(&(sm->cm_timer));
#endif
    sm->wpa_state= state;
}
...
int wpa_set_bss(u8 vif_idx, u8 sta_idx, char *macddr, char *bssid, u8 pairwise_cipher, u8 group_cipher, bool pmf_required, u8 mgmt_group_cipher)
{
    ...
    sm->rx_replay_counter_set = false;  //init state not intall replay counter value
    ...
    if (pmf_required) {
        ...
        sm->pmf_cfg.required = true;
        sm->mgmt_group_cipher = cipher_type_map_public_to_supp(mgmt_group_cipher);
    } else {
    ...
    wpa_config_assoc_ie(sm->vif_idx, sm->proto, sm->assoc_wpa_ie, sm->assoc_wpa_ie_len);
    return 0;
}
...
int wpa_set_parm_at_connect(wifi_connect_parm_t *parm)
{
    ...
    sm->vif_idx = parm->vif_idx;
    sm->sta_idx = parm->sta_idx;

    wpa_set_passphrase(parm->passphrase, parm->ssid.ssid, parm->ssid.len);
...
void wpa_set_passphrase(char *passphrase, u8 *ssid, size_t ssid_len)
{
    ...
    if (strlen(passphrase) == 64) {
        hexstr2bin(passphrase, pmk, PMK_LEN);
    } else {
        pbkdf2_sha1(passphrase, (char *)ssid, ssid_len, 4096, pmk, PMK_LEN);
    }
#if 0
        ...
        bool pairwise)
{
    ...
    wpa_install_key(sm->vif_idx, sm->sta_idx, alg, key_idx, set_tx, seq, seq_len, key, key_len, pairwise);
    return 0;
}

int wpa_sm_get_key(uint8_t *ifx, int *alg, u8 *addr, int *key_idx, u8 *key, size_t key_len, int key_entry_valid)
{
    return wpa_get_key(ifx, alg, addr, key_idx, key, key_len, key_entry_valid);
}
...
static void fourway_hs_timeout_handler_(void *arg)
{
    struct wpa_sm *sm = &gWpaSm;
