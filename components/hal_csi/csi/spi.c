        ...
        uint16_t rx_size, uint32_t timeout)
{
    ...
    csi_spi_send(spi->priv, tx_data, tx_size);
    ret = aos_sem_wait(&spi_sem[spi->port], timeout);
    csi_spi_receive(spi->priv, rx_data, rx_size);
    ret = aos_sem_wait(&spi_sem[spi->port], timeout);
    aos_sem_free(&spi_sem[spi->port]);
    return ret;
        ...
        uint16_t tx2_size, uint32_t timeout)
{
    ...
    csi_spi_send(spi->priv, tx1_data, tx1_size);
    ret = aos_sem_wait(&spi_sem[spi->port], timeout);
    csi_spi_send(spi->priv, tx2_data, tx2_size);
    ret = aos_sem_wait(&spi_sem[spi->port], timeout);
    aos_sem_free(&spi_sem[spi->port]);
    return ret;
