...
#define SC3336_I2C_ADDR_1 0x30
#define SC3336_I2C_ADDR_2 0x32
#define SC3336_I2C_ADDR_IS_VALID(addr)	((addr) == SC3336_I2C_ADDR_1 || (addr) == SC3336_I2C_ADDR_2)

/****************************************************************************
 * global variables                                                            *
 ****************************************************************************/

ISP_SNS_STATE_S *g_pastSC3336[VI_MAX_PIPE_NUM] = {CVI_NULL};
...
#define SC3336_SENSOR_RESET_CTX(dev)         (g_pastSC3336[dev] = CVI_NULL)

ISP_SNS_COMMBUS_U g_aunSC3336_BusInfo[VI_MAX_PIPE_NUM] = {
	[0] = { .s8I2cDev = 2},
	[1 ... VI_MAX_PIPE_NUM - 1] = { .s8I2cDev = -1}
};

CVI_U16 g_au16SC3336_GainMode[VI_MAX_PIPE_NUM] = {0};
CVI_U16 g_au16SC3336_L2SMode[VI_MAX_PIPE_NUM] = {0};

ISP_SNS_MIRRORFLIP_TYPE_E g_aeSc3336_MirrorFip[VI_MAX_PIPE_NUM] = {0};
...
#define SC3336_AGAIN_ADDR                     (0x3e09)
...
#define SC3336_EXPACCURACY                    (1)

static CVI_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	const SC3336_MODE_S *pstMode;
	...
	pstMode = &g_astSC3336_mode[pstSnsState->u8ImgMode];
#if 0
	...
	pstAeSnsDft->stIntTimeAccu.f32Accuracy = SC3336_EXPACCURACY;
...
static CVI_S32 cmos_fps_set(VI_PIPE ViPipe, CVI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	u32Vts = g_astSC3336_mode[pstSnsState->u8ImgMode].u32VtsDef;
	pstSnsRegsInfo = &pstSnsState->astSyncInfo[0].snsCfg;
	f32MaxFps = g_astSC3336_mode[pstSnsState->u8ImgMode].f32MaxFps;
	f32MinFps = g_astSC3336_mode[pstSnsState->u8ImgMode].f32MinFps;

	switch (pstSnsState->u8ImgMode) {
	case SC3336_MODE_2304X1296P30:
...
static CVI_S32 cmos_get_wdr_size(VI_PIPE ViPipe, ISP_SNS_ISP_INFO_S *pstIspCfg)
{
	const SC3336_MODE_S *pstMode = CVI_NULL;
	ISP_SNS_STATE_S *pstSnsState = CVI_NULL;
...
static CVI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, CVI_U8 u8Mode)
{
	...
	switch (u8Mode) {
	case WDR_MODE_NONE:
		pstSnsState->u8ImgMode = SC3336_MODE_2304X1296P30;
		pstSnsState->enWDRMode = WDR_MODE_NONE;
		pstSnsState->u32FLStd = g_astSC3336_mode[pstSnsState->u8ImgMode].u32VtsDef;
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		pstCfg0->snsCfg.unComBus.s8I2cDev = g_aunSC3336_BusInfo[ViPipe].s8I2cDev;
		...
		for (i = 0; i < pstCfg0->snsCfg.u32RegNum; i++) {
			...
			pstI2c_data[i].u8DevAddr = sc3336_i2c_addr;
			pstI2c_data[i].u32AddrByteNum = sc3336_addr_byte;
			pstI2c_data[i].u32DataByteNum = sc3336_data_byte;
		}

		switch (pstSnsState->enWDRMode) {
		case WDR_MODE_NONE:
			//Linear Mode Regs
			pstI2c_data[LINEAR_EXP_H_ADDR].u32RegAddr = SC3336_EXP_H_ADDR;
			pstI2c_data[LINEAR_EXP_M_ADDR].u32RegAddr = SC3336_EXP_M_ADDR;
			pstI2c_data[LINEAR_EXP_L_ADDR].u32RegAddr = SC3336_EXP_L_ADDR;
			pstI2c_data[LINEAR_AGAIN_ADDR].u32RegAddr = SC3336_AGAIN_ADDR;
			pstI2c_data[LINEAR_DGAIN_H_ADDR].u32RegAddr = SC3336_DGAIN_H_ADDR;
			pstI2c_data[LINEAR_DGAIN_L_ADDR].u32RegAddr = SC3336_DGAIN_L_ADDR;
			pstI2c_data[LINEAR_VMAX_H_ADDR].u32RegAddr = SC3336_VMAX_H_ADDR;
			pstI2c_data[LINEAR_VMAX_L_ADDR].u32RegAddr = SC3336_VMAX_L_ADDR;
			break;
		default:
			CVI_TRACE_SNS(CVI_DBG_ERR, "Not support WDR: %d\n", pstSnsState->enWDRMode);
			return CVI_FAILURE;
	...
	if (pstSensorImageMode->f32Fps <= 30) {
		if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
			if (SC3336_RES_IS_1296P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
				u8SensorImageMode = SC3336_MODE_2304X1296P30;
			} else {
...
static CVI_VOID sensor_mirror_flip(VI_PIPE ViPipe, ISP_SNS_MIRRORFLIP_TYPE_E eSnsMirrorFlip)
{
	...
	if (pstSnsState->bInit == CVI_TRUE && g_aeSc3336_MirrorFip[ViPipe] != eSnsMirrorFlip) {
		sc3336_mirror_flip(ViPipe, eSnsMirrorFlip);
		g_aeSc3336_MirrorFip[ViPipe] = eSnsMirrorFlip;
	}
}
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	memcpy(pstRxAttr, &sc3336_rx_attr, sizeof(*pstRxAttr));

	pstRxAttr->img_size.width = g_astSC3336_mode[pstSnsState->u8ImgMode].astImg[0].stSnsSize.u32Width;
	pstRxAttr->img_size.height = g_astSC3336_mode[pstSnsState->u8ImgMode].astImg[0].stSnsSize.u32Height;
	if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
...
{
	SNS_COMBO_DEV_ATTR_S *pstRxAttr = &sc3336_rx_attr;
	int i;
...
static CVI_S32 cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
	...
	pstSensorExpFunc->pfn_cmos_sensor_init = sc3336_init;
	pstSensorExpFunc->pfn_cmos_sensor_exit = sc3336_exit;
...
{
	if (SC3336_I2C_ADDR_IS_VALID(s32I2cAddr))
		sc3336_i2c_addr = s32I2cAddr;
}

static CVI_S32 sc3336_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
	g_aunSC3336_BusInfo[ViPipe].s8I2cDev = unSNSBusInfo.s8I2cDev;
...
static CVI_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
	...
	s32Ret = CVI_ISP_SensorUnRegCallBack(ViPipe, SC3336_ID);
	if (s32Ret != CVI_SUCCESS) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "sensor unregister callback function failed!\n");
		return s32Ret;
	}

	s32Ret = CVI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, SC3336_ID);
	if (s32Ret != CVI_SUCCESS) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "sensor unregister callback function to ae lib failed!\n");
		return s32Ret;
	}

	s32Ret = CVI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, SC3336_ID);
	if (s32Ret != CVI_SUCCESS) {
...
static CVI_S32 sensor_set_init(VI_PIPE ViPipe, ISP_INIT_ATTR_S *pstInitAttr)
{
	...
	g_au16SC3336_GainMode[ViPipe] = pstInitAttr->enGainMode;
	g_au16SC3336_L2SMode[ViPipe] = pstInitAttr->enL2SMode;

	return CVI_SUCCESS;
}

ISP_SNS_OBJ_S stSnsSC3336_Obj = {
	.pfnRegisterCallback	= sensor_register_callback,
	.pfnUnRegisterCallback	= sensor_unregister_callback,
	.pfnStandby		= sc3336_standby,
	.pfnRestart		= sc3336_restart,
	.pfnMirrorFlip		= sensor_mirror_flip,
	.pfnWriteReg		= sc3336_write_register,
	.pfnReadReg		= sc3336_read_register,
	.pfnSetBusInfo		= sc3336_set_bus_info,
