...
extern int32_t target_get_rtc(int32_t idx, uint32_t *base);
...
static const rtc_capabilities_t rtc_capabilities = {
    .interrupt_mode = 0,    /* supports Interrupt mode */
    .wrap_mode = 1          /* supports wrap mode */
};


extern void WaitRTCCount(uint32_t rtcDelyCnt);

static uint8 phy_rc32k_calibration(void)
{
...
{
    return (addr->RTC_CNT + (addr->RTC_CC2) * (0xffffffff + 1)) / 8;
}
...
rtc_handle_t csi_rtc_initialize(int32_t idx, rtc_event_cb_t cb_event)
{
    ...
    real_idx = target_get_rtc(idx, &base);
...
//    drv_irq_register(RTC_IRQn, handler);
//    drv_irq_enable(RTC_IRQn);
...
int32_t csi_rtc_uninitialize(rtc_handle_t handle)
{
    ...
    drv_irq_disable(RTC_IRQn);
    drv_irq_unregister(RTC_IRQn);
