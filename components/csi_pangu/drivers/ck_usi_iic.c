...
#else
        addr->USI_CLK_DIV0  = (drv_get_apb_freq() / speed - 2) / 2;
#endif
...
iic_handle_t drv_usi_iic_initialize(int32_t idx, iic_event_cb_t cb_event)
{
    ...
    int32_t ret = target_usi_iic_init(idx, &base, &irq, &handler);
...
int32_t drv_usi_iic_master_send(iic_handle_t handle, uint32_t devaddr, const void *data, uint32_t num, bool xfer_pending)
{
    ...
    addr->USI_INTR_UNMASK = USI_INT_I2C_STOP | USI_INT_TX_EMPTY | USI_INT_I2C_NACK;
    addr->USI_INTR_EN = 0;
    ...
    iic_priv->tx_buf++;
    iic_priv->tx_cnt++;
    addr->USI_INTR_EN = USI_INT_I2C_STOP | USI_INT_TX_EMPTY | USI_INT_I2C_NACK;
    /* blocking mode */
#if 0
    addr->USI_INTR_EN = 0;
    addr->USI_EN = 0xf;     /* enable USI module */
