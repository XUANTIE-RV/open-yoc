...
#include <ck_etb.h>
#endif
#if defined CONFIG_CHIP_CH2201 || defined CONFIG_CHIP_DANICA
#include <pinmux.h>
#include <pin_name.h>
#include <drv/gpio.h>
#endif
...
extern void drv_pinmux_dwuart_restore(pin_name_e tx_pin, pin_name_e rx_pin);
extern void drv_pinmux_dwuart_save(int32_t idx, pin_name_e *tx_pin, pin_name_e *rx_pin);
static void pin_uart_to_gpio(usart_handle_t handle, pin_name_e *tx_pin, pin_name_e *rx_pin)
{
    dw_usart_priv_t *usart_priv = handle;
    drv_pinmux_dwuart_save(usart_priv->idx, tx_pin, rx_pin);
}

static void pin_gpio_to_uart(usart_handle_t handle, pin_name_e tx_pin, pin_name_e rx_pin)
{
    drv_pinmux_dwuart_restore(tx_pin, rx_pin);
}
#endif
...
int32_t usart_wait_timeout(usart_handle_t handle, dw_usart_reg_t *addr)
{
...
/**
  ...
  \return      error code
*/
int32_t ATTRIBUTE_DATA csi_usart_config_baudrate(usart_handle_t handle, uint32_t baud)
{
    ...
    pin_name_e tx_pin;
    pin_name_e rx_pin;
    pin_uart_to_gpio(handle, &tx_pin, &rx_pin);
#endif
...
#if defined CONFIG_CHIP_CH2201 || defined CONFIG_CHIP_DANICA
    pin_gpio_to_uart(handle, tx_pin, rx_pin);
#endif
...
static int32_t dw_usart_send_dma(dw_usart_priv_t *usart_priv)
{
    ...
    if (usart_priv->dma_tx_ch  == -1) {
        ...
        for (i = 0; i < CONFIG_PER_DMAC0_CHANNEL_NUM; i++) {
            usart_priv->dma_tx_ch  = csi_dma_alloc_channel_ex(i);
    ...
    int32_t ret = csi_dma_config_channel(usart_priv->dma_tx_ch, &config, (dma_event_cb_t)dw_usart_dma_event_cb, NULL);
...
static int32_t dw_usart_send_dma(dw_usart_priv_t *usart_priv)
{
    ...
    ret = csi_dma_config_channel(usart_priv->dma_tx_ch, &config, (dma_event_cb_t)dw_usart_dma_event_cb, NULL);
...
static int32_t dw_usart_receive_dma(dw_usart_priv_t *usart_priv)
{
    ...
    if (usart_priv->dma_rx_ch == -1) {
        ...
        for (i = 0; i < CONFIG_PER_DMAC0_CHANNEL_NUM; i++) {
            usart_priv->dma_rx_ch  = csi_dma_alloc_channel_ex(i);
    ...
    int32_t ret = csi_dma_config_channel(usart_priv->dma_rx_ch, &config, (dma_event_cb_t)dw_usart_dma_event_cb, NULL);
...
static int32_t dw_usart_receive_dma(dw_usart_priv_t *usart_priv)
{
    ...
    ret = csi_dma_config_channel(usart_priv->dma_rx_ch, &config, (dma_event_cb_t)dw_usart_dma_event_cb, NULL);
