...
static int bc_trigger(int pin_id, int evt_id)
{
    bc_msg_t msg;
...
static void bc_button_disable(bc_t *bc)
{
    for (int j = 0; j < MAX_COMBINATION_NUM; j ++) {
        button_t *b = button_find(bc->b.pin_id[j]);
        b->active = 0;
    }
}
...
static bc_t *bc_find(int pin_id)
{
    bc_t *bc;
...
static void bc_button_hdl(void)
{
    for (int i = 0; i < g_srv_bc.bc_num; i++) {
        ...
        if (bc->count != 0) {
            for (int j = 0; j < MAX_COMBINATION_NUM; j ++) {
                if (bc->count & (1 << j)) {
                    button_t *button = button_find(bc->b.pin_id[j]);
                    event_call(button);
                }
            }
...
int button_combination_init(const button_combinations_t bc_tbl[])
{
    ...
    while (1) {
        ...
        for (j = 0; j < bc_tbl[i].pin_sum; j++) {
            b[j] = button_find(bc_tbl[i].pin_id[j]);
...
static int _button_hdl(button_t *button)
{
    ...
    if (button->bc_flag && button->active == 1) {
        bc_t *bc = bc_find(button->pin_id);

        if (bc && (bc->b.evt_flag & (1 << button->evt_id))) {
            int ret = bc_trigger(button->pin_id, button->evt_id);
...
{
    button_t *button = button_find(pin_id);
