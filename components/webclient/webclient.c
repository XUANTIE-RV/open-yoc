...
static int webclient_read_line(struct webclient_session *session, char *buffer, int size)
{
    ...
    if (count > size) {
        LOGE(TAG, "read line failed. The line data length is out of buffer size(%d)!", count);
                                     ...
                                     const char *url, char **request)
{
    ...
    LOGI(TAG, "host address: %s , port: %s", session->host, port_str);
    ...
    /* resolve the host name. */
    {
        ...
        if(session->tls_session) {
            ...
            if (ret != 0) {
                LOGE(TAG, "getaddrinfo err: %d '%s'", ret, session->host);
                rc = -WEBCLIENT_ERROR;
            }
        ...
        if (ret != 0) {
            LOGE(TAG, "getaddrinfo err: %d, host = '%s', port_str = %s.", ret, session->host, port_str);
            rc = -WEBCLIENT_ERROR;
            goto __exit;
        }
...
static int webclient_open_tls(struct webclient_session *session, const char *URI)
{
    ...
    if(session->tls_session->buffer == NULL) {
        LOGE(TAG, "no memory for tls_session buffer!");
        return -WEBCLIENT_ERROR;
    }

    if((tls_ret = mbedtls_client_init(session->tls_session, (void *)pers, strlen(pers))) < 0) {
        LOGE(TAG, "initialize https client failed return: -0x%x.", -tls_ret);
        return -WEBCLIENT_ERROR;
    }
...
int webclient_connect(struct webclient_session *session, const char *URI)
{
    ...
    if (strncmp(URI, "https://", 8) == 0) {
        ...
        if(webclient_open_tls(session, URI) < 0) {
            LOGE(TAG, "connect failed, https client open URI(%s) failed!", URI);
            return -WEBCLIENT_ERROR;
        }
#else
        LOGE(TAG, "not support https connect, please enable webclient https configure!");
        rc = -WEBCLIENT_ERROR;
        goto __exit;
#endif
    }
    ...
    if (rc != WEBCLIENT_OK) {
        LOGE(TAG, "connect failed, resolve address error(%d).", rc);
        goto __exit;
    }
    ...
    if (session->tls_session) {
        ...
        if ((tls_ret = mbedtls_client_context(session->tls_session)) < 0) {
            LOGE(TAG, "connect failed, https client context return: -0x%x", -tls_ret);
            return -WEBCLIENT_ERROR;
        }

        if ((tls_ret = mbedtls_client_connect(session->tls_session)) < 0) {
            LOGE(TAG, "connect failed, https client connect return: -0x%x", -tls_ret);
...

    {
        ...
        if (socket_handle < 0) {
            LOGE(TAG, "connect failed, create socket(%d) error.", socket_handle);
        ...
        if (connect(socket_handle, res->ai_addr, res->ai_addrlen) != 0) {
            /* connect failed */
            LOGE(TAG, "connect failed, connect socket(%d) error.", socket_handle);
            rc = -WEBCLIENT_CONNECT_FAILED;
            goto __exit;
        }
...
int webclient_header_fields_add(struct webclient_session *session, const char *fmt, ...)
{
    ...
    if (session->header->length >= session->header->size) {
        LOGE(TAG, "not enough header buffer size(%d)!", session->header->size);
        return -WEBCLIENT_ERROR;
    }
        ...
        if (method != WEBCLIENT_USER_METHOD) {
            ...
            if (memcmp(header, "HTTP/1.", strlen("HTTP/1."))) {
                ...
                if (header_buffer == NULL) {
                    LOGE(TAG, "no memory for header buffer!");
                    rc = -WEBCLIENT_NOMEM;
                    goto __exit;
                }
            ...
            /* check header size */
            if (session->header->length > session->header->size) {
                LOGE(TAG, "send header failed, not enough header buffer size(%d)!", session->header->size);
...
int webclient_handle_response(struct webclient_session *session)
{
    ...
    if (mime_ptr == NULL) {
        LOGE(TAG, "no memory for get http status code buffer!");
        return -WEBCLIENT_NOMEM;
    }
...
struct webclient_session *webclient_session_create(size_t header_sz)
{
    ...
    if (session == NULL) {
        LOGE(TAG, "webclient create failed, no memory for webclient session!");
        return NULL;
    }
    ...
    if (session->header == NULL) {
        LOGE(TAG, "webclient create failed, no memory for session header!");
        web_free(session);
        session = NULL;
        return NULL;
    }
    ...
    if (session->header->buffer == NULL) {
        LOGE(TAG, "webclient create failed, no memory for session header buffer!");
...
int webclient_get(struct webclient_session *session, const char *URI)
{
    ...
    if (resp_status > 0) {
        ...
        if ((resp_status == 302 || resp_status == 301) && location) {
            ...
            LOGE(TAG, "get failed, handle response(%d) error!", resp_status);
            return resp_status;
        }
    }
...
int webclient_post(struct webclient_session *session, const char *URI, const char *post_data)
{
    ...
    if (post_data) {
        ...
        resp_status = webclient_handle_response(session);
        if (resp_status != 200) {
            LOGE(TAG, "post failed, handle response(%d) error.", resp_status);
            return resp_status;
        }
    }
    ...
    do {
        ...
        if (bytes_read <= 0) {
            ...
            if(session->tls_session && bytes_read == MBEDTLS_ERR_SSL_WANT_READ)
                continue;
#endif
            LOGE(TAG, "receive data error(%d).", bytes_read);
    ...
    *response = NULL;
    ...
    if (session->content_length < 0) {
        ...
        while (1) {
            ...
            if (new_resp == NULL) {
                LOGE(TAG, "no memory for realloc new response buffer!");
                break;
            }
...
int webclient_request(const char *URI, const char *header, const char *post_data, unsigned char **response)
{
    ...
    if (post_data == NULL && response == NULL) {
        LOGE(TAG, "request get failed, get response data cannot be empty.");
        return -WEBCLIENT_ERROR;
    }
