...
#define TM_CAMERA_CHANNEL_MAX_COUNT 3
...
class TMCamera : public TMVideoInput
{
   public:
    enum class PropID : uint32_t
    {
        ...
        CAMERA_EXPOSURE_MODE,              // enum, 0: MODE_AUTO; 1: MANUAL; 2: SHUTTER_PRIORITY; 3: APERTURE_PRIORITY; 4: GAIN PRIORITY
        CAMERA_EXPOSURE_BIAS,              // int, Exposure Bias, min: -6, max: 6, step: 1, default: 0
        CAMERA_AUTO_PRESET_WHITE_BALANCE,  // enum, Auto White Balance, 0: MANUAL; 1: AUTO; 2: CUSTOM1; 3: CUSTOM2; 4: TUNGSTEN; 5: INCANDESCENT; 6: SUNNY; 7: CLOUDY
        ...
        CAMERA_EXPOSURE_ABSOLUTE,          // int, sensor exposure time/100us, min: 1, max: 10000, step: 1, default: 100
        ...
        CHANNEL_OUTPUT_FRAMERATE,     // int, 30, 24, 20, 15, 10, 5 maybe 60?
        CHANNEL_FRAME_POOL_INIT_COUNT,
    };
   ...
   protected:
    ...
    virtual void InitDefaultPropertyList()
    {
        ...
        for (int loop = 0; loop < 2; loop++)
        {
            ...
            pList->Add(TMProperty((int)PropID::CAMERA_AUTO_PRESET_WHITE_BALANCE, 1, "auto preset white balance"));
            ...
            pList->Add(TMProperty((int)PropID::CAMERA_EXPOSURE_ABSOLUTE, 100, "exposure value"));

            for (uint32_t i = 0; i < TM_CAMERA_CHANNEL_MAX_COUNT; i++)
            {
                pList = (loop == 0) ? &mDefaultChannelPropertyList[i] : &mCurrentChannelPropertyList[i];
                pList->Add(
                    TMProperty((int)PropID::CHANNEL_CAPTURE_TYPE, TMCAMERA_CAPTURE_VIDEO, "channel capture type"));
                ...
                pList->Add(TMProperty((int)PropID::CHANNEL_OUTPUT_PIXEL_FORMAT,
                                      (int32_t)(TMImageInfo::PixelFormat::PIXEL_FORMAT_NV12),
                                      "channel output pixel format"));
                pList->Add(TMProperty((int)PropID::CHANNEL_OUTPUT_FRAMERATE, -1, "channel frame rate(fps)"));
                pList->Add(TMProperty((int)PropID::CHANNEL_FRAME_POOL_INIT_COUNT, -1, "channel frame pool count"));
            }
        }
    }
};
