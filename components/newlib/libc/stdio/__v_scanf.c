/*
 * __v_scanf.c - the file contains the functions __v_scanf.
 *
...
exp_out:
		...
		if (neg) d*=-1;//consumed=consumed-1;}
		*pd=d;
	      } 
	      else 
	      {
		pf=(float *)va_arg(arg_ptr,float*);
		if (neg) d*=-1;//consumed=consumed-2;}
		*pf=d;
	      }
	      ++n;
	    }
