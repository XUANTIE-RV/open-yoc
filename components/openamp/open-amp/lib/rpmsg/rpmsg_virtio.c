...
static void rpmsg_virtio_return_buffer(struct rpmsg_virtio_device *rvdev,
				       void *buffer, unsigned short idx)
{
...
static int _rpmsg_virtio_get_buffer_size(struct rpmsg_virtio_device *rvdev)
{
	...
	if (role == RPMSG_REMOTE) {
		 ...
		 */
		length =
		    virtqueue_get_desc_size(rvdev->svq) -
...
static void rpmsg_virtio_release_rx_buffer(struct rpmsg_device *rdev, void *rxbuf)
{
	...
	rpmsg_virtio_return_buffer(rvdev, rp_hdr, rp_hdr->reserved);
					       ...
					       const void *data, int len)
{
	...
	status = rpmsg_virtio_enqueue_buffer(rvdev, hdr, idx);
...
static void rpmsg_virtio_rx_callback(struct virtqueue *vq)
{
	...
	while (rp_hdr) {
		...
		if (rp_hdr->reserved & RPMSG_BUF_HELD) {
			...
			/* Return used buffers. */
			rpmsg_virtio_return_buffer(rvdev, rp_hdr, idx);
		}
				    ...
				    size_t len, uint32_t src, void *priv)
{
	...
	if (ns_msg->flags == RPMSG_NS_DESTROY) {
		...
		if (!_ept) {
			...
			_ept->dest_addr = dest;
			metal_mutex_release(&rdev->lock);
			if (rdev->support_bind)
				rpmsg_send_ns_message(_ept, RPMSG_NS_BIND);
		}
	} else {
		if (_ept)
			_ept->dest_addr = dest;
		    ...
		    struct rpmsg_virtio_shm_pool *shpool)
{
	...
	rdev->support_bind = !!(vdev->features & (1 << VIRTIO_RPMSG_F_BIND));
	 ...
	 */
	if (rdev->support_ns || rdev->support_bind) {
