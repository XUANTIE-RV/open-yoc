...
extern int32_t target_usart_init(int32_t idx, uint32_t *base, int32_t *irq, void **handler);
...
#if defined  CONFIG_DW_DMAC
static void dw_usart_dma_event_cb(int32_t ch, dma_event_e event, void *cb_arg)
{
...
int32_t csi_usart_config_baudrate(usart_handle_t handle, uint32_t baud)
{
    ...
    uint32_t divisor = ((drv_get_usart_freq(0)  * 10) / baud) >> 4;
...
static int32_t dw_usart_send_dma(dw_usart_priv_t *usart_priv)
{
    ...
    ret = csi_dma_config_channel(usart_priv->dma_tx_ch, &config, dw_usart_dma_event_cb, NULL);
...
static int32_t dw_usart_receive_dma(dw_usart_priv_t *usart_priv)
{
    ...
    int32_t ret = csi_dma_config_channel(usart_priv->dma_rx_ch, &config, dw_usart_dma_event_cb, NULL);
...
static int32_t dw_usart_receive_dma(dw_usart_priv_t *usart_priv)
{
    ...
    ret = csi_dma_config_channel(usart_priv->dma_rx_ch, &config, dw_usart_dma_event_cb, NULL);
