...
int32_t csi_iic_config_mode(iic_handle_t handle, iic_mode_e mode)
{
    ...
    switch (mode) {
        case IIC_MODE_MASTER:
            ...
            addr->IC_CON |= DW_IIC_CON_SLAVE_DISABLE;
            break;

        case IIC_MODE_SLAVE:
            iic_priv->mode = IIC_MODE_SLAVE;
            addr->IC_CON |= DW_IIC_CON_SLAVE_DISABLE;  /* not support slave mode */
            addr->IC_CON &= ~DW_IIC_CON_MASTER_ENABLE;  /* master disabled */
