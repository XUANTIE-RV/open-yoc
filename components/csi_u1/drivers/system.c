...
int g_system_clock = 91000000;
int g_uart_clock = 14745600;
...
static void _system_init_for_kernel(void)
{
    ...
    irq_vectors_init();
#endif
    csi_coret_config(drv_get_sys_freq() / CONFIG_SYSTICK_HZ, PWR_IRQn);    //10ms
    drv_irq_enable(PWR_IRQn);
    drv_irq_enable(CK802_CTIM_INT_VLD_IRQn);
