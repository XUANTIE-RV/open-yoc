...
static inline struct pbuf* _handle_frame_from_stack_with_zeroycopy(void *swdesc, unsigned int msdu_offset, struct wifi_pkt *pkt)
{
    struct pbuf *h, *t;
    int i;
        ...
        if (hw_rxhdr->flags_is_80211_mpdu) {
            ...
            if (bl_vif) {
                ...
                if (bl_wifi_pkt_eth_input_hook) {
                    ...
                    if (h == NULL) {
                        // hook ate the packet, so keep pbuf in pool
                        return -1;//Still free since we copy pbuf with pool
                    }
                }
#endif
...
#if 0
#undef printf
                extern uint8_t tcpip_stack_input_printf;
                if (tcpip_stack_input_printf) {
                    uint8_t seq[4];
                    uint32_t seq_num;
                    uint8_t *pay_data = skb_payload;
                    pay_data += 14;
                    memcpy(seq, pay_data+24, sizeof(seq));
                    if (pay_data[9] == 0x06) {
                        seq_num = (uint32_t)seq[0] << 24 | (uint32_t)seq[1] << 16 | (uint32_t)seq[2] << 8 | (uint32_t)seq[3];

                        //printf("\r\n");
                        //for (int i = 0; i < 32; i++) {
                        //    printf("%02x ", pay_data[i]);
                        //}
                        printf("\r\nseq:%08x %lu, len:%d\r\n", seq_num, seq_num, hw_rxhdr->hwvect.len);
                    }
                }
#endif
...
#else
                h = _handle_frame_from_stack_with_zeroycopy(swdesc, msdu_offset, pkt);
                if (bl_vif->dev && ERR_OK == bl_vif->dev->input(h, bl_vif->dev)) {
                    return 0;
                }
#endif
