...
static partition_t spiffs_partition = -1;
// #define USE_MEM_DEBUG
#ifdef USE_MEM_DEBUG
static uint8_t *g_spimem = NULL;
#endif
...
static int32_t spiffs_hal_read(uint32_t addr, uint32_t size, uint8_t *dst)
{
    ...
    return 0;
#endif
    int ret = partition_read(spiffs_partition, addr, dst, size);
    return ret == 0 ? 0 : -1;
}
...
{
#ifdef USE_MEM_DEBUG
    memcpy((uint8_t *)addr, dst, size);
    return 0;
#endif
    int ret = partition_write(spiffs_partition, addr, dst, size);
    return ret == 0 ? 0 : -1;
}
...
static int32_t spiffs_hal_erase(uint32_t addr, uint32_t size)
{
    ...
    return 0;
#endif
    partition_info_t *node = hal_flash_get_info(spiffs_partition);
    if (!node || size % node->sector_size != 0) {
        return -1;
    }

    int ret = partition_erase(spiffs_partition, addr, size / node->sector_size);
    return ret == 0 ? 0 : -1;
}
...
static aos_dirent_t *_spiffs_readdir(file_t *fp, aos_dir_t *dir)
{
    ...
    strncpy(dp->cur_dirent.d_name, p, SPIFFS_OBJ_NAME_LEN);
...
static void _spiffs_deinit(void)
{
    ...
    if (spiffs_partition != -1) {
        partition_close(spiffs_partition);
    }
...
static int _spiffs_init(const char *partition_name)
{
    if (g_spiffs_mgr) {
        return 0;
    }
    ...
    spiffs_partition = partition_open(partition_name);
    partition_info_t *node = hal_flash_get_info(spiffs_partition);
    if (!node) {
        SPIFFS_DBG("partition get info error");
        goto err;
    }
    ...
    g_spiffs_mgr->cfg->log_block_size    = node->sector_size;
    g_spiffs_mgr->cfg->log_page_size     = CONFIG_SPIFFS_LOG_PAGE_SZ;
    g_spiffs_mgr->cfg->phys_addr         = 0;
    g_spiffs_mgr->cfg->phys_erase_block  = node->sector_size;
    g_spiffs_mgr->cfg->phys_size         = node->length;

    SPIFFS_DBG("description: %s", node->description);
    SPIFFS_DBG("sector_size: 0x%x", node->sector_size);
    SPIFFS_DBG("base_addr: 0x%x", node->base_addr);
    SPIFFS_DBG("start_addr: 0x%x", node->start_addr);
    SPIFFS_DBG("size: 0x%x", node->length);

#ifdef USE_MEM_DEBUG
    g_spimem = aos_zalloc(0x200000 + 4096);
    uint32_t spimem = (uint32_t)g_spimem + 4096 - ((uint32_t)g_spimem % 4096);
    if (!g_spimem) {
        SPIFFS_DBG("mem alloc error");
        goto err;
    }
    g_spiffs_mgr->cfg->log_block_size    = 4096;
    g_spiffs_mgr->cfg->log_page_size     = CONFIG_SPIFFS_LOG_PAGE_SZ;
    g_spiffs_mgr->cfg->phys_addr         = spimem;
    g_spiffs_mgr->cfg->phys_erase_block  = 4096;
    g_spiffs_mgr->cfg->phys_size         = 0x200000;
#endif


#if 0//SPIFFS_SINGLETON == 0
    ...
    // g_spiffs_mgr->fs->cfg = *g_spiffs_mgr->cfg;
    memcpy(&g_spiffs_mgr->fs->cfg, g_spiffs_mgr->cfg, sizeof(spiffs_config));
...
int vfs_spiffs_register(const char *partition_name)
{
    ...
    ret = _spiffs_init(partition_name);
    if (ret != SPIFFS_OK) {
        return ret;
    }
