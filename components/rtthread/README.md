...
| libcpu/risc-v | 玄铁系列内核移植代码，例如C906,C908等 |
| src           | RT-Thread 内核的源文件。              |
| documentation | 文档相关附件。                        |

RT-Thread内核的源码适配到YOC平台的AOS层，可直接调用AOS层API，同时平台将许多RT-Thread的组件移植至YOC平台，用户可参考这些组件，移植相应的组件至YOC平台。

### 使能rtthread组件

当使用全志D1平台时，找到相对路径`components/sdk_chip_d1`下的`package.yaml`配置文件，进行如下配置：
...
depends:
  ...
  # - rhino: v7.8.y
  # - rhino_arch: v7.8.y
  # - rhino_pwrmgmt: v7.8.y
  # - freertos: v7.8.y
  - rtthread: v7.8.y // 使能rtthread内核
```

上述配置即可将rtthread内核加到工程中。

### 配置文件说明

rtconfig.h是rtthread组件的配置文件，通过该文件可进行组件的功能裁剪，[配置文件说明](./documentation/README.md)
...
- RISC-V RV32E/RV32I[F]/RV64[D]：如芯片制造商sifive、[嘉楠Kendryte](https://github.com/RT-Thread/rt-thread/tree/master/bsp/k210)、[博流](https://github.com/RT-Thread/rt-thread/tree/master/bsp/bouffalo_lab)、[芯来Nuclei](https://nucleisys.com/)、[达摩院玄铁](https://www.xrvm.cn)、[先楫](https://github.com/RT-Thread/rt-thread/tree/master/bsp/hpmicro)
- ARC：如芯片制造商SYNOPSYS
- DSP：如芯片制造商 TI
- C-Sky
- x86
