...
uint16_t bt_gatt_get_service_handle(struct bt_gatt_service *serv)
{
    struct bt_gatt_service *tmp = NULL;
    uint16_t last_handle = bt_gatt_get_last_handle();
    uint16_t handle = 0;

    /* DB shall always have its service in ascending order */
    SYS_SLIST_FOR_EACH_CONTAINER(&db, tmp, node) {
        if (tmp == serv) {
            if(tmp->attrs[0].handle <= last_handle){
                handle = tmp->attrs[0].handle;
            }else{
                handle = last_handle;
            }
        }
