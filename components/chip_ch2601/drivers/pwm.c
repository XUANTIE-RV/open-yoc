...
*/
#define WJ_PWM_CAL_ELAPSED_TIME_US(_CUR_, _PER_)                ((_CUR_ > _PER_) ? (_CUR_ - _PER_) : (((0xFFFFFFFFFFFFFFFFULL - _PER_) + _CUR_) + 1ULL))
...
uint32_t count_timer_sta[8] = {0}; ///< 0:counter,1:timer
/**
                                   ...
                                   uint32_t count)
{
    ...
    do {
        ...
        if (count > 0) {
            ...
            count_timer_sta[channel] = 1;
            wj_pwm_capctl_mode_ch_time(pwm_base, channel);
            wj_pwm_capint_ie_time_ch_en(pwm_base, channel);
...
void csi_pwm_capture_stop(csi_pwm_t *pwm, uint32_t channel)
{
    ...
    if (1 == count_timer_sta[channel]) {
        wj_pwm_capint_ie_time_ch_dis(pwm_base, channel);
    } else {
