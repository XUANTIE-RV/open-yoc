...
static int cvi_json_c_vsnprintf(char *str, size_t size, const char *format, va_list ap)
{
	int ret;
...
#define vsnprintf cvi_json_c_vsnprintf

static int cvi_json_c_snprintf(char *str, size_t size, const char *format, ...)
{
	va_list ap;
	int ret;
	va_start(ap, format);
	ret = cvi_json_c_vsnprintf(str, size, format, ap);
	va_end(ap);
	return ret;
}
#define snprintf cvi_json_c_snprintf
