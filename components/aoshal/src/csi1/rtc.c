...
int32_t hal_rtc_init(rtc_dev_t *rtc)
{
    ...
    rtc_handle_t handle = csi_rtc_initialize(rtc->port, NULL);

    if (!handle) {
        return -1;
    }

    rtc->priv = (void *)handle;
...
int32_t hal_rtc_get_time(rtc_dev_t *rtc, rtc_time_t *time)
{
    ...
    ret = csi_rtc_get_time((rtc_handle_t)rtc->priv, (struct tm *)&tim);
    ...
    time->weekday = ALG_RTC_CAL_WEEK(year, mon, day) % 7 + 1;
...
int32_t hal_rtc_set_time(rtc_dev_t *rtc, const rtc_time_t *time)
{
    ...
    ret = csi_rtc_set_time((rtc_handle_t)rtc->priv, (const struct tm *)&tim);

    if (ret < 0) {
        return -1;
    }

    ret = csi_rtc_start((rtc_handle_t)rtc->priv);
...
int32_t hal_rtc_finalize(rtc_dev_t *rtc)
{
    ...
    ret = csi_rtc_stop((rtc_handle_t)rtc->priv);

    if (ret < 0) {
        return -1;
    }

    ret = csi_rtc_uninitialize((rtc_handle_t)rtc->priv);
