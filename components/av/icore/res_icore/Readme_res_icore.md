...
res_icore，即核间音频重采样库(inter-core resampler)。 该库运行在应用程序处理器(ap)侧, 实际重采样运算工作跑在协处理器(cp)侧。该库屏蔽了底层核间通信的细节，应用开发人员可直接基于该库使用芯片的多核异构重采样运算能力。

###  核间重采样使用示例

```c
#include "res_icore/res_icore.h"
