...
// #define fcntl                 lwip_fcntl //implememt in user c library
#endif
...
int wsession_get_range(wsession_t *session, const char *url, int redirect, int range_s, int range_e)
{
    ...
    char range_v[48];
    int redirect_cnt = 0;
    char *r_url = NULL, *tmp_url = (char*)url;

    CHECK_PARAM(session && url, -1);
retry:
    session->redirect_cnt_max = redirect;
    rc = wsession_open(session, tmp_url, 3000);
    CHECK_RET_WITH_RET(rc == 0, -1);
    ...
    if (session->code != 200) {
        LOGI(TAG, "HTTP response: %d %s, url = %s", session->code, session->phrase, tmp_url);
    }
    switch (session->code) {
    case 200:
    /* Partial Content */
    ...
    case 307:
        ...
        redirect_cnt++;
        CHECK_RET_WITH_GOTO(redirect_cnt < session->redirect_cnt_max, err);
