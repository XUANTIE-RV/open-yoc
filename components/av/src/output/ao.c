...
#define ao_drain_set()      (g_aoers.refs.need_drain = 1)
#define ao_drain_get()      (g_aoers.refs.need_drain)
#define ao_drain_clear()    (g_aoers.refs.need_drain = 0)
...
static int __ao_open(ao_cls_t *ao, sf_t sf, const ao_conf_t *ao_cnf)
{
    ...
    if (ao_ref_open_get() == 0) {
        ...
        if (ao_cnf->resample_rate) {
            osf = sf_make_channel(av_get_ao_channel_num()) | sf_make_rate(ao_cnf->resample_rate) | sf_make_bit(16) | sf_make_signed(1);
        } else {
            osf = sf_make_channel(av_get_ao_channel_num()) | sf_make_rate(sf_get_rate(sf)) | sf_make_bit(16) | sf_make_signed(1);
        }
...
static int __ao_drain(ao_cls_t *ao)
{
...
#if CONFIG_AO_MIXER_SUPPORT
    {
        ...
        mixer_cnl_set_weof(ao->cnl, 1);
        ao_ref_lock();
        if (ao_drain_get() == 0 && mixer_get_nb_cnls(ao_get_mixer()) == 1) {
            ao_drain_set();
        } else {
...
static void _mixer_ao_task(void *arg)
{
    ...
    for (;;) {
        ...
        ao_ref_lock();
        if (ao_drain_get() && mixer_check_empty_weof(ao_get_mixer()) > 0) {
            ao_get_real()->ops->drain(ao_get_real());
            mixer_drain_signal(ao_get_mixer());
            ao_drain_clear();
        }
        ao_ref_unlock();
    }
...
ao_cls_t* ao_open(sf_t sf, const ao_conf_t *ao_cnf)
{
    ...
    if (iao_cnf->resample_rate) {
        osf = sf_make_channel(av_get_ao_channel_num()) | sf_make_rate(iao_cnf->resample_rate) | sf_make_bit(16) | sf_make_signed(1);
    } else {
        osf = sf_make_channel(av_get_ao_channel_num()) | sf_make_rate(sf_get_rate(sf)) | sf_make_bit(16) | sf_make_signed(1);
    }
...
retry:
    ao_ref_lock();
    if (ao_drain_get()) {
        /* FIXME: wait drain out of the 1st-stream when ao_drain calling */
        ao_ref_unlock();
        aos_msleep(10);
        goto retry;
    }
    if (!ao_get_mixer()) {
        ...
        rc = aos_task_new_ext(&task, "mixer_ao", _mixer_ao_task, NULL, 4*1024, AOS_DEFAULT_APP_PRI - 2);
        if (rc != 0) {
        ...
        if (mixer_get_nb_cnls(ao_get_mixer()) > 0) {
            ...
            if (sf_get_rate(msf) != sf_get_rate(o->sf)) {
                ...
                osf   = sf_make_channel(av_get_ao_channel_num()) | sf_make_rate(sf_get_rate(msf)) | sf_make_bit(16) | sf_make_signed(1);
