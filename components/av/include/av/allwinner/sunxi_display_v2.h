...
typedef struct disp_rectsz
{
    unsigned int width;
    unsigned int height;
} disp_rectsz;

typedef struct disp_position
{
    int x;
    int y;
} disp_position;
...
typedef enum {
    DISP_CSC_TYPE_RGB        = 0,
    DISP_CSC_TYPE_YUV444     = 1,//HDMI
    DISP_CSC_TYPE_YUV422     = 2,//TV
...
typedef struct disp_output
{
    unsigned int type;
    unsigned int mode;
} disp_output;

typedef struct disp_rect64
{
    long long x;
    long long y;
    long long width;
...
typedef struct disp_fb_info
{
    ...
    unsigned int          align[3];         //align for 3 comonent,unit: bytes(
                                            //align=2^n,i.e. 1/2/4/8/16/32..)
    ...
    unsigned char         pre_multiply;     //true: pre-multiply fb
...
typedef struct disp_layer_config
{
...
typedef struct disp_colorkey
{
    disp_color      ck_max;
    disp_color      ck_min;
    unsigned int    red_match_rule;//0/1:always match; 2:match if min<=color<=max;
                                   //3:match if color>max or color<min
    unsigned int    green_match_rule;//0/1:always match; 2:match if min<=color<=max;
                                     //3:match if color>max or color<min
    unsigned int    blue_match_rule;//0/1:always match; 2:match if min<=color<=max;
                                    //3:match if color>max or color<min
} disp_colorkey;

typedef struct disp_s_frame
{
    disp_pixel_format format;
    disp_rectsz size[3];
    disp_rect crop;
...
typedef struct disp_capture_info
{
 ...
 */
typedef struct disp_device_config {
    disp_output_type       type;
    disp_tv_mode           mode;
    disp_csc_type          format;
    disp_data_bits         bits;
    disp_eotf              eotf;
    disp_color_space       cs;
    disp_dvi_hdmi          dvi_hdmi;
    disp_color_range       range;
    disp_scan_info         scan;
    unsigned int           aspect_ratio;
    unsigned int           reserve1;
} disp_device_config;

typedef struct disp_video_timings
{
...
typedef struct disp_fb_create_info
{
...
typedef enum {
    ...
    DISP_INIT_MODE_TWO_DIFF_SCREEN_SAME_CONTENTS = 5,//fb0 for two different screen
                                                     //(screen0 layer is normal layer,
                                                     //screen1 layer is scaler layer);
} disp_init_mode;

typedef struct disp_hdmi_func
{
    int (*hdmi_open)(void);
    int (*hdmi_close)(void);
 ...
 */
typedef struct disp_vdevice_interface_para
{
    unsigned int intf;
...
typedef struct disp_vdevice_init_data
{
    char name[32];
    u32 disp;
    u32 fix_timing;
...
typedef enum disp_tv_dac_source
{
