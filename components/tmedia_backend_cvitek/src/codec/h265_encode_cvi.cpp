...
#include <tmedia_backend_cvitek/cvi_encode/h265_encode_cvi.h>
#include "tmedia_backend_cvitek/cvi_util/util_cvi.h"

TMH265EncoderCVI::TMH265EncoderCVI()
{
...
TMSrcPad *TMH265EncoderCVI::GetSrcPad(int padID) 
{
...
TMSinkPad *TMH265EncoderCVI::GetSinkPad(int padID)
{
...
int TMH265EncoderCVI::Open(TMCodecParams &codecParam, TMPropertyList *propList) 
{
    VENC_CHN VeChn = -1;
    ...
    if(mFirstCreateFlag == 1) {
        mDefaultPropertyList.Get(TMH265Encoder::PropID::OUTPUT_GOP_NUMBER, &GopNum);
        mDefaultPropertyList.Get(TMH265Encoder::PropID::OUTPUT_TARGET_BITRATE, &TargetBitrate);
        mDefaultPropertyList.Get(TMH265Encoder::PropID::OUTPUT_FPS, &FPS);
    } else {
        mCurrentPropertyList.Get(TMH265Encoder::PropID::OUTPUT_GOP_NUMBER, &GopNum);
        mCurrentPropertyList.Get(TMH265Encoder::PropID::OUTPUT_TARGET_BITRATE, &TargetBitrate);
        mCurrentPropertyList.Get(TMH265Encoder::PropID::OUTPUT_FPS, &FPS);
    }
    
    if (propList != NULL)
    {
        propList->Get(TMH265Encoder::PropID::OUTPUT_GOP_NUMBER, &GopNum);
        propList->Get(TMH265Encoder::PropID::OUTPUT_TARGET_BITRATE, &TargetBitrate);
        propList->Get(TMH265Encoder::PropID::OUTPUT_FPS, &FPS);
    }
    mCurrentPropertyList.Assign(TMH265Encoder::PropID::OUTPUT_GOP_NUMBER, GopNum);
    mCurrentPropertyList.Assign(TMH265Encoder::PropID::OUTPUT_TARGET_BITRATE, TargetBitrate);
    mCurrentPropertyList.Assign(TMH265Encoder::PropID::OUTPUT_FPS, FPS);
    TMEDIA_PRINTF("Encode Open, codec id:%d\n", (int)mCodecID);
    TMEDIA_PRINTF("Encode: Width:%d Height:%d GOP:%d Target Bitrate:%dkb/s FPS:%d\n", codec_param->mWidth, codec_param->mHeight, GopNum, TargetBitrate, FPS);
    ...
    VencChnAttr.stVencAttr.enType = PT_H265;
    VencChnAttr.stRcAttr.enRcMode = VENC_RC_MODE_H265CBR;
    VencChnAttr.stRcAttr.stH265Cbr.bVariFpsEn = 0;
    VencChnAttr.stRcAttr.stH265Cbr.u32SrcFrameRate = 30;
    VencChnAttr.stRcAttr.stH265Cbr.fr32DstFrameRate = FPS;
    VencChnAttr.stRcAttr.stH265Cbr.u32BitRate = TargetBitrate;
    VencChnAttr.stRcAttr.stH265Cbr.u32Gop = GopNum;
    VencChnAttr.stRcAttr.stH265Cbr.u32StatTime = 2;
    ...
    UTIL_CVI_CHECK_RET_WITH_VALUE(CVI_VENC_GetH265Trans(VeChn, &stH265Trans));
    stH265Trans.cb_qp_offset = 0;
    stH265Trans.cr_qp_offset = 0;
    UTIL_CVI_CHECK_RET_WITH_VALUE(CVI_VENC_SetH265Trans(VeChn, &stH265Trans));

    VENC_H265_VUI_S stH265Vui = {0};
    UTIL_CVI_CHECK_RET_WITH_VALUE(CVI_VENC_GetH265Vui(VeChn, &stH265Vui));
    ...
    UTIL_CVI_CHECK_RET_WITH_VALUE(CVI_VENC_SetH265Vui(VeChn, &stH265Vui));
...
int TMH265EncoderCVI::SetConfig(TMPropertyList &propertyList) 
{
    int ret;
    ...
    if (propertyList.Get(TMH265Encoder::PropID::OUTPUT_TARGET_BITRATE, &mTargetBitrate) == 0)
    {
        ...
        mCurrentPropertyList.Assign(TMH265Encoder::PropID::OUTPUT_TARGET_BITRATE, mTargetBitrate);
    }
    return TMResult::TM_OK;
}

int TMH265EncoderCVI::GetConfig(TMPropertyList &propertyList) 
{
    propertyList.Reset();
...
int TMH265EncoderCVI::Flush() 
{
    return TMResult::TM_OK;
}

int TMH265EncoderCVI::Stop() 
{
    int ret;
    pthread_mutex_lock(&mEncodeMutex);
...
int TMH265EncoderCVI::SendFrame(TMVideoFrame &frame, int timeout)
{
    int ret;
...
int TMH265EncoderCVI::RecvPacket(TMVideoPacket &pkt, int timeout) 
{
...
REGISTER_VIDEO_ENCODER_CLASS(TMMediaInfo::CodecID::H265, TMH265EncoderCVI)