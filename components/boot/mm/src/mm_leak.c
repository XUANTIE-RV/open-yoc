...
static void traverse_one_list(struct m_dbg_hdr *cur, scan_cb cb, void *cookie)
{
    ...
    while (cur) {
        if (!is_valid_address(cur)) {
            cur = prev;
            printf("!!!already corrupted after, stop traversaling!!!ptr=%lx size=%d pid=%d caller=%lx leak=%c\n",
                   (unsigned long)(cur + 1), cur->size, cur->pid, (unsigned long)cur->caller, cur->referenced ? 'N' : 'Y');
            break;
        }
...
{
    (void)unused;
    printf("ptr=%lx size=%d pid=%d caller=%lx\n",
           (unsigned long)cur + 1, cur->size, cur->pid, (unsigned long)cur->caller);
...
{
    printf("caller=[<%lx>] ptr=%lx size=%d\t pid=%d leak=%c corrupted=%c\n",
           (unsigned long)cur->caller, (unsigned long)(cur + 1), cur->size, cur->pid,
...
{
    void **p = (void **)((unsigned long)start & ~3);
...
void mm_leak_search_chunk(void *p)
{
    ...
    if (caddr) {
        printf("%s before : addr=%lx caller=%lx\n", __func__, (unsigned long)caddr, (unsigned long)caddr->caller);
    }

    if (caddr_l != (void *) - 1UL) {
        printf("%s after : addr=%lx caller=%lx\n", __func__, (unsigned long)caddr_l, (unsigned long)caddr_l->caller);
    }
}
