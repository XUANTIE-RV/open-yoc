...
static        activation_info_t g_activation_info;

int activation_get_report_msg(char* report_msg, int len)
{
    uint8_t mac[16];
    char mac_format[18];
    const char *partner = g_activation_info.partner;
    const char *project = g_activation_info.project;
    const char *app_net = g_activation_info.app_net;
    ...
    if((strstr(SYSINFO_KERNEL_VERSION, SYSINFO_KERNEL_VERSION_PREFIX)) != NULL) {
        ...
        memcpy(version, kernel_version+strlen(SYSINFO_KERNEL_VERSION_PREFIX), SYSINFO_KERNEL_VERSION_POSTFIX_MAX_LEN);
...
{
    g_activation_info.os      = os;
    g_activation_info.partner = partner;
    g_activation_info.app_net = app_net;
    g_activation_info.type    = type;
    g_activation_info.project = project;
    g_activation_info.cloud   = cloud;
