...
{
    memset(ctx, 0, sizeof(sc_mbedtls_aes_context));
}
...
void sc_mbedtls_aes_free(sc_mbedtls_aes_context *ctx)
{
    ...
    sc_mbedtls_zeroize(ctx, sizeof(sc_mbedtls_aes_context));
}
                               ...
                               unsigned int keybits)
{
    ...
    switch (keybits) {
        ...
        case 256:
            ...
            break;
        default:
            return (SC_MBEDTLS_ERR_AES_INVALID_KEY_LENGTH);
    }
...
{
    int                     i, j, ret;
    sc_mbedtls_aes_context cty;
    uint32_t *              RK;
    uint32_t *              SK;

    sc_mbedtls_aes_init(&cty);

    ctx->rk = RK = ctx->buf;

    /* Also checks keybits */
    if ((ret = sc_mbedtls_aes_setkey_enc(&cty, key, keybits)) != 0)
        goto exit;
 ...
 */
int sc_mbedtls_internal_aes_encrypt(sc_mbedtls_aes_context *ctx, const unsigned char input[16],
 ...
 */
int sc_mbedtls_internal_aes_decrypt(sc_mbedtls_aes_context *ctx, const unsigned char input[16],
                                     unsigned char output[16])
{
    int       i;
    uint32_t *RK, X0, X1, X2, X3, Y0, Y1, Y2, Y3;
 ...
 */
int sc_mbedtls_aes_crypt_ecb(sc_mbedtls_aes_context *ctx, int mode, const unsigned char input[16],
                              unsigned char output[16])
{
    int ret;
    if (mode == MBEDTLS_AES_ENCRYPT) {
        ret = sc_mbedtls_internal_aes_encrypt(ctx, input, output);
    } else {
        ret = sc_mbedtls_internal_aes_decrypt(ctx, input, output);
    }
 ...
 */
int sc_mbedtls_aes_crypt_cbc(sc_mbedtls_aes_context *ctx, int mode, size_t length,
                              ...
                              unsigned char *output)
{
    ...
    if (length % 16)
        return (SC_MBEDTLS_ERR_AES_INVALID_INPUT_LENGTH);

    if (mode == MBEDTLS_AES_DECRYPT) {
        while (length > 0) {
            memcpy(temp, input, 16);
            sc_mbedtls_aes_crypt_ecb(ctx, mode, input, output);
        ...
        while (length > 0) {
            ...
            sc_mbedtls_aes_crypt_ecb(ctx, mode, output, output);
            memcpy(iv, output, 16);
 ...
 */
int sc_mbedtls_aes_crypt_cfb128(sc_mbedtls_aes_context *ctx, int mode, size_t length,
                                 ...
                                 unsigned char *output)
{
    ...
    if (mode == MBEDTLS_AES_DECRYPT) {
        while (length--) {
            if (n == 0)
                sc_mbedtls_aes_crypt_ecb(ctx, MBEDTLS_AES_ENCRYPT, iv, iv);
 ...
 */
int sc_mbedtls_aes_crypt_cfb8(sc_mbedtls_aes_context *ctx, int mode, size_t length,
                               unsigned char iv[16], const unsigned char *input,
                               unsigned char *output)
{
    unsigned char c;
