...
{
    memset( ctx, 0, sizeof( sc_mbedtls_sha1_context ) );
}
...
void sc_mbedtls_sha1_free( sc_mbedtls_sha1_context *ctx )
{
    ...
    sc_mbedtls_zeroize( ctx, sizeof( sc_mbedtls_sha1_context ) );
}
...
void sc_mbedtls_sha1_update( sc_mbedtls_sha1_context *ctx, const unsigned char *input, size_t ilen )
{
    ...
    if( left && ilen >= fill )
    {
        ...
        sc_mbedtls_sha1_process( ctx, ctx->buffer );
        input += fill;
        ilen  -= fill;
        left = 0;
    }
    ...
    while( ilen >= 64 )
    {
        sc_mbedtls_sha1_process( ctx, input );
        input += 64;
        ilen  -= 64;
    }
...
void sc_mbedtls_sha1_finish( sc_mbedtls_sha1_context *ctx, unsigned char output[20] )
{
    ...
    sc_mbedtls_sha1_update( ctx, sha1_padding, padn );
    sc_mbedtls_sha1_update( ctx, msglen, 8 );
...
{
    sc_mbedtls_sha1_context ctx;

    sc_mbedtls_sha1_init( &ctx );
    sc_mbedtls_sha1_starts( &ctx );
    sc_mbedtls_sha1_update( &ctx, input, ilen );
    sc_mbedtls_sha1_finish( &ctx, output );
    sc_mbedtls_sha1_free( &ctx );
}
