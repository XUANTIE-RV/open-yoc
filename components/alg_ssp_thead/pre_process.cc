...
bool PreProcess::Process(const std::vector<cx::BufferPtr>& data_vec) {
  ...
  int16_t* data_out = (int16_t *)oMemory->data();
    ...
    interleaved_buf_ = (int16_t *)malloc(iMeta->frame() * (iMeta->format() / 8) * iMeta->chn_num());
    /* data interleaved check */
    for (int j = 0; j < iMeta->frame(); j++) {
        for (int i = 0; i < iMeta->chn_num(); i++) {
            interleaved_buf_[iMeta->frame() * i + j] = data_in[iMeta->chn_num() * j + i];
        }
    }
#else
    interleaved_buf_ = data_in;
#endif


  // ssp and vad process
  //RUN_TIME_CHECK(500);
  int ret = T_Head_audio_process(interleaved_buf_, interleaved_buf_ + iMeta->frame() * (iMeta->chn_num() - 1), data_out);
  //RUN_TIME_CHECK(500);
