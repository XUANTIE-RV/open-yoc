...
typedef struct {
#ifdef CONFIG_LPM
    uint8_t sha_power_status;
    uint32_t sha_regs_saved[23];
#endif
    uint32_t base;
    uint32_t irq;
    sha_event_cb_t cb;
...
extern int32_t drv_clock_manager_config(clock_manager_e device, uint8_t enable);
...
#ifdef CONFIG_LPM
static void manage_clock(sha_handle_t handle, uint8_t enable)
{
    drv_clock_manager_config(CLOCK_MANAGER_SHA, enable);
}

static void do_prepare_sleep_action(sha_handle_t handle)
{
    ck_sha_priv_t *sha_priv = handle;
    uint32_t *sbase = (uint32_t *)(sha_priv->base);
    registers_save(&sha_priv->sha_regs_saved[1], sbase + 3, 22);
    registers_save(sha_priv->sha_regs_saved, sbase, 1);
}

static void do_wakeup_sleep_action(sha_handle_t handle)
{
    ck_sha_priv_t *sha_priv = handle;
    uint32_t *sbase = (uint32_t *)(sha_priv->base);
    registers_save(sbase, sha_priv->sha_regs_saved, 1);
    registers_save(sbase + 3, &sha_priv->sha_regs_saved[1], 22);
}
#endif
...
*/
__WEAK sha_handle_t csi_sha_initialize(int32_t idx, void *context, sha_event_cb_t cb_event)
{
    (void)idx;
...
#ifdef CONFIG_LPM
    csi_sha_power_control(sha_priv, DRV_POWER_FULL);
#endif
...
/**
  ...
  \return      error code
*/
__WEAK int32_t csi_sha_uninitialize(sha_handle_t handle)
{
...
#ifdef CONFIG_LPM
    csi_sha_power_control(sha_priv, DRV_POWER_OFF);
#endif
    return 0;
}
...
/**
  ...
  \return      error code
*/
__WEAK int32_t csi_sha_power_control(sha_handle_t handle, csi_power_stat_e state)
{
    SHA_NULL_PARAM_CHK(handle);
...
*/
__WEAK sha_capabilities_t csi_sha_get_capabilities(int32_t idx)
{
    if (idx < 0 || idx >= CONFIG_SHA_NUM) {
...
/**
  ...
  \return      error code
*/
__WEAK int32_t csi_sha_config(sha_handle_t handle, sha_mode_e mode, sha_endian_mode_e endian_mode)
{
    SHA_NULL_PARAM_CHK(handle);
...
/**
  ...
  \return      error code
*/
__WEAK int32_t csi_sha_start(sha_handle_t handle, void *context)
{
    SHA_NULL_PARAM_CHK(handle);
...
/**
  ...
  \return      error code
*/
__WEAK int32_t csi_sha_update(sha_handle_t handle, void *context, const void *input, uint32_t len)
{
    SHA_NULL_PARAM_CHK(handle);
...
/**
  ...
  \return      error code
*/
__WEAK int32_t csi_sha_finish(sha_handle_t handle, void *context, void *output)
{
    SHA_NULL_PARAM_CHK(handle);
...
*/
__WEAK sha_status_t csi_sha_get_status(sha_handle_t handle)
{
    if (handle == NULL) {
        sha_status_t ret;
