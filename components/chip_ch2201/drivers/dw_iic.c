...
static inline void dw_iic_set_transfer_speed(dw_iic_reg_t *addr, DWENUM_IIC_SPEED speed)
{
    ...
    if (speed == DW_IIC_STANDARDSPEED) {
        uint32_t ic_clk = drv_get_apb_freq(0);
        ic_clk = ic_clk / 1000000;
        addr->IC_SS_SCL_HCNT = (5U * ic_clk) - 8U;
        addr->IC_SS_SCL_LCNT = (5U * ic_clk) - 1U;
    }

    if (speed == DW_IIC_FASTSPEED) {
        uint32_t ic_clk = drv_get_apb_freq(0);
        ic_clk = ic_clk / 1000000;
        addr->IC_FS_SCL_HCNT = (125U * ic_clk) / 100U - 8U;
        addr->IC_FS_SCL_LCNT = (125U * ic_clk) / 100U - 1U;
    }
}
