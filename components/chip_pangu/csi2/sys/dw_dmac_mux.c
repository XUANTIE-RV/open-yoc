...
#define DMA1_FUNC_START_IDX DMA_HF_CODEC_ADC_0
...
int32_t dma_mux_channel_alloc(dw_dma_hf_t func)
{
    if (func < 0 || func > DMA_HF_CODEC_ADC_3) {
        return -1;
    }

    if (func < DMA_HF_UART1_RX) {
...
int32_t dma_mux_channel_release(dw_dma_hf_t func)
{
    if (func < 0 || func > DMA_HF_CODEC_ADC_3) {
        return -1;
    }
