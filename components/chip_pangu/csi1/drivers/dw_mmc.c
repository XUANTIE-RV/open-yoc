...
void dw_sdio_irqhandler(int32_t idx)
{
    assert(s_sdifState[idx]);
...
status_t SDIF_SendCommand(SDIF_TYPE *base, sdif_command_t *cmd, uint32_t timeout)
{
    ...
    //LOG_D("index %d cmd->argument 0x%x cmd 0x%x\n", cmd->index, cmd->argument, SDIF_CMD_CMD_INDEX(cmd->index) | SDIF_CMD_START_CMD_MASK | (cmd->flags & (~SDIF_CMD_CMD_INDEX_MASK)));
...
uint32_t SDIF_SetCardClock(SDIF_TYPE *base, uint32_t srcClock_Hz, uint32_t target_HZ)
{
    ...
    uint32_t divider = 1U, targetFreq = target_HZ;
    ...
    if (targetFreq != srcClock_Hz) {
        divider = ((srcClock_Hz + (srcClock_Hz % targetFreq)) / targetFreq);
    }

#if defined CONFIG_BOARD_TX216_CPU0_FPGA || defined CONFIG_BOARD_TX216_CPU1_FPGA

    /* divider can not be 0 because of sampling */
    /* when divider is 0, the speed of ddr can not support the speed of eMMC & SD card in tx216 FPGA */
    if (divider == 0) {
        divider = 1;
    }
   ...
   /* load the clock divider */
    ...
    return (divider != 0U) ? (srcClock_Hz / divider) : srcClock_Hz;
}
...
void SDIF_Init(SDIF_TYPE *base, sdif_config_t *config)
{
    ...
    SDIF_Clock_Delay(base, 1, 0, 1, 0);

#ifdef DW_SDIO_NOSUPPORT_DMA
    /* enable Interrupt from SDIO card */
    SDIF_EnableInterrupt(base, 0x10000);
#else
...
status_t SDIF_TransferBlocking(SDIF_TYPE *base, sdif_dma_config_t *dmaConfig, sdif_transfer_t *transfer)
{
    ...
    int timeout = 0x4ffffff; /* Pangu 166MHz 5 seconds */

    /* FIXME: maybe not idle here */
    while (timeout--) {
        if (base->IDSTS >> SDIF_IDSTS_FSM_SHIFT == 0) {
            break;
        }
    }
    ...
    if (SDIF_WaitCommandDone(base, transfer->command) != kStatus_Success) {
        LOG_E("SDIF_WaitCommandDone error\n");
        return kStatus_SDIF_SendCmdFail;
    }
                                  ...
                                  sdif_transfer_t *transfer)
{
    ...
    if ((data != NULL) && (dmaConfig != NULL)) {
        /* use internal DMA mode to transfer between the card and host*/
        if ((error = SDIF_InternalDMAConfig(base, dmaConfig, data->rx_date_buffer ? 
                                            data->rx_date_buffer : data->tx_data_buffer,
                                            data->block_size * data->block_count)) ==
            kStatus_SDIF_DescriptorBufferLenError) {
            return kStatus_SDIF_DescriptorBufferLenError;
        }
...
{
    drv_irq_register(id, handle);
    drv_irq_enable(id);
}

void SDIF_IRQDisable(int id, void *handle)
{
    printf("sdif irq disbale\n");
    drv_irq_disable(id);
    drv_irq_unregister(id);
}
...
{
    uint32_t base_n = 0u;
    int32_t ret = target_sdio_init(idx, &base_n, NULL, NULL);

    if (ret == -1) {
        return NULL;
    }

    s_sdifBase[idx]     = (SDIF_TYPE *)base_n;
...
sdif_handle_t drv_sdif_initialize(uint32_t idx, sdif_callback_t *callback, void *user_data)
{
    ...
    uint32_t irq = 0u;
    void *handler;
    int32_t ret = target_sdio_init(idx, &base_n, &irq, &handler);
...
uint32_t drv_sdif_set_clock(sdif_handle_t handle, uint32_t target_hz)
{
    ...
    if (base == (SDIF_TYPE*)CSKY_SDIO0_BASE) {
        idx = 0;
    } else if (base == (SDIF_TYPE*)CSKY_SDIO1_BASE) {
        idx = 1;
    } else {
        return kStatus_InvalidArgument;
    }

    drv_set_sdio_freq(idx, 50000000);

    source_clock_hz = drv_get_sdio_freq(idx);
