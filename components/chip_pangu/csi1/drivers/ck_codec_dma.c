...
extern void ck_codec_gasket_error_irqhandler(int32_t idx);
...
uint32_t g_ad_timer_period = 8000; //US-30ms
...
static int32_t ck_codec_input_timer_init()
{
    *((volatile uint32_t *)(0x39800008)) = 0;
    ...
    ret += csi_timer_set_timeout(ad_timer_handle, g_ad_timer_period);
...
void ck_codec_dma_ch_release(int ch)
{
    ...
    DMAC1_CFG(DMA_REG_LLPX + (ch * 0x58)) = 0;
#if defined(CONFIG_CHIP_PANGU)
