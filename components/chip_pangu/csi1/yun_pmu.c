...
void drv_reset_usart(int32_t idx)
{
    if (idx == 0) {
        YUN_CPR0->CPR0_UART0_CTL &= ~(CPR0_APB0SWRST_UART0_RST_EN);
        YUN_CPR0->CPR0_UART0_CTL |= (CPR0_APB0SWRST_UART0_RST_EN);
    } else if (idx == 1) {
        YUN_CPR0->CPR0_UART1_CTL &= ~(CPR0_APB0SWRST_UART1_RST_EN);
        YUN_CPR0->CPR0_UART1_CTL |= (CPR0_APB0SWRST_UART1_RST_EN);
    } else if (idx == 2) {
        YUN_CPR1->CPR1_APB1SWRST &= ~(CPR1_APB1SWRST_UART2_RST_EN);
        YUN_CPR1->CPR1_APB1SWRST |= (CPR1_APB1SWRST_UART2_RST_EN);
    } else if (idx == 3) {
        YUN_CPR1->CPR1_APB1SWRST &= ~(CPR1_APB1SWRST_UART3_RST_EN);
        YUN_CPR1->CPR1_APB1SWRST |= (CPR1_APB1SWRST_UART3_RST_EN);
    }
}
...
int drv_set_sdio_freq(int idx, int freq)
{
    ...
    uint32_t div = get_pll_freq() / 2 / freq;
    if (div > 8) {
        return -1;
    }

    /* sdio clk shoud not over 50M */
    if ((get_pll_freq() / 2 / div > 50000000) && div < 8) {
        div += 1;
    }
