...
extern int32_t target_get_sha(int32_t idx, uint32_t *base, uint32_t *irq, void **handler);
...
static int32_t sha_input_data(uint32_t *data, uint32_t length)
{
    uint8_t i;
    uint32_t tmp;
    uint32_t *input_data = (uint32_t *)(((uint32_t) & (sha_reg->SHA_DATA1)) & ~(0x3));
...
sha_handle_t csi_sha_initialize(int32_t idx, void *context, sha_event_cb_t cb_event)
{
    ...
    target_get_sha(0, &base, &irq, &handler);
    ...
    void *handler;
    /* obtain the sha information */
    int32_t real_idx = target_get_sha(idx, &base, &irq, &handler);
...
#ifndef CONFIG_SHA_BLOCK_MODE
    drv_irq_enable(sha_priv->irq);
    drv_irq_register(sha_priv->irq, handler);
#endif
...
int32_t csi_sha_uninitialize(sha_handle_t handle)
{
    ...
    drv_irq_disable(sha_priv->irq);
    drv_irq_unregister(sha_priv->irq);
#endif
#endif
