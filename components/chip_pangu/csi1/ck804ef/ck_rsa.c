...
#define RSA_SP_HOLE1_DOWN    0x100
#define RSA_SP_HOLE1_UP      0x200
#define RSA_SP_HOLE2_DOWN    0x900
#define RSA_SP_HOLE2_UP      0xa00
#define TEE_CONEXT_SIZE      32
#define IS_SP_IN_HOLE1_AREA(addr) \
    (addr >= RSA_SP_HOLE1_DOWN && addr < (RSA_SP_HOLE1_UP + TEE_CONEXT_SIZE))

#define IS_SP_IN_HOLE2_AREA(addr) \
    (addr >= RSA_SP_HOLE2_DOWN && addr < (RSA_SP_HOLE2_UP + TEE_CONEXT_SIZE))
static uint32_t rsa_run_adjust_sp_if_need(uint32_t sp)
{
    uint32_t sp_lower = (sp & ((1 << 12) - 1));

    // if current sp is in hole, adjust new sp
    if (IS_SP_IN_HOLE1_AREA(sp_lower) || IS_SP_IN_HOLE2_AREA(sp_lower)) {
        return 1;
    } else {
        return 0;
    }
...
static void rsa_calq_by_assemble(void)
{
...
/**
  ...
  \return      error code
*/
__WEAK int32_t csi_rsa_encrypt(rsa_handle_t handle, void *n, void *e, void *src, uint32_t src_size, void *out, uint32_t *out_size, rsa_padding_t padding)
{
    RSA_NULL_PARAM_CHK(handle);
...
/**
  ...
  \return      error code
*/
__WEAK int32_t csi_rsa_decrypt(rsa_handle_t handle, void *n, void *d, void *src, uint32_t src_size, void *out, uint32_t *out_size, rsa_padding_t padding)
{
    RSA_NULL_PARAM_CHK(handle);
    RSA_NULL_PARAM_CHK(n);
...
/**
  ...
  \return      error code
*/
__WEAK int32_t csi_rsa_sign(rsa_handle_t handle, void *n, void *d, void *src, uint32_t src_size, void *signature, uint32_t *sig_size, rsa_padding_t padding)
{
    RSA_NULL_PARAM_CHK(handle);
    RSA_NULL_PARAM_CHK(n);
    RSA_NULL_PARAM_CHK(d);
...
/**
  ...
  \return      error code
*/
__WEAK int32_t csi_rsa_verify(rsa_handle_t handle, void *n, void *e, void *src, uint32_t src_size, void *signature, uint32_t sig_size, void *result, rsa_padding_t padding)
{
    RSA_NULL_PARAM_CHK(handle);
    RSA_NULL_PARAM_CHK(n);
    RSA_NULL_PARAM_CHK(e);
