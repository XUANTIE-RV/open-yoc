...
#define aos_list_entry list_entry
...
static aos_task_key_t g_cli_key = -1;
...
{
    return (cli_console *) aos_task_getspecific(g_cli_key);
}
...
{
    return aos_task_setspecific(g_cli_key, NULL);
}


int cli_task_set_console(void *console)
{
    int ret;
    if (g_cli_key == -1) {
        aos_task_key_create(&g_cli_key);
    }

    if (mul_console_init_flag == 1) {
        cli_console_shared_sem_take(&console_sem);
        ret = aos_task_setspecific(g_cli_key, console);
        cli_console_shared_sem_post(&console_sem);
    } else {
        ret = aos_task_setspecific(g_cli_key, console);
    }
