...
static u16_t last_static_handle = 0;
			      ...
			      const void *pdu, u16_t length, void *user_data)
{
	...
	params->func(conn, params->attr, err, (void*)params);
}
...
__attribute__((weak)) void ble_stack_gatt_notify_cb(int16_t conn_handle,
			  ...
			  const void *data, u16_t length)
{
	...
	ble_stack_gatt_notify_cb(bt_conn_index(conn), handle, data, length);
		   ...
		   void *cb_arg)
{
	if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
		...
		BT_DBG("Restored CCC for id:%d addr:%s",
