...
#define BT_AVRCP_MAX_METADETA_ATTR_SIZE 7
...
/** @brief metadata attribute mask */
typedef enum {
    BT_PRF_AVRCP_METADATA_ATTR_TITLE = 0x1,                 /*!< title of the playing track */
    BT_PRF_AVRCP_METADATA_ATTR_ARTIST = 0x2,                /*!< track artist */
    BT_PRF_AVRCP_METADATA_ATTR_ALBUM = 0x4,                 /*!< album name */
    BT_PRF_AVRCP_METADATA_ATTR_TRACK_NUM = 0x8,             /*!< track position on the album */
    BT_PRF_AVRCP_METADATA_ATTR_NUM_TRACKS = 0x10,           /*!< number of tracks on the album */
    BT_PRF_AVRCP_METADATA_ATTR_GENRE = 0x20,                /*!< track genre */
    BT_PRF_AVRCP_METADATA_ATTR_PLAYING_TIME = 0x40          /*!< total album playing time in miliseconds */
} bt_prf_avrcp_md_attr_mask_t;
 ...
 *             Will response with BT_PRF_AVRCP_CT_GET_RN_CAPABILITIES_RSP_EVT event.
 * @return     bt_stack_status_t  BT_STACK_STATUS_SUCCESS: success
 *                                other: failed
 */
 ...
 *             The result will response with BT_PRF_AVRCP_CT_PASSTHROUGH_RSP_EVT event.
 * @param[in]  operation_id         passthrough command code \ref bt_prf_avrcp_op_id_t
 * @param[in]  operation_state      passthrough command key state \ref bt_prf_avrcp_op_stat_t
 * @return     bt_stack_status_t    BT_STACK_STATUS_SUCCESS: success
 *                                  other: failed
 */
 ...
 *             The result will response with BT_PRF_AVRCP_CT_SET_ABSOLUTE_VOLUME_RSP_EVT event.
...
/**
 * @brief      Send get metadata command to AVRCP target
 *             The result will response with BT_PRF_AVRCP_CT_METADATA_RSP_EVT event.
 * @param[in]  attr_bit_map       bitmap \ref bt_prf_avrcp_md_attr_mask_t
 * @return     bt_stack_status_t  BT_STACK_STATUS_SUCCESS: success
 *                                other: failed
 */
...
/**
 * @brief      AVRCP target connect to controller.
...
/**
 * @brief      AVRCP target disconnect to controller.
 * @param[in]  peer_addr          bluetooth device address of peer device
 * @return     bt_stack_status_t  BT_STACK_STATUS_SUCCESS: success
 *                                other: failed
 *
 ...
 *                  Should Call this API in BT_PRF_AVRCP_TG_REGISTER_NOTIFICATION_EVT event report
 *                  with BT_AVRCP_RESPONSE_INTERIM response and should response with BT_AVRCP_RESPONSE_CHANGED_STABLE after status changed
