...
#define SETTINGS_HANDLER_DEFINE(_hname, _tree, _get, _set, _commit,   \
				       _export)				     \
	static struct settings_handler settings_handler_##_hname = {     \
		.name = _tree,						     \
		.h_get = _get,						     \
		.h_set = _set,						     \
		.h_commit = _commit,					     \
	...
	settings_register(& settings_handler_ ## _hname);
