...
typedef struct regulatory_cert_data_list_t {
    uint8_t *list;    /* Pointer the byte array containing the encoded opaque structure based on IEEE 11073-20601
                         specification. */
...
/// deprecated fucntion dis_init, use ble_prf_dis_init instead
static inline __bt_deprecated dis_handle_t dis_init(dis_info_t *info)
{
    return ble_prf_dis_init(info);
}
