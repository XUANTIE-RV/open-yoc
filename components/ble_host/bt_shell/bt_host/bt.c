...
static int cmd_phy_rate_set(int argc, char *argv[])
{
    ...
    if(tx_phy_rate != PREF_PHY_1M && tx_phy_rate != PREF_PHY_2M) {
        return -EINVAL;
    }

    rx_phy_rate = strtoul(argv[2], NULL, 16);
    if(rx_phy_rate != PREF_PHY_1M && rx_phy_rate != PREF_PHY_2M) {
        return -EINVAL;
    }
...
#endif
    { "phy-rate-set", cmd_phy_rate_set, "<tx rate:1M:0x01/2M:0x02> <rx rate:1M:0x01/2M:0x02>" },
