...
static uint8_t                       yoc_app_bt_hfp_connect_flag;
static uint8_t                       yoc_app_bt_hfp_disconnect_flag;
static YOC_APP_BT_HFP_IMPL_CB_FUNC_T g_hfp_callback;
static yoc_app_bt_hfp_call_status_t  g_hfp_call_status = CALL_NONE;

static aos_sem_t yoc_app_bt_hfp_disconnect_sem;
...
// yoc_hf_client_in_band_ring_state_t
...
static void bt_app_hf_client_cb(bt_prf_hfp_hf_cb_event_t event, bt_prf_hfp_hf_cb_param_t *param)
{
    ...
    case BT_PRF_HFP_HF_CONNECTION_STATE_EVT: {
        ...
        char bdAddr[YOC_APP_BDADDR_MAX_LEN];
        ...
        if (param->conn_stat.state == BT_PRF_HFP_HF_CONNECTION_STATE_CONNECTED ||
            ...
            yoc_app_bt_hfp_connect_flag = 1;
        } else {
            yoc_app_bt_hfp_connect_flag = 0;
        }
        ...
        if (yoc_app_bt_hfp_disconnect_flag == 1 &&
            param->conn_stat.state == BT_PRF_HFP_HF_CONNECTION_STATE_DISCONNECTED) {
            aos_sem_signal(&yoc_app_bt_hfp_disconnect_sem);
            yoc_app_bt_hfp_disconnect_flag = 0;
        }
...
int32_t yoc_app_bt_hfp_disconnect(const char *mac)
{
    ...
    yoc_app_bt_hfp_disconnect_flag = 1;

    int ret = bt_prf_hfp_hf_disconnect(&addr);

    if (ret == BT_STACK_STATUS_SUCCESS) {
        aos_sem_wait(&yoc_app_bt_hfp_disconnect_sem, -1);
    }
    return 0;
}
...
int32_t yoc_app_bt_hfp_send_command(YOC_APP_BT_HFP_COMMAND_T command)
{
    switch (command) {
        /* Three-way calling */
    case YOC_APP_BT_HFP_COMMAND_TYPE_REJECT_INCOMING:
        bt_prf_hfp_hf_send_chld_cmd(BT_PRF_HFP_HF_CHLD_TYPE_REL, 0);
        break;
    case YOC_APP_BT_HFP_COMMAND_TYPE_TERMINATE_ACTIVE_ACCEPT_INCOMING:
        bt_prf_hfp_hf_send_chld_cmd(BT_PRF_HFP_HF_CHLD_TYPE_REL_ACC, 0);
        break;
    case YOC_APP_BT_HFP_COMMAND_TYPE_HOLD_ACTIVE_ACCEPT_INCOMING:
        bt_prf_hfp_hf_send_chld_cmd(BT_PRF_HFP_HF_CHLD_TYPE_HOLD_ACC, 0);
        break;
        /* Two-way calling */
    case YOC_APP_BT_HFP_COMMAND_TYPE_ANSWER:
        if (g_hfp_call_status != CALL_IN_PROGRESS) {
            bt_prf_hfp_hf_answer_call();
        }
        break;
    case YOC_APP_BT_HFP_COMMAND_TYPE_TERMINATE:
...
static int32_t yoc_app_bt_hfp_init(void)
{
    aos_sem_new(&yoc_app_bt_hfp_disconnect_sem, 0);
...
static int bt_hfp_start(const char *url, uint64_t seek_time, int resume)
{
    return yoc_app_bt_hfp_send_command(YOC_APP_BT_HFP_COMMAND_TYPE_ANSWER);
}

static int bt_hfp_pause(void)
{
    ctrl_bt_hfp.status = SMTAUDIO_STATE_PAUSE;
    return yoc_app_bt_hfp_send_command(YOC_APP_BT_HFP_COMMAND_TYPE_HOLD_ACTIVE_ACCEPT_INCOMING);
}

static int bt_hfp_stop(void)
{
    ctrl_bt_hfp.status = SMTAUDIO_STATE_STOP;
    return yoc_app_bt_hfp_send_command(YOC_APP_BT_HFP_COMMAND_TYPE_TERMINATE);
}
...
static int bt_hfp_vol_set(int vol)
{
    yoc_app_bt_hfp_volume_update(BT_PRF_HFP_HF_VOLUME_CONTROL_TARGET_SPK, vol * 15 / 100);
    return 0;
}
