                      ...
                      const char *client_pwd, size_t client_pwd_len)
{
   ...
   // SSL_LOG("ok");
    ...
    mbedtls_ssl_set_bio(&(pTlsData->ssl), &(pTlsData->fd), _net_send, _net_recv, _net_recv_timeout);

    /*
     * 1. Start the connection
     */
    SSL_LOG("Conn /%s/%s", addr, port);
    if (0 != (ret = _net_connect(&(pTlsData->fd), addr, port, MBEDTLS_NET_PROTO_TCP))) {
...
static int _network_ssl_read(TLSDataParams_t *pTlsData, char *buffer, int len, uint32_t timeout_ms)
{
    uint32_t        readLen = 0;
...
static int _network_ssl_write(TLSDataParams_t *pTlsData, const char *buffer, int len, uint32_t timeout_ms)
{
...
int HAL_SSL_Read(_IN_ uintptr_t handle, _OU_ char *buf, _OU_ int len, _IN_ int timeout_ms)
{
    return _network_ssl_read((TLSDataParams_t *)handle, buf, len, timeout_ms);;
}

int HAL_SSL_Write(_IN_ uintptr_t handle, _IN_ const char *buf, _IN_ int len, _IN_ int timeout_ms)
{
