...
uintptr_t HAL_TCP_Establish(const char *host, uint16_t port)
{
    ...
    PLATFORM_LOG("establish tcp connection with server(host=%s port=%u)", host, port);
    ...
    if (0 == rc) {
        PLATFORM_LOG("fail to establish tcp");
    } else {
        PLATFORM_LOG("success to establish tcp, fd=%d", rc);
    }
...
int32_t HAL_TCP_Write(uintptr_t fd, const char *buf, uint32_t len, uint32_t timeout_ms)
{
        ...
        if (0 != t_left) {
            ...
            if (ret > 0) {
                if (0 == FD_ISSET(fd, &sets)) {
                    PLATFORM_LOG("Should NOT arrive");
                    /* If timeout in next loop, it will not sent any data */
                    ret = 0;
                    continue;
                }
            } else if (0 == ret) {
                PLATFORM_LOG("select-write timeout %d", (int)fd);
                break;
            } else {
                if (EINTR == errno) {
                    PLATFORM_LOG("EINTR be caught");
                    continue;
                }
    ...
    } while ((len_sent < len) && (_time_left(t_end, _get_time_ms()) > 0));
