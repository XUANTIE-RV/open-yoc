...
#include "ad_icore_cp.h"

#define TAG    "ad_icore_cp"

struct ad_icore_cp_priv {
...
{
    int rc = -1;
    ad_icore_reset_t *inp;

    inp = icore_get_msg(msg, ad_icore_reset_t);
    CHECK_RET_TAG_WITH_RET(inp, -1);
...
{
    int rc = -1;
    ad_icore_close_t *inp;

    inp = icore_get_msg(msg, ad_icore_close_t);
    CHECK_RET_TAG_WITH_RET(inp, -1);
...
{
    struct ad_icore_cp_priv *priv = arg;

    switch (msg->command) {
    case IPC_CMD_AD_ICORE: {
...
static void _ad_icore_cp_task(void *arg)
{
    int i;
    uint32_t len;
    message_t msg;
...
int ad_icore_cp_init()
{
    ...
    if (!inited) {
        struct ad_icore_cp_priv *priv;

        priv = aos_zalloc(sizeof(struct ad_icore_cp_priv));
        CHECK_RET_TAG_WITH_RET(priv, -1);
        aos_queue_new(&priv->queue, priv->qbuf, sizeof(message_t) * MESSAGE_NUM, sizeof(message_t));
        priv->ipc = ipc_get(AD_ICORE_AP_IDX);
        ipc_add_service(priv->ipc, AD_ICORE_IPC_SERIVCE_ID, _ipc_process, priv);
        aos_task_new("ad_cp_task", _ad_icore_cp_task, (void *)priv, 96*1024);
        ad_register_all();
        inited = 1;
    }
