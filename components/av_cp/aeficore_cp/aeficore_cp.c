...
#include "aeficore_cp.h"

#define TAG    "aeficore_cp"

struct aeficore_cp_priv {
...
static int _icore_aefx_new(icore_msg_t *msg)
{
    ...
    inp = icore_get_msg(msg, aeficore_new_t);
...
{
    struct aeficore_cp_priv *priv = arg;

    switch (msg->command) {
    case IPC_CMD_AEFICORE: {
...
static void _aeficore_cp_task(void *arg)
{
    int i;
    uint32_t len;
    message_t msg;
 ...
 * @return 0/-1
 */
int aeficore_cp_init()
{
    static int inited = 0;

    if (!inited) {
        struct aeficore_cp_priv *priv;

        priv = aos_zalloc(sizeof(struct aeficore_cp_priv));
        CHECK_RET_TAG_WITH_RET(priv, -1);
        aos_queue_new(&priv->queue, priv->qbuf, sizeof(message_t) * MESSAGE_NUM, sizeof(message_t));
        priv->ipc = ipc_get(AEFICORE_AP_IDX);
        ipc_add_service(priv->ipc, AEFICORE_IPC_SERIVCE_ID, _ipc_process, priv);
        aefx_register();
        aos_task_new("aef_cp_task", _aeficore_cp_task, (void *)priv, 4*1024);
        inited = 1;
    }
