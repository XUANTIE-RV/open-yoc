...
#include "eq_icore_cp.h"

#define TAG    "eq_icore_cp"

struct eq_icore_cp_priv {
...
static int _icore_eqx_new(icore_msg_t *msg)
{
    ...
    inp = icore_get_msg(msg, eq_icore_new_t);
...
{
    struct eq_icore_cp_priv *priv = arg;

    switch (msg->command) {
    case IPC_CMD_EQ_ICORE: {
...
static void _eq_icore_cp_task(void *arg)
{
    int i;
    uint32_t len;
    message_t msg;
...
int eq_icore_cp_init()
{
    ...
    if (!inited) {
        struct eq_icore_cp_priv *priv;

        priv = aos_zalloc(sizeof(struct eq_icore_cp_priv));
        CHECK_RET_TAG_WITH_RET(priv, -1);
        aos_queue_new(&priv->queue, priv->qbuf, sizeof(message_t) * MESSAGE_NUM, sizeof(message_t));
        priv->ipc = ipc_get(EQ_ICORE_AP_IDX);
        ipc_add_service(priv->ipc, EQ_ICORE_IPC_SERIVCE_ID, _ipc_process, priv);
        eqx_register();
        aos_task_new("eq_cp_task", _eq_icore_cp_task, (void *)priv, 4*1024);
        inited = 1;
    }
