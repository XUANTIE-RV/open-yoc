...
int pthread_mutex_init(pthread_mutex_t *p_mutex, const pthread_mutexattr_t *attr)
{
    ...
    if (attr != NULL) {
        p_mutex->attr = (pthread_mutexattr_t *)attr;
    }

    ret = krhino_mutex_dyn_create(K_MUTEX_PP(&p_mutex->mutex), "mutex");
...
int pthread_mutexattr_settype(pthread_mutexattr_t *attr, int type)
{
    ...
    /* kernel only support PTHREAD_MUTEX_ERRORCHECK mode */
    // FIXME: add || type == PTHREAD_MUTEX_RECURSIVE
