...
static void *demo_task10(void *arg)
{
    ...
    if (ret == 0) {
        if ((((ktask_t*)pthread_self())->prio == PRI_CONVERT_PX_RH(param.sched_priority))
            && (((ktask_t*)pthread_self())->time_slice == (param.slice * 1000) / RHINO_CONFIG_TICKS_PER_SECOND)) {
...
static void *demo_task12(void *arg)
{
    ...
    if (ret == 0) {
        if (((ktask_t*)pthread_self())->prio == PRI_CONVERT_PX_RH(10)) {
