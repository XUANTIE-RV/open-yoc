...
#define WJ_MEM_RET_CTRL_LP_V_SEL_Pos              (1U)
...
static inline void wj_set_bc_wakeup_source(wj_pmu_reg_t *pmu_base, uint32_t wakeupn)
{
    pmu_base->BC_WIME &= ~((uint32_t)1U << wakeupn);
}

static inline void wj_clear_bc_wakeup_source(wj_pmu_reg_t *pmu_base, uint32_t wakeupn)
{
    pmu_base->BC_WIME |= ((uint32_t)1U << wakeupn);
}
...
static inline void wj_set_main_pwr_vsel(wj_pmu_reg_t *pmu_base, uint32_t vreg)
{
    ...
    pmu_base->PMU_TRIM |= vreg << WJ_PMU_TRIM_MAIN_PWR_VSEL_Pos;
}
...
static inline void wj_set_rf_pwr_vsel(wj_pmu_reg_t *pmu_base, uint32_t vreg)
{
    ...
    pmu_base->PMU_TRIM |= vreg << WJ_PMU_TRIM_RF_PWR_VSEL_Pos;
}
