...
#define HEAD "iic"
#define log(format, ...) \
    do {printf("[%s]%s(%d):"format"",HEAD,__func__,__LINE__, \
        ##__VA_ARGS__);} while(0)
#else
#define log(fmt, args...)
#endif
...
typedef struct {
    ...
    __IOM uint32_t IC_SDA_HOLD;               /* Offset: 0x07C (R/W)  SDA hold time register */
    __IOM uint32_t IC_TX_ABRT_SOURCE;         /* Offset: 0x080 (R/W)  I2C Transmit Abort Status Register */
    __IOM uint32_t IC_SLV_DAT_NACK_ONLY;      /* Offset: 0x084 (R/W)  I2C Slave Address1 */
...
static inline void iic_dump_register(dw_iic_regs_t *iic_base)
{
    printf("[%s]: ===dump start\n", __func__);
    printf("IC_CON = %#x\n", iic_base->IC_CON);
    printf("IC_TAR = %#x\n", iic_base->IC_TAR);
    printf("IC_SS_SCL_HCNT = %#x\n", iic_base->IC_SS_SCL_HCNT);
    printf("IC_SS_SCL_LCNT = %#x\n", iic_base->IC_SS_SCL_LCNT);
    printf("IC_ENABLE = %#x\n", iic_base->IC_ENABLE);
    printf("IC_STATUS = %#x\n", iic_base->IC_STATUS);
    printf("IC_RAW_INTR_STAT = %#x\n", iic_base->IC_RAW_INTR_STAT);
    printf("[%s]: ===dump end\n", __func__);
}

extern void dw_iic_set_target_address(dw_iic_regs_t *iic_base, uint32_t address);

extern void dw_iic_set_reg_address(dw_iic_regs_t *iic_base,
...
extern int32_t dw_iic_xfer_finish(dw_iic_regs_t *iic_base);

extern int32_t dw_iic_xfer_init(dw_iic_regs_t *iic_base, uint32_t dev_addr,
