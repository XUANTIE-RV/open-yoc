...
#define ETHPHY_NULL_PARAM_CHK_RE_NULL(cvi_para) CSI_PARAM_CHK(cvi_para, NULL)
#define ETHPHY_NULL_PARAM_CHK(cvi_para) CSI_PARAM_CHK(cvi_para, -1)

#define pr_err(cvi_x, args...) do {printf("[%s][ERR]%s():(%d) - " cvi_x,\
                                HEAD, __func__, __LINE__, ##args);\
                                } while(0)
#ifdef CSI_DRV_DEBUG
#define pr_debug(cvi_x, args...) do {printf("[%s][DEBUG]%s():(%d) - " cvi_x,\
                                HEAD, __func__, __LINE__, ##args);\
                                } while(0)

#define pr_info(cvi_x, args...) do {printf("[%s][INFO]%s():(%d) - " cvi_x,\
                                HEAD, __func__, __LINE__, ##args);\
                                } while(0)

#define pr_notice(cvi_x, args...) do {printf("[%s][NOTICE]%s():(%d) - " cvi_x,\
                                HEAD, __func__, __LINE__, ##args);\
                                } while(0)

#define pr_warn(cvi_x, args...) do {printf("[%s][WARN]%s():(%d) - " cvi_x,\
                                HEAD, __func__, __LINE__, ##args);\
                                } while(0)
#else
#define pr_warn(cvi_x, args...)
#define pr_notice(cvi_x, args...)
#define pr_info(cvi_x, args...)
#define pr_debug(cvi_x, args...)
#endif
