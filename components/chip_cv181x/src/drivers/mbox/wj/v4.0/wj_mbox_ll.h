...
typedef struct {
    ...
    uint32_t RESERVED[1011];
} wj_mbox_ch_t;

typedef struct {
    __IOM wj_mbox_ch_t MBOX_CH[8];
} wj_mbox_cpu_t;
...
static inline uint32_t wj_read_cpu_ch_interrupt_state(wj_mbox_regs_t *mbox_base, uint32_t cpu_id, uint32_t ch)
{
    return mbox_base->MBOX_CPU[cpu_id].MBOX_CH[ch].INTR_STA;
}

static inline void wj_clear_cpu_ch_interrupt(wj_mbox_regs_t *mbox_base, uint32_t cpu_id, uint32_t ch)
{
    mbox_base->MBOX_CPU[cpu_id].MBOX_CH[ch].INTR_CLR = (uint32_t)((uint32_t)1U << 0);
}

static inline uint32_t wj_read_cpu_ch_raw_interrupt_state(wj_mbox_regs_t *mbox_base, uint32_t cpu_id, uint32_t ch)
{
    return mbox_base->MBOX_CPU[cpu_id].MBOX_CH[ch].INTR_RAW;
}

static inline uint32_t wj_read_cpu_ch_interrupt_mask(wj_mbox_regs_t *mbox_base, uint32_t cpu_id, uint32_t ch)
{
    return mbox_base->MBOX_CPU[cpu_id].MBOX_CH[ch].INTR_MASK;
}

static inline void wj_write_cpu_ch_interrupt_mask(wj_mbox_regs_t *mbox_base, uint32_t cpu_id, uint32_t ch, uint32_t state)
{
    mbox_base->MBOX_CPU[cpu_id].MBOX_CH[ch].INTR_MASK = state;
}
...
static inline uint32_t wj_read_ip_id(wj_mbox_regs_t *mbox_base, uint32_t cpu_id)
{
    //return mbox_base->MBOX_CPU[cpu_id].IP_ID;
    return 0;
}
