...
static void traverse_one_list(struct m_dbg_hdr *cur, scan_cb cb, void *cookie)
{
    ...
    while (cur) {
        if (!is_valid_address(cur)) {
            cur = prev;
            printf("!!!already corrupted after, stop traversaling!!!ptr=%lu size=%d pid=%d caller=%lu leak=%c\n",
...
{
    (void)unused;
    printf("ptr=%lu size=%d pid=%d caller=%lu\n",
...
static int show_corrupted_chunk(struct m_dbg_hdr *cur, void *unused)
{
    ...
    printf("caller=%lu ptr=%lu size=%d  pid=%d leak=%c corrupted=%c\n",
