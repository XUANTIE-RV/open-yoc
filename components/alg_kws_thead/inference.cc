        ...
        = participant_->CreateReader<AlgCmdMessageT>("AlgCmdMsg", [this](const std::shared_ptr<AlgCmdMessageT> &msg) {
              ...
              switch (msg->body().cmd()) {
                  case thead::voice::proto::PUSH2TALK_CMD:
                      push2talk_ = msg->body().push2talk_mode();
                      break;
...
bool Inference::Process(const std::vector<cx::BufferPtr> &data_vec)
{
    ...
    if (inited_flag_ == 0) {
        int ret  = thead_kws_init(iMeta->chn_num(), iMeta->sample_rate(), &frame_buf_len_, &temp_buf_len_);
        ...
        LOGD(TAG, "thead_kws_init. chn_num: %d, frame %d, temp:%p(%ld), ret %d\n", 
             iMeta->chn_num(), iMeta->frame(), temp_buf_, temp_buf_byte_len, ret);
        inited_flag_ = 1;
    }
    ...
    thead_kws_postprocess(temp_buf_, temp_buf_len_, output_data_, iMeta->chn_num());
