...
#else
    ssl->in_msg = ssl->handshake->hs_msg;
#endif
...
#define CK_FLASH_DEBUG
#ifdef CK_FLASH_DEBUG
#define CK_FLAHS_DBG printf
#else
#define CK_FLAHS_DBG(...)
#endif

#define CK_FLASH_DRV_NAME   "eflash"
#define CK_FLASH_POOL_MAGIC 0x95134681
#define CK_FLASH_BLOCK_ADDR(a) ((a) * ck_cert_flash_pool.block_size)
#define CK_FLAHS_NEXT_DATA(a) ((flash_data_t *)((unsigned int)(a) + (a)->h.blocks * ck_cert_flash_pool.block_size))
typedef struct {
    unsigned int magic;
    unsigned int used:   8 ;     /* 0xAA means used */
    unsigned int length: 16;
    unsigned int blocks: 8 ;   /* date is align by flash block */
} flash_data_head_t;
...
struct {
    ...
    unsigned int end;
    flash_data_t *free;
    flash_data_t *data;   /*pointer to data block*/
    unsigned int block_size;
    unsigned int block_num;
    unsigned int init_flag;
} static ck_cert_flash_pool = {0};

static flash_dev_obj_t * g_eflash_dev = NULL;
int ck_cert_flash_init()
{
    int i;
    flash_data_t *p;

    memset(&ck_cert_flash_pool, 0, sizeof(ck_cert_flash_pool));

    uint32_t base = 0u;
    eflash_info_t info;
    extern int32_t target_get_eflash(int32_t idx, uint32_t *base, eflash_info_t * info);
    target_get_eflash(0, &base, &info);
    g_eflash_dev = (flash_dev_obj_t*)yoc_dev_get_device(CK_FLASH_DRV_NAME);

    ck_cert_flash_pool.chip_start = info.start;
    ck_cert_flash_pool.block_size = flash_get_blksize(g_eflash_dev);
    ck_cert_flash_pool.start = CK_FLASH_BLOCK_ADDR(CK_FLASH_BLOCK_START) + ck_cert_flash_pool.chip_start;
    ck_cert_flash_pool.end = CK_FLASH_BLOCK_ADDR(CK_FLASH_BLOCK_START + CK_FLAHS_BLOCK_NUM) + ck_cert_flash_pool.chip_start;
    ck_cert_flash_pool.block_num = CK_FLAHS_BLOCK_NUM;
    ck_cert_flash_pool.data = (flash_data_t *)(ck_cert_flash_pool.start);

    CK_FLAHS_DBG("chip start %x,start %x,end %x,block size %d,block Num %d, data %x, free_ptr %x\n",
                 ck_cert_flash_pool.chip_start, ck_cert_flash_pool.start, ck_cert_flash_pool.end, ck_cert_flash_pool.block_size,
                 ck_cert_flash_pool.block_num, ck_cert_flash_pool.data);

    if (ck_cert_flash_pool.data->h.magic != CK_FLASH_POOL_MAGIC) {
        CK_FLAHS_DBG("erase\n");

        for (i = 0; i < ck_cert_flash_pool.block_num; i++) {
            CK_FLAHS_DBG("erase addr %x\n", ck_cert_flash_pool.start - ck_cert_flash_pool.chip_start + \
                         i * ck_cert_flash_pool.block_size);
            flash_erase(g_eflash_dev, ck_cert_flash_pool.start - ck_cert_flash_pool.chip_start + \
                             i * ck_cert_flash_pool.block_size);
        }
    }

    p = ck_cert_flash_pool.data;

    CK_FLAHS_DBG("p %x\n", p);

    while (p->h.magic == 0xAA) {
        p = CK_FLAHS_NEXT_DATA(p);
    }

    if (ck_cert_flash_pool.end <= (unsigned int)p) {
        CK_FLAHS_DBG("flash overflow p %x\n", p);
        return -1;
    }

    ck_cert_flash_pool.free = p;

    CK_FLAHS_DBG("chip start %x,start %x,end %x,block size %d,block Num %d, head %x, free_ptr %x\n",
                 ck_cert_flash_pool.chip_start, ck_cert_flash_pool.start, ck_cert_flash_pool.end, ck_cert_flash_pool.block_size,
                 ck_cert_flash_pool.block_num, ck_cert_flash_pool.data, ck_cert_flash_pool.free);

    ck_cert_flash_pool.init_flag = 1;
    return 0;
}

int ck_cert_flash_erase(flash_data_t *data)
{
    unsigned char *p = (unsigned char *)data;

    CK_FLAHS_DBG("ck_cert_flash_erase, data %x\n", data);

    if (!ck_cert_flash_pool.init_flag) {
        return ck_cert_flash_init();
    }

    while ((unsigned int)p < ck_cert_flash_pool.end) {
        flash_erase(g_eflash_dev, (unsigned int)p - ck_cert_flash_pool.chip_start);
        CK_FLAHS_DBG("erased %x\n", p);
        p += ck_cert_flash_pool.block_size;
        CK_FLAHS_DBG("next erase %x\n", p);
    }

    ck_cert_flash_pool.free = data;
    return 0;
}

static int ck_cert_flash_copy(flash_data_t *dst, uint8_t *src, uint32_t len)
{
    flash_data_t *p = dst;
    int ret;

    if (!ck_cert_flash_pool.init_flag) {
        CK_FLAHS_DBG("flash copy,no init\n");
        return -1;
    }

    if (p->h.used != 0xAA) {
        CK_FLAHS_DBG("wrong flash data data %x, used %x\n", p, p->h.used);
        return -1;
    }

    if (p->h.length  < len) {
        CK_FLAHS_DBG("len is too large, data len %d, len %d\n", p->h.length, len);
        return -1;
    }

    ret = flash_program(g_eflash_dev, (unsigned int)(p->buf)  - ck_cert_flash_pool.chip_start, src, len);
    return (len == ret);
}

static flash_data_t *ck_cert_flash_alloc(uint32_t size)
{
    flash_data_t data = {{0,0,0,0},{}};
    flash_data_t *p;
    ...
    CK_FLAHS_DBG("flash pool, alloc size %d\n", size);

    data.h.magic = CK_FLASH_POOL_MAGIC;
    data.h.used = 0xAA;
    data.h.length = size;
    data.h.blocks = (size + ck_cert_flash_pool.block_size - 1) / ck_cert_flash_pool.block_size;

    if ((unsigned int)ck_cert_flash_pool.free
        + data.h.length
        + sizeof(flash_data_head_t)
        >= ck_cert_flash_pool.end) {
        CK_FLAHS_DBG("no space in flash pool, alloc size %d\n", size);
        return NULL;
    }

    p = ck_cert_flash_pool.free;

    CK_FLAHS_DBG("ck_cert_flash_pool.free->buf %x length %d, blocks %d\n",
                 ck_cert_flash_pool.free->buf, ck_cert_flash_pool.free->h.length, ck_cert_flash_pool.free->h.blocks);
    flash_program(g_eflash_dev, (unsigned int)(ck_cert_flash_pool.free) - ck_cert_flash_pool.chip_start,
                       (void *)&data.h, sizeof(flash_data_head_t));

    CK_FLAHS_DBG("ck_cert_flash_pool.free->buf %x length %d, blocks %d\n",
                 ck_cert_flash_pool.free->buf, ck_cert_flash_pool.free->h.length, ck_cert_flash_pool.free->h.blocks);

    ck_cert_flash_pool.free = CK_FLAHS_NEXT_DATA(ck_cert_flash_pool.free);

    CK_FLAHS_DBG("flash_alloc %x, free %x\n", p, ck_cert_flash_pool.free);
    return p;
}

unsigned char *ck_cert_store(unsigned char *buf, unsigned int size, unsigned char *is_exist)
{
    flash_data_t *p;
    int ret = 0;
    ...
    *is_exist = 0;
    ...
    if (p->h.used == 0xAA) {
        if (p->h.length == size && 0 == memcmp((unsigned char *)(p + 1), buf, size)) {
            CK_FLAHS_DBG("same data is already in flash\n");
            *is_exist = 1;
        } else {
            CK_FLAHS_DBG("cert file is changed, erase\n");
            /*cert is changed ,erase */
            ck_cert_flash_erase(p);
            p = ck_cert_flash_alloc(size);
        }
    } else {
        p = ck_cert_flash_alloc(size);
    }
    ...
    if (p) {
        ck_cert_flash_pool.data = CK_FLAHS_NEXT_DATA(p);

        if (*is_exist) {
            return (unsigned char *)(p->buf);
        }

        ret = ck_cert_flash_copy(p, buf, size);
