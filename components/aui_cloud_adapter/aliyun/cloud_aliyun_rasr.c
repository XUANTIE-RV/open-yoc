...
static int aliyun_rasr_source_pcm(aui_t *aui, void *data, size_t size)
{
    ...
    if (!(aui && aui->ops.asr->priv && data && size > 0)) {
        return -1;
    }

    priv = aui->ops.asr->priv;
    ...
    if (priv->sock && rws_socket_is_connected(priv->sock)) {
        ...
        if (priv->status != 0) {
            ...
            if (!(flags & EVENT_ASR_END)) {
                ...
                if (aui && aui->cb.asr_cb) {
                    if (priv->result)
                        aui->cb.asr_cb(priv->result, strlen(priv->result), aui->cb.asr_priv);
                    else {
                        char text[] = "{\"error\":\"aliyun rasr failed\"}";
                        aui->cb.asr_cb(text, strlen(text), aui->cb.asr_priv);
                    }
                }
                ret = rws_true;
            }
...
{
    aos_check_param(aui);
    aui_cloud_asr_register(aui, &aliyun_asr_cls, cb, priv);
}