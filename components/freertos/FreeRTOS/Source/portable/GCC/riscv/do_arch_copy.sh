...
archs_2="c906 c906fd c906fdv c908 c908i c908v c910 c910v2 c920 c920v2 r910 r920"
archs_3="c907 c907fd c907fdv c907fdvm c907-rv32 c907fd-rv32 c907fdv-rv32 c907fdvm-rv32"
if [ $# -lt 1 ]; then
...
if [[ "$archs_2" =~ "$arch_ori" ]];then
	echo "arch is in c906x/c908x/c910x/c920x/r910x/r920x list"
	mv $arch_ori "/tmp/"$arch_ori
	rm c906* -rf
	rm c908* -rf
	rm c910* -rf
