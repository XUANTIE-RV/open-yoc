...
#endif  /* CONFIG_ECC_L1_ENABLE || CONFIG_ECC_ITCM_ENABLE || CONFIG_ECC_DTCM_ENABLE */
}
...
static void secondary_cpu_init(void)
{
    ...
    /* disable theadisaee & enable copinstee & enable MM */
    unsigned long status = __get_MXSTATUS();
    status &= ~(1 << 22);
    status |= (1 << 24 | 1 << 15);
    __set_MXSTATUS(status);
#elif CONFIG_CPU_XUANTIE_R908_CP_XT || CONFIG_CPU_XUANTIE_R908FD_CP_XT || CONFIG_CPU_XUANTIE_R908FDV_CP_XT
    /* enable theadisaee & enable copinstee & enable MM */
    unsigned long status = __get_MXSTATUS();
    status |= (1 << 22) | (1 << 24 | 1 << 15);
    __set_MXSTATUS(status);
#else
