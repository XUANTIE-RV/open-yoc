...
struct bt_gatt_ccc_cfg_t {
    ...
    bt_addr_le      peer;
    uint16_t            value;
    uint8_t         data[4]     __attribute__((__aligned__(4)));
};
struct bt_gatt_ccc_t {
    struct bt_gatt_ccc_cfg_t    *cfg;
    uint32_t            cfg_len;
    uint16_t            value;
    void (*cfg_changed)(const gatt_attr_t *attr, \
                        uint16_t value);
};
...
#define GATT_CHAR_DEFINE(_uuid, _props) \
    ...
    NULL, NULL,(&(struct gatt_char_t){.uuid= _uuid,.properties = _props,}))
...
#define GATT_CHAR_CCC_DEFINE(_perm) \
    GATT_ATT_DEFINE(UUID_GATT_CCC, GATT_PERM_READ | GATT_PERM_WRITE, NULL, NULL, (&(struct bt_gatt_ccc_t) { .cfg = _perm,\
                .cfg_len = ARRAY_SIZES(_perm),}))
