...
static void vnd_func(struct bt_mesh_model *model,
                     struct bt_mesh_msg_ctx *ctx,
                     struct net_buf_simple *buf)
{
...
static int cmd_net_key_update(int argc, char *argv[])
{
    ...
    err =  bt_mesh_cfg_net_key_update(send_net_idx, send_dst, key_net_idx,
                                      default_key, &status);
...
static int cmd_local_model_pub_set(int argc, char *argv[])
{
    ...
    if (argc < 9) {
        printf("usage: local-model-pub-set <modex_index> <PubAddr> <AppKeyIndex> <cred> <ttl> <period> <count> <interval>\n");
        return  -EINVAL;
    }
...
static int local_model_send(struct bt_mesh_model *model, uint16_t addr, uint8_t ttl,
...
static void unprovison_dev_found(const u8_t addr[6], const u8_t addr_type,
                                 ...
                                 u16_t oob_info, bt_mesh_prov_bearer_t bearer)
{
    if (show_unprovison_dev) {
        ...
        printf("unprovisioned device: %s adv type:%d UUID: %s oob_info: 0x%x bearer: 0x%x\n",
               bt_addr_le_str(&btaddr), adv_type, bt_uuid_str(&uuid.uuid), oob_info, bearer);
    }
}
...
static int prov_appkey_add(int argc, char *argv[])
{
    ...
    if (argc < 3) {
        ...
        printf("add appkey success, netkey id %d, appkey id %d, appkey %s\n",
               netkey_idx, appkey_idx, bt_hex(bt_mesh_provisioner_local_app_key_get(netkey_idx, appkey_idx), 16));
        return 0;
    }
...
static const struct shell_cmd mesh_commands[] = {
    ...
    { "oob-mode", cmd_oob_mode, "<mode:0 NO_OOB 1 STATIC 2 OUTPUT 3 INPUT> [action: 4 ENTER_NUM 8 ENTER_STR|OUTPUT_NUM 16 OUTPUT_STR]" },
...
/*[Genie end] add by wenbing.cwb at 2021-01-21*/
...
#if defined (CONFIG_BT_MESH_PROVISIONER)
    { "provisioner", cmd_provisioner, "<bearer: 0x01 PB-ADV 0x02 PB-GATT 0x03 PB-ADV|PB-GATT> <onoff:0 off 1 on> <show unprovison dev: 0 off 1 on>" },
    ...
    {
        "prov-local_info", prov_local_info,
        NULL
    },
    {
        "prov-appkey-model-bind", prov_appkey_bind,
        "<elem_addr> <model id> <Copmany id> <appkey idx>"
    },
    {
        "prov-appkey-netkey-bind", prov_netkey_bind,
        "<netkey idx> <appkey idx>"
    },
    {
        "prov-node-reset", cmd_prov_node_reset,
        "<netkey idx> <dst_addr>"
    },
    {
        "prov-node-del", cmd_prov_node_del,
        "<dst_addr>"
    },
    {
        "local-model-sub-set", cmd_local_model_sub_set,
        "<model_index> <sub_addr>"
    },
    {
        "local-model-sub-del", cmd_local_model_sub_del,
        "<model_index> <sub_addr>"
    },
    {
    ...
    {
        "onoff-model-cli-send", cmd_onoff_model_cli_send,
        "<modex_index> <ack:1 unack: 0> <on:1 off:0> <addr> [ttl]"
    },
#endif
    { NULL, NULL, NULL}
};
