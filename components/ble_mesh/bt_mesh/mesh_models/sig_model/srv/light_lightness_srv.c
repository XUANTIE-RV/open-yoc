...
static void _light_lightness_prepare_buf(struct bt_mesh_model *model, struct net_buf_simple *p_msg, bool is_ack)
{
    ...
    LOGD(TAG, "cur_actual(0x%04x) tar_actual(0x%04x) remain(0x%02x)", elem->state.lightness_actual[T_CUR],
         elem->state.lightness_actual[T_TAR], remain_byte);
...
static void _light_lightness_status(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx, bool is_ack)
{
...
static E_MESH_ERROR_TYPE _light_lightness_analyze(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                                                  struct net_buf_simple *buf)
{
    ...
    if (lightness < elem->powerup.lightness_range.range_min || lightness > elem->powerup.lightness_range.range_max) {
        LOGE(TAG, "MESH_ANALYZE_ARGS_ERROR lightness(0x%04x) min(0x%04x) max(0x%04x)", lightness,
             elem->powerup.lightness_range.range_min, elem->powerup.lightness_range.range_max);
        return MESH_ANALYZE_ARGS_ERROR;
    }
    ...
    LOGD(TAG, "tar_actual(0x%04x) trans(0x%02x) delay(0x%02x)", elem->state.lightness_actual[T_TAR], elem->state.trans,
         elem->state.delay);
...
static void _light_lightness_get(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx, struct net_buf_simple *buf)
{
    _light_lightness_status(model, ctx, 0);
}

static void _light_lightness_set(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx, struct net_buf_simple *buf)
{
...
static void _light_lightness_set_unack(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                                       struct net_buf_simple *buf)
{
...
static void _light_lightness_linear_prepare_buf(struct bt_mesh_model *model, struct net_buf_simple *p_msg, bool is_ack)
{
    ...
    LOGD(TAG, "cur_linear(0x%04x) tar_linear(0x%04x) remain(0x%02x)", mesh_state->lightness_linear[T_CUR],
         mesh_state->lightness_linear[T_TAR], remain_byte);
 ...
 * */
static E_MESH_ERROR_TYPE _light_lightness_linear_analyze(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                                                         struct net_buf_simple *buf)
{
    u16_t         linear     = 0;
    u8_t          tid        = 0;
    ...
    LOGD(TAG, "tar_linear(0x%04x) trans(0x%02x) delay(0x%02x)", mesh_state->lightness_linear[T_TAR], mesh_state->trans,
         mesh_state->delay);
...
static void _light_lightness_linear_get(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                                        struct net_buf_simple *buf)
{
    LOGD(TAG, "");
 ...
 * */
static void _light_lightness_linear_set(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                                        struct net_buf_simple *buf)
{
    LOGD(TAG, "");
 ...
 * 1. <done> remove pub_need and lightness_linear_action, mesh_publication related operations
 * */
static void _light_lightness_linear_set_unack(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                                              struct net_buf_simple *buf)
{
    LOGD(TAG, "");
...
static void _light_lightness_last_get(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                                      struct net_buf_simple *buf)
{
    LOGD(TAG, "");
...
static void _light_lightness_default_get(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                                         struct net_buf_simple *buf)
{
    LOGD(TAG, "");
...
static void _light_lightness_range_status(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx)
{
    ...
    LOGD(TAG, "min(0x%04x) max(0x%04x)", mesh_powerup->lightness_range.range_min,
         mesh_powerup->lightness_range.range_max);
...
static void _light_lightness_range_get(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                                       struct net_buf_simple *buf)
{
    LOGD(TAG, "");
...
static E_MESH_ERROR_TYPE _light_lightness_default_analyze(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                                                          struct net_buf_simple *buf)
{
    S_ELEM_STATE *  elem         = NULL;
    S_MESH_POWERUP *mesh_powerup = NULL;
    ...
    if (default_temp < mesh_powerup->lightness_range.range_min
        || default_temp > mesh_powerup->lightness_range.range_max) {
        LOGE(TAG, "MESH_ANALYZE_ARGS_ERROR tar(0x%04x) min(0x%04x) max(0x%04x)", default_temp,
...
static void _light_lightness_default_set(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                                         struct net_buf_simple *buf)
{
    LOGD(TAG, "");
...
static void _light_lightness_default_set_unack(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                                               struct net_buf_simple *buf)
{
    LOGD(TAG, "");
...
static E_MESH_ERROR_TYPE _light_lightness_range_analyze(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                                                        struct net_buf_simple *buf)
{
    u16_t           min          = 0;
    u16_t           max          = 0;
    ...
    LOGD(TAG, "min_actual(0x%04x) max_actual(0x%04x)", mesh_powerup->lightness_range.range_min,
         mesh_powerup->lightness_range.range_max);
 ...
 * */
static void _light_lightness_range_set(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                                       struct net_buf_simple *buf)
{
    LOGD(TAG, "");
...
static void _light_lightness_range_set_unack(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                                             struct net_buf_simple *buf)
{
    LOGD(TAG, "");
...
const struct bt_mesh_model_op g_light_lightness_op[LIGHT_LIGHTNESS_OPC_NUM] = {
    ...
    { BT_MESH_MODEL_OP_2(0x82, 0x4c), 3,
      _light_lightness_set }, /* bind operation executed after action done in genie_mesh */
