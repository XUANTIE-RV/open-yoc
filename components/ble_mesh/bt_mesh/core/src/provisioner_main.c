...
bool provisioner_check_msg_dst_addr(u16_t dst_addr)
{
    ...
    for (i = 0; i < ARRAY_SIZE(mesh_nodes); i++) {
        ...
        if (node->node_active && dst_addr >= node->unicast_addr &&
...
int bt_mesh_provisioner_print_node_info(void)
{
    ...
    for (i = 0; i < ARRAY_SIZE(mesh_nodes); i++) {
        ...
        BT_WARN("*   Addr: %02X:%02X:%02X:%02X:%02X:%02X (%s)",
                node->addr_val[5], node->addr_val[4], node->addr_val[3], node->addr_val[2], node->addr_val[1],
                node->addr_val[0], node->addr_type == 0 ? "Public" : "random");
        BT_WARN("*   Unicast Addr: %04x  ELEM Nums: %02x  Net ID %04x", node->unicast_addr, node->element_num, node->net_idx);
