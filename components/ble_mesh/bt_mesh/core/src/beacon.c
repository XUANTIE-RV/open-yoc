...
static u32_t unprov_beacon_interval = UNPROVISIONED_INTERVAL;
static u32_t prov_beacon_interval = PROVISIONED_INTERVAL;
	...
	if (bt_mesh_is_provisioned()) {
		...
		if (bt_mesh_beacon_get() == BT_MESH_BEACON_ENABLED ||
			...
			k_delayed_work_submit(&beacon_timer,
					      bt_mesh_get_beacon_interval(BEACON_TYPE_SECURE));
		}
	} else {
...
u32_t bt_mesh_get_beacon_interval(u8_t type)
{
    switch (type) {
