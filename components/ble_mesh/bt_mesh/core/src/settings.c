...
#ifdef CONFIG_BT_MESH_PROVISIONER
static int prov_node_set(const char *name, size_t len_rd, settings_read_cb read_cb,
		   void *cb_arg)
{
    ...
    int node_index = provisioner_prov_restore_nodes_info(&addr, node_info.dev_uuid, node_info.oob_info, node_info.element_num,
                                        node_info.unicast_addr, node_info.net_idx, node_info.flags, node_info.iv_index, node_info.dev_key, 0 ,lpm_flag, node_info.hb_period_log);
    if (node_index < 0)
    {
...
static int mesh_commit(void)
{
    ...
    if (bt_mesh.app_keys[0].net_idx == BT_MESH_KEY_UNUSED)
    {
        BT_ERR("no app key 0\n");
        return 0;
    }
...
static void schedule_store(int flag)
{
    ...
    if (atomic_test_bit(bt_mesh.flags, BT_MESH_NET_PENDING) ||
        ...
        atomic_test_bit(bt_mesh.flags, BT_MESH_KEYS_PENDING) ||
        atomic_test_bit(bt_mesh.flags, BT_MESH_MOD_PENDING) ||
        atomic_test_bit(bt_mesh.flags, BT_MESH_CFG_PENDING) ||
...
{
#ifndef CONFIG_GENIE_MESH_ENABLE
    entry->store = true;
...
static struct key_update *key_update_find(bool app_key, u16_t key_idx,
        struct key_update **free_slot, bool p_key)
{
...
static void store_pending_mod_check(struct bt_mesh_model *mod,
                                    struct bt_mesh_elem *elem, bool vnd,
                                    bool primary, void *user_data)
{
...
void bt_mesh_settings_init(void)
{
    ...
    SETTINGS_HANDLER_DEFINE(bt_mesh, "bt/mesh", NULL, mesh_set, mesh_commit,
			       NULL);
}
