...
static void net_krp_status(struct bt_mesh_model *model,
			   struct bt_mesh_msg_ctx *ctx,
			   struct net_buf_simple *buf)
{
...
static void mod_sub_list_status(struct bt_mesh_model *model,
			   struct bt_mesh_msg_ctx *ctx,
			   struct net_buf_simple *buf)
{
	u16_t elem_addr;
...
static void mod_sub_list_vnd_status(struct bt_mesh_model *model,
			   struct bt_mesh_msg_ctx *ctx,
			   struct net_buf_simple *buf)
{
	u16_t elem_addr;
...
static void cli_ctrl_relay_status(struct bt_mesh_model *model,
						  struct bt_mesh_msg_ctx *ctx,
						  struct net_buf_simple *buf)
{
...
#if defined(CONFIG_BT_BQB) && CONFIG_BT_BQB
static struct bt_mesh_app_key *_app_key_alloc(void)
{
    int i;
...
int bt_mesh_cfg_krp_set(u16_t net_idx, u16_t addr, u16_t key_net_idx,
...
int bt_mesh_cfg_krp_get(u16_t net_idx, u16_t addr, u16_t key_net_idx,
			    u8_t *phase, u8_t *status)
{
...
int bt_mesh_cfg_mod_sub_get(u16_t net_idx, u16_t addr, u16_t mod_id,
			   uint16_t *sub, u8_t *status)
{
	...
	err = cli_prepare(&param, OP_MOD_SUB_LIST);
	if (err) {
		return err;
	}
...
int bt_mesh_cfg_mod_sub_get_vnd(u16_t net_idx, u16_t addr, u16_t mod_id,u16_t cid,
			   uint16_t *sub, u8_t *status)
{
	if ((sub == NULL && status) || (sub && status == NULL)) {
		return -EINVAL;
	...
	err = cli_prepare(&param, OP_MOD_SUB_LIST_VND);
	if (err) {
		return err;
	}
						...
						struct ctrl_relay_param *cr, u8_t *status)
{
	...
	err = cli_prepare(&param, OP_CTRL_RELAY_CONF_STATUS);
	if (err)
	{
		return err;
	}
