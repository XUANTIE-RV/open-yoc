                        ...
                        bool vnd, bool primary, void *user_data)
{
    ...
    if (!vnd &&
        (mod->id == BT_MESH_MODEL_ID_CFG_SRV ||  \
         mod->id ==  BT_MESH_MODEL_ID_CFG_CLI || \
...
int ble_mesh_node_init(node_config_t *param)
{
    ...
    if (PROVISIONER == param->role) {
        ...
        if (!param->provisioner_config || \
            mesh_comp->elem_count > (param->provisioner_config->config.unicast_addr_start - param->provisioner_config->config.unicast_addr_local)) {
            return -EINVAL;
        }
