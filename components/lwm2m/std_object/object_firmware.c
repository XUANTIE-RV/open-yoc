...
static fota_data_t fota_data;

void oceancon_fota_finish(struct fota_data *f_data) {
    firmware_data_t * data = (firmware_data_t*)(f_data->data);
    lwm2m_uri_t uri;
    data->state = 2;
    fprintf(stdout, "\n\t FIRMWARE FINISH\r\n\n");
    lwm2m_stringToUri("/5/0/3", 6, &uri);
    lwm2m_resource_value_changed(data->contextP, &uri);
    lwm2m_break_recv(data->contextP);

    aos_kv_del(FW_URL_KEY);
    aos_kv_del("fota_offset");
};

void oceancon_fota_fail(struct fota_data *f_data) {
    firmware_data_t * data = (firmware_data_t*)(f_data->data);
    lwm2m_uri_t uri;
    data->state = 0;
    fprintf(stdout, "\n\t FIRMWARE FAIL\r\n\n");
    ...
    lwm2m_resource_value_changed(data->contextP, &uri);
    lwm2m_break_recv(data->contextP);
    lwm2m_printf("fota_cb: FOTA_EVENT_FAIL\r\n");
};

void oceancon_fota_reboot(struct fota_data *f_data) {
    firmware_data_t * data = (firmware_data_t*)(f_data->data);
    lwm2m_uri_t uri;
    fprintf(stdout, "\n\t FIRMWARE UPDATE\r\n\n");
    data->state = 3;
    ...
    event_publish_delay(EVENT_OS_REBOOT, NULL, 5000);
};
    ...
    do {
        switch (dataArray[i].id) {
            ...
            case RES_M_PACKAGE_URI:
                ...
                data->url = lwm2m_malloc(dataArray[i].value.asBuffer.length + 1);
                if (data->url == NULL) {
                    return COAP_500_INTERNAL_SERVER_ERROR;
                }

                memcpy(data->url, dataArray[i].value.asBuffer.buffer, dataArray[i].value.asBuffer.length);
                data->url[dataArray[i].value.asBuffer.length] = 0;
                fota_data.url = data->url;
                fota_data.data = data;
                // printf("publish start1: (%p) (%p) (%p) (%p) (%p) (%p)\n", fota_data, &fota_data, fota_data.url, fota_data.data, data->url, data);
                event_publish(EVENT_FOTA_START, &fota_data);
                // char url_old[80];
                // if (aos_kv_getstring(FW_URL_KEY, url_old, 80) > 0) {
                //     iscontinue = (strcmp(url, url_old) == 0 ? 1 : 0);
                // }

                // if (iscontinue == 0) {
                //     aos_kv_setstring(FW_URL_KEY, url);
                // }

                // // start_fota(url, iscontinue);
    ...
    switch (resourceId) {
        case RES_M_UPDATE:
            if (data->state == 2) {
                fota_data.url = data->url;
                fota_data.data = data;
                printf("publish update: (%p) (%p) (%p) (%p) (%p) (%p)\n", fota_data, &fota_data, fota_data.url, fota_data.data, data->url, data);
                // printf("publish update: (%p) (%p) (%p) (%p) (%p)\n", fota_data, fota_data->url, fota_data->data, data->url, data);
                event_publish(EVENT_FOTA_UPDATE, &fota_data);
...
lwm2m_object_t *get_object_firmware(lwm2m_context_t *contextP)
{
    /*
    ...
    if (NULL != firmwareObj) {
        ...
        if (NULL != firmwareObj->userData) {
            ...
            data->state = 0;
            data->result = 0;
            strcpy(data->pkg_name, "csky");
            strcpy(data->pkg_version, aos_get_app_version());
            ...
            //         lwm2m_printf("firmware restart\r\n");
            //         // start_fota(url, 0);
            //         event_publish_delay(EVENT_FOTA_RESTART, NULL, 0);
            //     }
            // #endif
            fota_data.url = data->url;
            fota_data.data = data;
#if FOTA_RESUME
            printf("publish start2: (%p) (%p) (%p) (%p) (%p) (%p)\n", fota_data, &fota_data, fota_data.url, fota_data.data, data->url, data);
            // printf("publish start2: (%p) (%p) (%p) (%p) (%p)\n", fota_data, fota_data->url, fota_data->data, data->url, data);
            event_publish(EVENT_FOTA_START, &fota_data);
#endif
        } else {
            lwm2m_free(firmwareObj);
