...
{
    remove_log_poll();
    g_log_poll = UTILS_malloc(LOG_POLL_SIZE);
    int ret = reset_log_poll();
    return ret;
}
...
void parse_switch_info(_IN_ char *payload, _IN_ int payload_len)
{
    ...
    char *sample_target = lite_sample_target.value;
    int sample_target_len  = lite_sample_target.value_length;
