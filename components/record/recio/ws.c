...
static void on_socket_connected(rws_socket socket)
{
    ...
    aos_sem_signal(&priv->conn_notify);
}
		...
		if (priv->sock) {
			...
			for (int i = 0; i < s_sock_count; i++) {
				if (priv->sock == tmp[i].sock) {
					aos_free(tmp[i].path_pre);
					aos_mutex_free(&tmp[i].sendmtx);
					cnt ++;
					continue;
				}
...
static int ws_open(recio_t *io, const char *path)
{
    ...
    if (sock) {
        ws_config_t *priv = aos_zalloc_check(sizeof(ws_config_t));
		if (got_same == 0)
        	aos_sem_new(&priv->conn_notify, 0);
		...
		if (got_same == 0) {
			aos_sem_wait(&priv->conn_notify, 5000);
			aos_sem_free(&priv->conn_notify);			
		}
		...
		if (got_same == 0) {
			if (s_sock_count < MAX_SOCKET_FD - 1) {
				...
				aos_mutex_new(&s_socks_mgr[s_sock_count].sendmtx);
				s_sock_count ++;				
			}
		}
		LOGD(TAG, "s_sock_count:%d", s_sock_count);
...
static int ws_write(recio_t *io, uint8_t *buffer, int length, int timeoutms)
{
    ...
    uint8_t *tmpbuf = aos_malloc_check(slen + 1);
    if (tmpbuf == NULL) {
        return -1;
    }
	if (p_sendmtx) {
		aos_mutex_lock(p_sendmtx, AOS_WAIT_FOREVER);
	}
