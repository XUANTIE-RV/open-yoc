...
USB_SLIST_DEFINE(usbh_class_driver_list);
...
{
    usb_slist_t *list_node;
    usb_slist_for_each(list_node, &usbh_class_driver_list)
    {
        const struct usbh_class_info *index = usb_slist_entry(list_node, struct usbh_class_info_list, list)->info;
        ...
        if ((index->match_flags & (USB_CLASS_MATCH_INTF_CLASS | USB_CLASS_MATCH_INTF_SUBCLASS | USB_CLASS_MATCH_INTF_PROTOCOL)) ==
        ...
        if ((index->match_flags & (USB_CLASS_MATCH_VENDOR | USB_CLASS_MATCH_PRODUCT | USB_CLASS_MATCH_INTF_CLASS)) ==
        ...
        if ((index->match_flags & (USB_CLASS_MATCH_VENDOR | USB_CLASS_MATCH_PRODUCT)) ==
                   (USB_CLASS_MATCH_VENDOR | USB_CLASS_MATCH_PRODUCT)) {
            if (index->vid == vid && index->pid == pid) {
                return index->class_driver;
            }
        }
        if (index->match_flags & (USB_CLASS_MATCH_INTF_CLASS)) {
...
int usbh_register_class_driver(const struct usbh_class_info *usbh_driver)
{
    if (usbh_driver == NULL) return -1;

    struct usbh_class_info_list *list = usb_malloc(sizeof(struct usbh_class_info_list));

    if (list == NULL) {
        return -1;
    }

    list->info = usbh_driver;

    if (usbh_find_class_driver(usbh_driver->class, usbh_driver->subclass, usbh_driver->protocol, usbh_driver->vid, usbh_driver->pid) == NULL) {
        //register driver into list
        usb_slist_add_tail(&usbh_class_driver_list, &list->list);
    }
    return 0;
}
...
int usbh_initialize(void)
{
    ...
    usbh_register_class_driver(&hub_info);
    //usbh_register_class_driver();
                ...
                if (hport->connected) {
                    for (uint8_t i = 0; i < hport->config.config_desc.bNumInterfaces; i++) {
                        if (hport->config.intf[i].class_driver) {
                            USB_LOG_RAW("    |__Port %u,Port addr:0x%02x, ID %04x:%04x, If %u,ClassDriver=%s\r\n",
                                        hport->port,
                                        hport->dev_addr,
                                        hport->device_desc.idVendor,
                                        hport->device_desc.idProduct,
                            ...
                            USB_LOG_RAW("    |__Port %u,Port addr:0x%02x, ID %04x:%04x, If %u,ClassDriver=NULL\r\n",
                                        hport->port,
                                        hport->dev_addr,
                                        hport->device_desc.idVendor,
                                        hport->device_desc.idProduct,
...
#ifdef AOS_COMP_CLI
ALIOS_CLI_CMD_REGISTER(lsusb, lsusb, lsusb devices);
#endif
