...
static inline int usbh_msc_scsi_testunitready(struct usbh_msc *msc_class)
{
    ...
    cbw->bCBLength = USB_SCSI_CMD_TESTUNITREADY_SIZEOF;
    cbw->CB[0] = USB_SCSI_CMD_TESTUNITREADY;
...
static inline int usbh_msc_scsi_requestsense(struct usbh_msc *msc_class)
{
    ...
    cbw->bmFlags = 0x80;
    cbw->bCBLength = USB_SCSI_RESP_FIXEDSENSEDATA_SIZEOF;
    cbw->dDataLength = USB_SCSI_CMD_REQUESTSENSE_SIZEOF;
    cbw->CB[0] = USB_SCSI_CMD_REQUESTSENSE;
    cbw->CB[4] = USB_SCSI_RESP_FIXEDSENSEDATA_SIZEOF;
    ...
    if (nbytes >= 0) {
        ...
        nbytes = usbh_msc_bulk_in_transfer(msc_class, g_msc_buf, USB_SCSI_RESP_FIXEDSENSEDATA_SIZEOF, CONFIG_USBHOST_MSC_TIMEOUT);
        if (nbytes >= 0) {
            /* Receive the CSW */
            nbytes = usbh_msc_bulk_in_transfer(msc_class, g_msc_buf, USB_SIZEOF_MSC_CSW, CONFIG_USBHOST_MSC_TIMEOUT);
            if (nbytes >= 0) {
...
static inline int usbh_msc_scsi_inquiry(struct usbh_msc *msc_class)
{
    ...
    cbw->dDataLength = USB_SCSI_RESP_INQUIRY_SIZEOF;
    cbw->bmFlags = 0x80;
    cbw->bCBLength = USB_SCSI_CMD_INQUIRY_SIZEOF;
    cbw->CB[0] = USB_SCSI_CMD_INQUIRY;
    cbw->CB[4] = USB_SCSI_RESP_INQUIRY_SIZEOF;
    ...
    if (nbytes >= 0) {
        /* Receive the sense data response */
        nbytes = usbh_msc_bulk_in_transfer(msc_class, g_msc_buf, USB_SCSI_RESP_INQUIRY_SIZEOF, CONFIG_USBHOST_MSC_TIMEOUT);
        if (nbytes >= 0) {
            /* Receive the CSW */
            nbytes = usbh_msc_bulk_in_transfer(msc_class, g_msc_buf, USB_SIZEOF_MSC_CSW, CONFIG_USBHOST_MSC_TIMEOUT);
            if (nbytes >= 0) {
...
static inline int usbh_msc_scsi_readcapacity10(struct usbh_msc *msc_class)
{
    ...
    cbw->dDataLength = USB_SCSI_RESP_READCAPACITY10_SIZEOF;
    cbw->bmFlags = 0x80;
    cbw->bCBLength = USB_SCSI_CMD_READCAPACITY10_SIZEOF;
    cbw->CB[0] = USB_SCSI_CMD_READCAPACITY10;
    ...
    if (nbytes >= 0) {
        /* Receive the sense data response */
        nbytes = usbh_msc_bulk_in_transfer(msc_class, g_msc_buf, USB_SCSI_RESP_READCAPACITY10_SIZEOF, CONFIG_USBHOST_MSC_TIMEOUT);
        if (nbytes >= 0) {
...
int usbh_msc_scsi_write10(struct usbh_msc *msc_class, uint32_t start_sector, const uint8_t *buffer, uint32_t nsectors)
{
    ...
    cbw->bCBLength = USB_SCSI_CMD_WRITE10_SIZEOF;
    cbw->CB[0] = USB_SCSI_CMD_WRITE10;
...
int usbh_msc_scsi_read10(struct usbh_msc *msc_class, uint32_t start_sector, const uint8_t *buffer, uint32_t nsectors)
{
    ...
    cbw->dDataLength = (msc_class->blocksize * nsectors);
    cbw->bmFlags = 0x80;
    cbw->bCBLength = USB_SCSI_CMD_READ10_SIZEOF;
    cbw->CB[0] = USB_SCSI_CMD_READ10;
