...
static bool USB_SCSI_processWrite(uint32_t nbytes);
static bool USB_SCSI_processRead(void);

/**
* @brief  USB_SCSI_SetSenseData
...
*/
static void USB_SCSI_SetSenseData(uint32_t KCQ)
{
...
static bool USB_SCSI_testUnitReady(uint8_t **data, uint32_t *len)
{
    if (usbd_msc_cfg.cbw.dDataLength != 0U) {
        USB_SCSI_SetSenseData(USB_SCSI_KCQIR_INVALIDCOMMAND);
        return false;
    }
    *data = NULL;
    *len = 0;
...
static bool USB_SCSI_requestSense(uint8_t **data, uint32_t *len)
{
    uint8_t data_len = USB_SCSI_RESP_FIXEDSENSEDATA_SIZEOF;
    if (usbd_msc_cfg.cbw.dDataLength == 0U) {
        USB_SCSI_SetSenseData(USB_SCSI_KCQIR_INVALIDCOMMAND);
        return false;
    }

    if (usbd_msc_cfg.cbw.CB[4] < USB_SCSI_RESP_FIXEDSENSEDATA_SIZEOF) {
        data_len = usbd_msc_cfg.cbw.CB[4];
    }

    uint8_t request_sense[USB_SCSI_RESP_FIXEDSENSEDATA_SIZEOF] = {
        0x70,
        0x00,
        0x00, /* Sense Key */
        0x00,
        ...
        0x00,
        0x00,
        USB_SCSI_RESP_FIXEDSENSEDATA_SIZEOF - 8,
        0x00,
        0x00,
        0x00,
        0x00,
...
static bool USB_SCSI_inquiry(uint8_t **data, uint32_t *len)
{
    uint8_t data_len = USB_SCSI_RESP_INQUIRY_SIZEOF;
    ...
    uint8_t inquiry[USB_SCSI_RESP_INQUIRY_SIZEOF] = {
        /* 36 */
        ...
        0x02,
        0x02,
        (USB_SCSI_RESP_INQUIRY_SIZEOF - 5),
        0x00,
        0x00,
        0x00,
    ...
    if ((usbd_msc_cfg.cbw.CB[1] & 0x01U) != 0U) { /* Evpd is set */
        if (usbd_msc_cfg.cbw.CB[2] == 0U) {       /* Request for Supported Vital Product Data Pages*/
            ...
            USB_SCSI_SetSenseData(USB_SCSI_KCQIR_INVALIDFIELDINCBA);
            return false;
        }
    } else {
        if (usbd_msc_cfg.cbw.CB[4] < USB_SCSI_RESP_INQUIRY_SIZEOF) {
            data_len = usbd_msc_cfg.cbw.CB[4];
        }
...
static bool USB_SCSI_startStopUnit(uint8_t **data, uint32_t *len)
{
    if (usbd_msc_cfg.cbw.dDataLength != 0U) {
        USB_SCSI_SetSenseData(USB_SCSI_KCQIR_INVALIDCOMMAND);
        return false;
...
static bool USB_SCSI_preventAllowMediaRemoval(uint8_t **data, uint32_t *len)
{
    if (usbd_msc_cfg.cbw.dDataLength != 0U) {
        USB_SCSI_SetSenseData(USB_SCSI_KCQIR_INVALIDCOMMAND);
        return false;
...
static bool USB_SCSI_modeSense6(uint8_t **data, uint32_t *len)
{
    uint8_t data_len = 4;
    if (usbd_msc_cfg.cbw.dDataLength == 0U) {
        USB_SCSI_SetSenseData(USB_SCSI_KCQIR_INVALIDCOMMAND);
    ...
    if (usbd_msc_cfg.cbw.CB[4] < USB_SCSI_RESP_MODEPARAMETERHDR6_SIZEOF) {
        data_len = usbd_msc_cfg.cbw.CB[4];
    }

    uint8_t sense6[USB_SCSI_RESP_MODEPARAMETERHDR6_SIZEOF] = { 0x03, 0x00, 0x00, 0x00 };
...
static bool USB_SCSI_modeSense10(uint8_t **data, uint32_t *len)
{
    uint8_t data_len = 27;
    if (usbd_msc_cfg.cbw.dDataLength == 0U) {
        USB_SCSI_SetSenseData(USB_SCSI_KCQIR_INVALIDCOMMAND);
...
static bool USB_SCSI_readFormatCapacity(uint8_t **data, uint32_t *len)
{
    if (usbd_msc_cfg.cbw.dDataLength == 0U) {
        USB_SCSI_SetSenseData(USB_SCSI_KCQIR_INVALIDCOMMAND);
        return false;
    }
    uint8_t format_capacity[USB_SCSI_RESP_READFORMATCAPACITIES_SIZEOF] = {
        0x00,
        0x00,
        0x00,
    ...
    memcpy(*data, (uint8_t *)format_capacity, USB_SCSI_RESP_READFORMATCAPACITIES_SIZEOF);
    *len = USB_SCSI_RESP_READFORMATCAPACITIES_SIZEOF;
    return true;
}

static bool USB_SCSI_readCapacity10(uint8_t **data, uint32_t *len)
{
    if (usbd_msc_cfg.cbw.dDataLength == 0U) {
        USB_SCSI_SetSenseData(USB_SCSI_KCQIR_INVALIDCOMMAND);
        return false;
    ...
    uint8_t capacity10[USB_SCSI_RESP_READCAPACITY10_SIZEOF] = {
    ...
    memcpy(*data, (uint8_t *)capacity10, USB_SCSI_RESP_READCAPACITY10_SIZEOF);
    *len = USB_SCSI_RESP_READCAPACITY10_SIZEOF;
    return true;
}

static bool USB_SCSI_read10(uint8_t **data, uint32_t *len)
{
    ...
    if ((usbd_msc_cfg.start_sector + usbd_msc_cfg.nsectors) > usbd_msc_cfg.scsi_blk_nbr) {
        USB_SCSI_SetSenseData(USB_SCSI_KCQIR_LBAOUTOFRANGE);
    ...
    return USB_SCSI_processRead();
}

static bool USB_SCSI_read12(uint8_t **data, uint32_t *len)
{
    if (((usbd_msc_cfg.cbw.bmFlags & 0x80U) != 0x80U) || (usbd_msc_cfg.cbw.dDataLength == 0U)) {
        USB_SCSI_SetSenseData(USB_SCSI_KCQIR_INVALIDCOMMAND);
        return false;
...
static bool USB_SCSI_write10(uint8_t **data, uint32_t *len)
{
    uint32_t data_len = 0;
...
static bool USB_SCSI_write12(uint8_t **data, uint32_t *len)
{
    uint32_t data_len = 0;
    if (((usbd_msc_cfg.cbw.bmFlags & 0x80U) != 0x00U) || (usbd_msc_cfg.cbw.dDataLength == 0U)) {
        USB_SCSI_SetSenseData(USB_SCSI_KCQIR_INVALIDCOMMAND);
...
#if 0
static bool USB_SCSI_verify10(uint8_t **data, uint32_t *len)
{
...
static bool USB_SCSI_processRead(void)
{
    ...
    if (usbd_msc_sector_read(usbd_msc_cfg.start_sector, usbd_msc_cfg.block_buffer, transfer_len) != 0) {
        USB_SCSI_SetSenseData(USB_SCSI_KCQHE_UREINRESERVEDAREA);
        return false;
    }
#endif
...
static bool USB_SCSI_processWrite(uint32_t nbytes)
{
    ...
    if (usbd_msc_sector_write(usbd_msc_cfg.start_sector, usbd_msc_cfg.block_buffer, nbytes) != 0) {
        USB_SCSI_SetSenseData(USB_SCSI_KCQHE_WRITEFAULT);
        return false;
    }
#endif
...
static bool USB_SCSI_CBWDecode(uint32_t nbytes)
{
    ...
    if ((usbd_msc_cfg.cbw.bLUN > 1) || (usbd_msc_cfg.cbw.dSignature != MSC_CBW_Signature) || (usbd_msc_cfg.cbw.bCBLength < 1) || (usbd_msc_cfg.cbw.bCBLength > 16)) {
        ...
        switch (usbd_msc_cfg.cbw.CB[0]) {
            case USB_SCSI_CMD_TESTUNITREADY:
                ret = USB_SCSI_testUnitReady(&buf2send, &len2send);
                break;
            case USB_SCSI_CMD_REQUESTSENSE:
                ret = USB_SCSI_requestSense(&buf2send, &len2send);
                break;
            case USB_SCSI_CMD_INQUIRY:
                ret = USB_SCSI_inquiry(&buf2send, &len2send);
                break;
            case USB_SCSI_CMD_STARTSTOPUNIT:
                ret = USB_SCSI_startStopUnit(&buf2send, &len2send);
                break;
            case USB_SCSI_CMD_PREVENTMEDIAREMOVAL:
                ret = USB_SCSI_preventAllowMediaRemoval(&buf2send, &len2send);
                break;
            case USB_SCSI_CMD_MODESENSE6:
                ret = USB_SCSI_modeSense6(&buf2send, &len2send);
                break;
            case USB_SCSI_CMD_MODESENSE10:
                ret = USB_SCSI_modeSense10(&buf2send, &len2send);
                break;
            case USB_SCSI_CMD_READFORMATCAPACITIES:
                ret = USB_SCSI_readFormatCapacity(&buf2send, &len2send);
                break;
            case USB_SCSI_CMD_READCAPACITY10:
                ret = USB_SCSI_readCapacity10(&buf2send, &len2send);
                break;
            case USB_SCSI_CMD_READ10:
                ret = USB_SCSI_read10(NULL, 0);
                break;
            case USB_SCSI_CMD_READ12:
                ret = USB_SCSI_read12(NULL, 0);
                break;
            case USB_SCSI_CMD_WRITE10:
                ret = USB_SCSI_write10(NULL, 0);
                break;
            case USB_SCSI_CMD_WRITE12:
                ret = USB_SCSI_write12(NULL, 0);
                break;
            case USB_SCSI_CMD_VERIFY10:
                //ret = USB_SCSI_verify10(NULL, 0);
                ret = false;
                break;
...
void mass_storage_bulk_out(uint8_t ep, uint32_t nbytes)
{
    switch (usbd_msc_cfg.stage) {
        case MSC_READ_CBW:
            if (USB_SCSI_CBWDecode(nbytes) == false) {
        ...
        case MSC_DATA_OUT:
            switch (usbd_msc_cfg.cbw.CB[0]) {
                case USB_SCSI_CMD_WRITE10:
                case USB_SCSI_CMD_WRITE12:
                    if (USB_SCSI_processWrite(nbytes) == false) {
