...
typedef struct {
#ifdef CONFIG_LPM
    uint8_t crc_power_status;
    uint32_t crc_regs_saved[2];
#endif
    uint32_t base;
    crc_event_cb_t cb;
    crc_status_t status;
...
#ifdef CONFIG_LPM
static void manage_clock(crc_handle_t handle, uint8_t enable)
{
    drv_clock_manager_config(CLOCK_MANAGER_CRC, enable);
}

static void do_prepare_sleep_action(crc_handle_t handle)
{
    ck_crc_priv_t *crc_priv = handle;
    uint32_t *cbase = (uint32_t *)(crc_priv->base);
    registers_save(crc_priv->crc_regs_saved, cbase, 2);
}

static void do_wakeup_sleep_action(crc_handle_t handle)
{
    ck_crc_priv_t *crc_priv = handle;
    uint32_t *cbase = (uint32_t *)(crc_priv->base);
    registers_restore(cbase, crc_priv->crc_regs_saved, 2);
}
#endif
...
#ifdef CONFIG_LPM
    csi_crc_power_control(crc_priv, DRV_POWER_FULL);
#endif
...
#ifdef CONFIG_LPM
    csi_crc_power_control(crc_priv, DRV_POWER_OFF);
#endif
    return 0;
}
