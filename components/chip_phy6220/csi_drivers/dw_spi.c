...
int32_t csi_spi_config_baudrate(spi_handle_t handle, uint32_t baud)
{
    ...
    int32_t sckdv = drv_get_apb_freq(0) / baud;
...
int32_t csi_spi_config_mode(spi_handle_t handle, spi_mode_e  mode)
{
    ...
    if (mode == SPI_MODE_MASTER) {
        if (spi_priv->idx == 0) {
            AP_COM->PERI_MASTER_SELECT |= SPI0_MASTER_MODE | (1 << 4);
        } else {
            AP_COM->PERI_MASTER_SELECT |= SPI1_MASTER_MODE | (1 << 5);
        }
    ...
    if (mode == SPI_MODE_SLAVE) {
        if (spi_priv->idx == 0) {
            AP_COM->PERI_MASTER_SELECT &= ~SPI0_MASTER_MODE;
        } else {
            AP_COM->PERI_MASTER_SELECT &= ~SPI1_MASTER_MODE;
        }
