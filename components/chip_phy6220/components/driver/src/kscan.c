...
static void hal_kscan_config_row(KSCAN_ROWS_e row)
{
    ...
    phy_gpio_fmux(row_pin, Bit_DISABLE);
    phy_gpio_pull_set(row_pin, GPIO_PULL_DOWN);
...
static void hal_kscan_config_col(KSCAN_COLS_e col)
{
    ...
    if (col == KEY_COL_P16 || col == KEY_COL_P17) {
        phy_gpio_cfg_analog_io(col_pin, Bit_DISABLE);
    }

    phy_gpio_fmux(col_pin, Bit_DISABLE);
    phy_gpio_pull_set(col_pin, GPIO_PULL_DOWN);
...
static void kscan_sleep_handler(void)
{
    ...
    for (uint8_t i = 0; i < NUM_KEY_COLS; i++) {
        ...
        phy_gpio_pin_init(col_pin, GPIO_INPUT_1);
        phy_gpio_pull_set(col_pin, GPIO_PULL_UP_S);
    }
    ...
    for (uint8_t i = 0; i < NUM_KEY_ROWS; i++) {
        ...
        phy_gpio_pull_set(row_pin, GPIO_PULL_DOWN);
        phy_gpio_pin_init(row_pin, GPIO_INPUT_1);
        pol = phy_gpio_read(row_pin) ? POL_FALLING : POL_RISING;

        phy_gpio_wakeup_set(row_pin, pol);
...
static void kscan_wakeup_handler(void)
{
    ...
    for (uint8_t i = 0; i < NUM_KEY_ROWS; i++) {
        gpio_pin_e row_pin = (gpio_pin_e)KSCAN_ROW_GPIO[m_kscanCtx.cfg.key_rows[i]];
        phy_gpio_pin_init(row_pin, GPIO_INPUT_1);
        phy_gpio_pull_set(row_pin, GPIO_PULL_DOWN);//teddy add 20190122
    }
    ...
    for (uint8_t i = 0; i < NUM_KEY_ROWS; i++) {
        ...
        gpio_polarity_e pol = phy_gpio_read(row_pin) ? POL_RISING : POL_FALLING;
