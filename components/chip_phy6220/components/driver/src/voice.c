...
{
    phy_gpio_fmux_set(dmicDataPin, (Fmux_Type_e)FMUX_ADCC);
    phy_gpio_fmux_set(dmicClkPin, (Fmux_Type_e)FMUX_CLK1P28M);
}
...
void __attribute__((used)) hal_ADC_IRQHandler(void)
{
    ...
    uint32_t adc_int_status = GET_IRQ_STATUS;
    //LOG("Voice interrupt processing:%x\n",adc_int_status);
    MASK_VOICE_INT;

    if (adc_int_status & BIT(8)) {
        int n;
...
int hal_voice_start(void)
{
    ...
    //NVIC_EnableIRQ((IRQn_Type)ADCC_IRQn);//ADCC_IRQ_ENABLE;
