...
static int xfyun_tts_init(aui_t *aui)
{
    (void)aui;
...
*/
static void xfyun_tts_toggle_account()
{
...
*/
static int xfyun_tts(aui_t *aui, const char *text, const char* fifo_name)
{
    int ret = 0;
    int post_len = 0;
    int                       resp_status  = 0;
    ...
    if (strlen(text) >= XF_TTS_TXT_SIZE_MAX)
        return -1;
    ...
    post_len  = (strlen(text) * 3) + 16;
    post_data = (char *)aos_zalloc(post_len);
    strcpy(post_data, "text=");
    http_url_encode(post_data + strlen("text="), text);
    ...
    nsfifo_t *fifo = nsfifo_open(fifo_name, O_CREAT, 64*1024);
    if (NULL == fifo) {
...
static aui_tts_cls_t xunfei_tts_cls = {
    .init = xfyun_tts_init,
    .req_tts = xfyun_tts,
};
