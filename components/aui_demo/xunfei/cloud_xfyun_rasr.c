...
static int xfyun_rasr_init(aui_t *aui)
{
    (void)aui;
...
static int xfyun_rasr_source_pcm(aui_t *aui, void *data, size_t size)
{
    int wlen;
    char *pos;
    sfifo_t *fifo;
...
static aui_asr_cls_t xunfei_asr_cls = {
    .init = xfyun_rasr_init,
    .push_data = xfyun_rasr_source_pcm,
    .stop_push_data = xfyun_rasr_source_pcm_finish,
};

void aui_asr_register_xunfei(aui_t *aui)
{
    if (aui) {
        aui->ops.asr = &xunfei_asr_cls;
    }
}
