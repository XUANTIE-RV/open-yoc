...
static int mit_asr_event_cb(void *user_data, NuiThingsEvent event, int dialog_finish)
{
    ...
    LOGD(TAG, "call %s(>>>>event: %d(%s)<<<<) [start]", __FUNCTION__, event,
    ...
    switch (event) {
        ...
        case kNuiThingsEventDialogResult:
            if (context->status == MIT_BUFFER_FINISH) {
...
static int mit_asr_provide_data_cb(void *user_data, char *buffer, int len)
{
    ...
    if (context->status == MIT_BUFFER_ONGOING) {
...
static void tsk_wait_for_mit_consuming(void *arg)
{
    ...
    while (context->status != MIT_BUFFER_END && context->err == MIT_ASR_SUCCESS &&
    ...
    if (context->err == MIT_ASR_SUCCESS) {
        ...
        timeout_cnt = 0;
        while (context->status != MIT_BUFFER_END) {
            aos_msleep(200);
            if (++timeout_cnt == 100) {
...
{
    aos_check_return_einval(session_id);
...
int aui_cloud_start_pcm(aui_t *aui)
{
    ...
    if (stat != MIT_BUFFER_END) {
        unsigned int flag;
        
        context->status = MIT_BUFFER_END;
        nui_things_stop(1);

        if (stat == MIT_BUFFER_FINISH) {
    ...
    context->status     = MIT_BUFFER_ONGOING;
    context->err        = MIT_ASR_SUCCESS;
    bufferLocked        = 0;
    tts_connected       = 0;
