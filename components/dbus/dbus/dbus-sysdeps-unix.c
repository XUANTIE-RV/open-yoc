 ...
 again:
  ...
  if (bytes_read > 0) {
	  ...
	  for (i = 0; i < bytes_read; i++) {
		  rc += snprintf(tdata + rc, cnt - rc, "%02x", data[i]);
	  }
      _dbus_verbose ("read=>[%4d]:%s", bytes_read, tdata);
	  free(tdata);
  }
      /* put length back (doesn't actually realloc) */
      _dbus_string_set_length (buffer, start + bytes_read);
             ...
             int               len)
{
 ...
 again:
  ...
  if (bytes_written > 0) {
      ...
      _dbus_verbose ("write=>[%4d]:%s", bytes_written, tdata);
	  free(tdata);
  }
#else
  if (bytes_written > 0)
...
#if CONFIG_KERNEL_RTOS
  {
	  int n_ready = poll (fds, n_fds, timeout_milliseconds);
	  if (n_ready <= 0) {
		  if (timeout_milliseconds) {
			  //FIXME: path for lwip_poll, pollres may be zero of scheduler-reason
			  aos_msleep(100);
		  }
	  }
	  return n_ready;
...
#if CONFIG_KERNEL_RTOS
  //FIXME: fake random
  _dbus_string_append_len (str, (char*)&str, n_bytes);
  return TRUE;
#else
  int old_len;
  int fd;
