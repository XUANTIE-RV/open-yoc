...
signal_handler (int sig)
{
  ...
  _dbus_verbose("%s, %d not support", __FUNCTION__, __LINE__);
  return;
#else
...
static void dbus_daemon_task(void *arg)
{
	int rc;
	int argc = 3;
	char *argv[3] = {"dbus-daemon", "--system" , "--address="DBUS_SYSTEM_BUS_DEFAULT_ADDRESS };

	rc = dbus_daemon_main(argc, (char**)&argv);
	if (rc != 0) {
		LOGE("dbus", "dbus daemon start failed, rc = %d", rc);
	}
}

/**
 * @brief  start the dbus daemon for rtos
 * @return 0/-1 
 */
int dbus_daemon_start()
{
	int rc = 0;
	static int init;
	...
	if (!init) {
		aos_task_t task;
		rc = aos_task_new_ext(&task, "dbus-daemon", dbus_daemon_task, NULL, 1024*8, AOS_DEFAULT_APP_PRI - 1);
		if (rc != 0) {
			LOGE("dbus", "dbus_daemon_start failed, rc = %d", rc);
		}
	}
