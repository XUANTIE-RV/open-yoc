...
int wext_enable_testcmw500(const char *ifname, u8 fix_rate, u8 txpower, u8 pwrtrack_en)
{
	...
	if (iw_ioctl(ifname, SOICSITESTENABLE, &iwr) < 0) {
...
int wext_disable_testcmw500(const char *ifname)
{
	...
	int ret = 0;
	memset(&iwr, 0, sizeof(iwr));
	if (iw_ioctl(ifname, SOICSITESTDISABLE, &iwr) < 0) {
		RTW_API_INFO("\n\rioctl[SIOCSIWESSID] error");
		ret = -1;
	}
	return ret;
...
int wext_enable_btcolog(const char *ifname)
{
	struct iwreq iwr;
	int ret = 0;
	memset(&iwr, 0, sizeof(iwr));
	if (iw_ioctl(ifname, SOICSIBTCLENABLE, &iwr) < 0) {
		RTW_API_INFO("\n\rioctl[SIOCSIWESSID] error");
		ret = -1;
	}
	return ret;
...
int wext_disable_btcolog(const char *ifname)
{
	...
	int ret = 0;
	memset(&iwr, 0, sizeof(iwr));
	if (iw_ioctl(ifname, SOICSIBTCLDISABLE, &iwr) < 0) {
		RTW_API_INFO("\n\rioctl[SIOCSIWESSID] error");
		ret = -1;
	}
	return ret;
...
int wext_enable_rawdata_recv(const char *ifname, void *fun)
{
	...
	if (iw_ioctl(ifname, SIOCSIWENABLE, &iwr) < 0) {
		RTW_API_INFO("\n\rioctl[SIOCSIWESSID] error");
		ret = -1;
	}
	return ret;
...
int wext_disable_rawdata_recv(const char *ifname)
{
	...
	int ret = 0;
	memset(&iwr, 0, sizeof(iwr));
	if (iw_ioctl(ifname, SOICSIWDISABLE, &iwr) < 0) {
		RTW_API_INFO("\n\rioctl[SIOCSIWESSID] error");
		ret = -1;
	}
	return ret;
...
int wext_wowlan_ctrl(const char *ifname, int enable){
	...
	RTW_API_INFO("wext_wowlan_ctrl: enable=%d\n\r", enable);
	memset(&iwr, 0, sizeof(iwr));
...
int wext_wlan_redl_fw(const char *ifname){
	...
	RTW_API_INFO("+ wext_wlan_redl_fw\n\r");
	memset(&iwr, 0, sizeof(iwr));
	cmd_len = sizeof("wowlan_ctrl");
...
int wext_wowlan_unicast_wake_ctrl(const char *ifname, unsigned char enable){
	...
	RTW_API_INFO("wext_wowlan_unicast_wake_ctrl: enable=%d\n\r", enable);
	memset(&iwr, 0, sizeof(iwr));
	cmd_len = sizeof("wowlan_ctrl");
