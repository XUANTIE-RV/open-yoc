...
{
    tick_t          n_ticks;
    uint64_t        passed_nano_sec;
#if RHINO_CONFIG_TICKS_PER_SECOND == 1000
    uint32_t        ns_per_tick = 1000000;
#elif RHINO_CONFIG_TICKS_PER_SECOND == 100
    uint32_t        ns_per_tick = 10000000;
#else
    uint32_t        ns_per_tick = 1000000000 / RHINO_CONFIG_TICKS_PER_SECOND;
#endif
    static uint64_t ns_remain   = 0;

    if (cStateOneShotTimer[c_state_current]->one_shot_stop(&passed_nano_sec) !=
        0) {
...
#if RHINO_CONFIG_TICKS_PER_SECOND == 1000
    n_ticks = (tick_t)(passed_nano_sec / 1000000);
#elif RHINO_CONFIG_TICKS_PER_SECOND == 100
    n_ticks = (tick_t)(passed_nano_sec / 10000000);
#else
    n_ticks = (tick_t)((passed_nano_sec * RHINO_CONFIG_TICKS_PER_SECOND) / (1000000000));
#endif

    /* fixs ticks drift issue */
    ns_remain += passed_nano_sec % ns_per_tick;
    if (ns_remain >= ns_per_tick) {
        n_ticks++;
        ns_remain -= ns_per_tick;
    }
