...
/**
  \brief   No Operation with mem barrier
  \details No Operation does nothing. This instruction can be used for debugging purposes.
 */
__ALWAYS_STATIC_INLINE void __NOP_BARRIER(void)
{
    __ASM volatile("nop" ::
                       : "memory");
}
...
/**
  \brief   mem barrier for compiler
  \details This will not generate any actual instructions, but add mem barrier for compiler.
 */
__ALWAYS_STATIC_INLINE void __COMPILE_BARRIER(void)
{
    __ASM volatile("" ::
                       : "memory");
}
...
#else
__ALWAYS_STATIC_INLINE uint32_t __CLZ(uint32_t in)
{
    uint32_t result;
...
#else
    r = (abs((int32_t)(((x << 24) >> 24) - ((y << 24) >> 24)))) & 0x000000FF;
    s = (abs((int32_t)(((x << 16) >> 24) - ((y << 16) >> 24)))) & 0x000000FF;
    t = (abs((int32_t)(((x << 8) >> 24) - ((y << 8) >> 24)))) & 0x000000FF;
    u = (abs((int32_t)(((x) >> 24) - ((y) >> 24)))) & 0x000000FF;
#endif
    return (u + t + s + r + sum);
}
