...
static tls_fwup_status_callback g_fwup_status_callback = NULL;
...
request_finish:
					...
					if((fwup->updated_len >= (fwup->image_size)) || (request->status != TLS_FWUP_REQ_STATUS_SUCCESS))
					{
                        ...
                        if (g_fwup_status_callback)
					        g_fwup_status_callback(err_status);
...
void tls_fwup_reg_status_callback(tls_fwup_status_callback callback)
{
    g_fwup_status_callback = callback;
