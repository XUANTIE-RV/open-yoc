...
//typedef void (*pwm_irq_callback)(void *arg);
static void *pwm_callback_arg[PWM_CHANNEL_MAX_NUM] = {0};
static pwm_irq_callback pwm_callback[PWM_CHANNEL_MAX_NUM] = {0};
...
ATTRIBUTE_ISR void PWM_IRQHandler(void)
{
    ...
    value = tls_reg_read32(HR_PWM_INTSTS);

    if ((value & BIT(5)) || (value & BIT(6))) /* up */
    {
        for (i = 0; i < PWM_CHANNEL_MAX_NUM; i++)
        {
            if (pwm_callback[i])
                pwm_callback[i](pwm_callback_arg[i]);
        }
    }

    tls_reg_write32(HR_PWM_INTSTS, value);
}
 ...
 * @note           None
 */
void tls_pwm_isr_register(u8 channel, pwm_irq_callback callback, void *arg)
{
    pwm_callback_arg[channel] = arg;
    pwm_callback[channel] = callback;
...
u16 tls_pwm_get_clkdiv(u8 channel)
{
    u16 clkdiv;
    u32 temp = 0;
