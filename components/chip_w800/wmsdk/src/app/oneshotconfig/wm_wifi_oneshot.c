...
void wm_oneshot_net_status_event(uint32_t event_id, const void *data, void *context)
{
    if (oneshot_msg_q)
	{
...
u8 tls_wifi_oneshot_connect_by_ssid_bssid(u8 *ssid, u8 *bssid, u8 *pwd)
{
	...
	{	
		...
		event_subscribe(EVENT_NET_GOT_IP, wm_oneshot_net_status_event, NULL);
	}
...
void tls_oneshot_task_handle(void *arg)
{
    ...
    for(;;)
    {
        ...
        switch((u32)msg)
        {
           ...
           event_unsubscribe(EVENT_NET_GOT_IP, wm_oneshot_net_status_event, NULL);
...
#if TLS_CONFIG_AP_MODE_ONESHOT
		//tls_netif_add_status_event(wm_oneshot_netif_status_event);
        //event_subscribe(EVENT_NET_GOT_IP, wm_oneshot_netif_status_event, NULL);
		soft_ap_create();
#endif
	}
...
#if TLS_CONFIG_BT
__attribute__((weak)) tls_bt_status_t wm_bt_wifi_cfg_init()
{
	return TLS_BT_STATUS_FAIL;
}
__attribute__((weak)) tls_bt_status_t wm_bt_wifi_cfg_deinit()
{
	return TLS_BT_STATUS_FAIL;
}
#endif
