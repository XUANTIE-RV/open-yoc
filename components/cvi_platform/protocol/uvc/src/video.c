...
uvc_build_descriptors(struct uvc_format_info_st *format_info, uint32_t num_formats)
{
    ...
    unsigned int n_desc;
    unsigned int bytes;
    unsigned int config_desc_wTotal = 0;
    void *mem;
    ...
    bytes = uvc_device_descriptor.bLength
          + uvc_config_descriptor.bLength
          + uvc_iad.bLength + uvc_control_intf.bLength
          + uvc_streaming_intf_alt0.bLength
          + uvc_qual_descriptor.bLength;
    n_desc = 7;
    ...
    // not including the lenght of device descriptor
    config_desc_wTotal = bytes - uvc_device_descriptor.bLength
                       - uvc_qual_descriptor.bLength;
    ...
    uvc_config_header->wTotalLength = cpu_to_le16(config_desc_wTotal);
...
void uvc_destroy_descriptors(struct usb_descriptor_header **hdr)
{
    if (hdr != NULL) {
        free (hdr);
    }
