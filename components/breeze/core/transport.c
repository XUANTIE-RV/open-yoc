...
#define CONFIG_BREEZE_RX_THREAD_PRI (9)
#endif

static aos_task_t trans_rx_thread;
static aos_sem_t rx_sem;
static uint8_t rx_err_code = BZ_SUCCESS;
static uint8_t rx_err_src = BZ_SUCCESS;

static void transport_rx_dispatch_task(void * arg);
#endif
...
static ret_code_t send_fragment(void)
{
    ...
    do {
        ...
        if (g_transport.tx.active_func == ble_ais_send_indication)
            aos_mutex_unlock(g_transport.tx.mutex_indicate_done);
        if (ret != BZ_SUCCESS ||
...
ret_code_t bz_transport_init(ali_init_t const *p_init)
{
...
#if defined(CONFIG_BREEZE_RX_THREAD)
    {
        aos_sem_new(&rx_sem, 0);
        int ret = aos_task_new_ext(&trans_rx_thread, "breeze_rx", transport_rx_dispatch_task,
                                   NULL, CONFIG_BREEZE_RX_STACK_SIZE, CONFIG_BREEZE_RX_THREAD_PRI);
        if (ret < 0) {
            return BZ_ENOMEM;
        }
    }
    ...
    if (g_transport.tx.mutex_indicate_done == NULL) {
        g_transport.tx.mutex_indicate_done = aos_malloc(sizeof(aos_mutex_t));
        aos_mutex_new(g_transport.tx.mutex_indicate_done);
    }
...
static void transport_rx_dispatch_task(void * arg)
{
	BREEZE_VERBOSE("%s started", __func__);

	while (1) {
        if (!aos_sem_wait(&rx_sem, AOS_WAIT_FOREVER)) {
            if (rx_err_src != BZ_SUCCESS) {
                core_handle_err(rx_err_src, rx_err_code);
                rx_err_src = BZ_SUCCESS;
                rx_err_code = BZ_SUCCESS;
            } else {
                trans_rx_dispatcher();
                reset_rx();
            }
...
static void tr_core_handle_err(uint8_t src, uint8_t code)
{
    reset_rx();
    rx_err_src = src;
    rx_err_code = code;
    aos_sem_signal(&rx_sem);
}
#endif
...
void transport_rx(uint8_t *p_data, uint16_t length)
{
    ...
    if (length == 0) {
        ...
        tr_core_handle_err(ALI_ERROR_SRC_TRANSPORT_RX_BUFF_SIZE, BZ_EDATASIZE);
        return;
    }

    if (!rx_frames_left()) {
        if (FRAME_SEQ(p_data) != 0) {
            tr_core_handle_err(ALI_ERROR_SRC_TRANSPORT_1ST_FRAME, BZ_EINVALIDDATA);
            return;
        }
        ...
        if ((g_transport.rx.msg_id != MSG_ID(p_data)) ||
            ...
            tr_core_handle_err(ALI_ERROR_SRC_TRANSPORT_OTHER_FRAMES, BZ_EINVALIDDATA);
            return;
        } else if (((g_transport.rx.frame_seq + 1) & 0xF) != FRAME_SEQ(p_data) &&
                    g_transport.rx.cmd == BZ_CMD_OTA_DATA) {
            tr_core_handle_err(ALI_ERROR_SRC_TRANSPORT_FW_DATA_DISC, BZ_EINVALIDDATA);
            return;
        } else {
    ...
    if (IS_ENC(p_data) != 0) {
        if ((length - HEADER_SIZE) % 16 != 0) {
            tr_core_handle_err(ALI_ERROR_SRC_TRANSPORT_ENCRYPTED, BZ_EINVALIDDATA);
            return;
        }
        if (g_transport.p_key == NULL) {
            tr_core_handle_err(ALI_ERROR_SRC_TRANSPORT_ENCRYPTED, BZ_EFORBIDDEN);
            return;
        }
    }
    ...
    if ((length != HEADER_SIZE + FRAME_LEN(p_data) && IS_ENC(p_data) == 0)
        ...
        tr_core_handle_err(ALI_ERROR_SRC_TRANSPORT_OTHER_FRAMES, BZ_EDATASIZE);
        return;
    }
