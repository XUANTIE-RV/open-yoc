...
void bz_mbedtls_aes_init(mbedtls_aes_context *ctx)
{
    memset(ctx, 0, sizeof(mbedtls_aes_context));
}

void bz_mbedtls_aes_free(mbedtls_aes_context *ctx)
{
    if (ctx == NULL)
        return;
...
int bz_mbedtls_aes_setkey_enc(mbedtls_aes_context *ctx, const unsigned char *key,
...
int bz_mbedtls_aes_setkey_dec(mbedtls_aes_context *ctx, const unsigned char *key,
                           unsigned int keybits)
{
    int                 i, j, ret;
    ...
    /* Also checks keybits */
    if ((ret = bz_mbedtls_aes_setkey_enc(&cty, key, keybits)) != 0)
        goto exit;
 ...
 */
int bz_mbedtls_aes_crypt_ecb(mbedtls_aes_context *ctx, int mode,
                          const unsigned char input[16],
                          unsigned char       output[16])
{
#if defined(MBEDTLS_AESNI_C) && defined(MBEDTLS_HAVE_X86_64)
 ...
 */
int bz_mbedtls_aes_crypt_cbc(mbedtls_aes_context *ctx, int mode, size_t length,
                          ...
                          unsigned char *output)
{
    ...
    if (mode == MBEDTLS_AES_DECRYPT) {
        while (length > 0) {
            ...
            bz_mbedtls_aes_crypt_ecb(ctx, mode, input, output);
        ...
        while (length > 0) {
            ...
            bz_mbedtls_aes_crypt_ecb(ctx, mode, output, output);
            memcpy(iv, output, 16);
                             ...
                             const unsigned char *input, unsigned char *output)
{
    ...
    if (mode == MBEDTLS_AES_DECRYPT) {
        while (length--) {
            if (n == 0)
                bz_mbedtls_aes_crypt_ecb(ctx, MBEDTLS_AES_ENCRYPT, iv, iv);
 ...
 */
int bz_mbedtls_aes_crypt_ctr(mbedtls_aes_context *ctx, size_t length,
                          ...
                          const unsigned char *input, unsigned char *output)
{
    ...
    while (length--) {
        if (n == 0) {
            bz_mbedtls_aes_crypt_ecb(ctx, MBEDTLS_AES_ENCRYPT, nonce_counter,
                                  stream_block);
...
int mbedtls_aes_self_test(int verbose)
{
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (v == MBEDTLS_AES_DECRYPT) {
            bz_mbedtls_aes_setkey_dec(&ctx, key, 128 + u * 64);

            for (j = 0; j < 10000; j++)
                bz_mbedtls_aes_crypt_ecb(&ctx, v, buf, buf);
            ...
            bz_mbedtls_aes_setkey_enc(&ctx, key, 128 + u * 64);
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (v == MBEDTLS_AES_DECRYPT) {
            ...
            for (j = 0; j < 10000; j++)
                bz_mbedtls_aes_crypt_cbc(&ctx, v, 16, iv, buf, buf);
    ...
    for (i = 0; i < 6; i++) {
        ...
        offset = 0;
        bz_mbedtls_aes_setkey_enc(&ctx, key, 128);

        if (v == MBEDTLS_AES_DECRYPT) {
            len = aes_test_ctr_len[u];
            memcpy(buf, aes_test_ctr_ct[u], len);

            bz_mbedtls_aes_crypt_ctr(&ctx, len, &offset, nonce_counter,
