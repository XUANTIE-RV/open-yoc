...
static int lex_process(const char *jpath, slist_t *lex_list)
{
    ...
    do {
        ...
        if (wordbegin == NULL || wordlen <= 0) {
            LOGE(TAG, "cJPath Err1 at %s", pre_pos);
            break;
        }
        ...
        if (sub_opt_status_idx >= 0) {
            ...
            if ((sub_opt_status[sub_opt_status_idx + 1] == *wordbegin) ||
                ...
                if (isdigit(*wordbegin)) {
                    ...
                    ret = -1;
                    LOGE(TAG, "cJPath Err2 at %s", pre_pos);
                    break;
                }
            } else {
                ret = -1;
                LOGE(TAG, "cJPath Err3 at %s", pre_pos);
                break;
            }
            ...
            switch (sub_opt_status_idx) {
                ...
                default:
                    ...
                    cur_node = NULL;
                    ret      = -1;
                    LOGE(TAG, "cJPath Err4 at %s", pre_pos);
            }
        } else {
            /* 路径状态解析 */
            switch (*wordbegin) {
        ...
        */
        ...
        if (node->path != NULL) {
            ...
            if (cJSON_IsArray(jsquery)) {
                if (strcmp(node->query, "?") == 0) {
                    ...
                    for (int i = 0; i < arr_size; i++) {
                        ...
                        if (cJSON_IsString(qval)) {
                            ...
                            LOGW(TAG, "Non-string queries are not supported");
                        }
                    }
                ...
                LOGE(TAG, "Object<%s> is not an array", jsquery ? jsquery->string : "null");
            }
        } /* query */
    }     /* for each */
