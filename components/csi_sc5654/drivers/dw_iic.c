...
#define IIC_BUSY_TIMEOUT    0x1000000
...
static uint32_t drv_get_iic_freq(void)
{
    return SYSTEM_CLOCK;
}

static void dw_iic_set_hcnt(dw_iic_reg_t *addr, DWENUM_IIC_SPEED speed)
{
    switch (speed) {
        case DW_IIC_STANDARDSPEED:
        addr->IC_SS_SCL_HCNT = (drv_get_iic_freq() / 1000000) * 4;
        break;
        case DW_IIC_FASTSPEED:
        addr->IC_FS_SCL_HCNT = (drv_get_iic_freq() / 1000000) * 1;
        break;
        case DW_IIC_HIGHSPEED:
        addr->IC_HS_SCL_HCNT = ((drv_get_iic_freq() / 1000000) / 10) * 1;
        break;
        default:
        break;
    }
...
static void dw_iic_set_lcnt(dw_iic_reg_t *addr, DWENUM_IIC_SPEED speed)
{
    switch (speed) {
        case DW_IIC_STANDARDSPEED:
        addr->IC_SS_SCL_LCNT = (drv_get_iic_freq() / 1000000) * 5;
        break;
        case DW_IIC_FASTSPEED:
        addr->IC_FS_SCL_LCNT = (drv_get_iic_freq() / 1000000) * 1;
        break;
        case DW_IIC_HIGHSPEED:
        addr->IC_HS_SCL_HCNT = ((drv_get_iic_freq() / 1000000) / 10) * 1;
        break;
...
static void dw_iic_intr_tx_empty(int32_t idx, dw_iic_priv_t *iic_priv, uint32_t intr_stat)
{
    ...
    if (intr_stat & DW_IIC_INTR_TX_EMPTY) {
        ...
        while (addr->IC_STATUS & DW_IIC_TXFIFO_EMPTY) {
            timeout ++;

            if (timeout > IIC_BUSY_TIMEOUT) {
                if (iic_priv->cb_event) {
                    iic_priv->cb_event(idx, IIC_EVENT_TRANSFER_INCOMPLETE);
                }
...
iic_handle_t csi_iic_initialize(int32_t idx, iic_event_cb_t cb_event)
{
    ...
    silan_pic_request(PIC_IRQID_MISC, irq, (hdl_t)dw_iic_irqhandler);
...
int32_t csi_iic_uninitialize(iic_handle_t handle)
{
    ...
    drv_clk_disable(CLK_I2C1 + iic_priv->idx);
...
int32_t csi_iic_config_speed(iic_handle_t handle, iic_speed_e speed)
{
    ...
    switch (speed) {
        case IIC_BUS_SPEED_STANDARD:
            ...
            dw_iic_set_hcnt(addr, DW_IIC_STANDARDSPEED);
            dw_iic_set_lcnt(addr, DW_IIC_STANDARDSPEED);
            break;

        case IIC_BUS_SPEED_FAST:
            dw_iic_set_transfer_speed(addr, DW_IIC_FASTSPEED);
            dw_iic_set_hcnt(addr, DW_IIC_FASTSPEED);
            dw_iic_set_lcnt(addr, DW_IIC_FASTSPEED);
            break;
        ...
        case IIC_BUS_SPEED_HIGH:
            ...
            dw_iic_set_hcnt(addr, DW_IIC_HIGHSPEED);
            dw_iic_set_lcnt(addr, DW_IIC_HIGHSPEED);
            break;
