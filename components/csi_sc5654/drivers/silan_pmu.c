...
SRAM_TEXT static int silan_pre_sysclk_sel(silan_sysclk_ref_t sysclk_ref, silan_syspllsel_t if_clk_sel)
{
    uint32_t tmp;
    uint8_t retry = 100;
    sysclk_regs_protect_open();
...
SRAM_TEXT void silan_sysclk_change(silan_sysclk_ref_t sysclk_ref, silan_syspllsel_t if_clk_sel)
{
    if (sysclk_ref == SYSCLK_FROM_PLL) {
        silan_pre_sysclk_sel(SYSCLK_FROM_RCH, 0);
    }

    silan_pre_sysclk_sel(sysclk_ref, if_clk_sel);
}
