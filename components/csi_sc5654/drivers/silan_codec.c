...
static void silan_codec_power_up(silan_codec_t codec)
{
    ...
    while (regs->R9 != 0x87);
    regs->DA_R3 = 0x9f;
    while (regs->DA_R3 != 0x9f);
    regs->DA_R1 = 0x4a;
    while (regs->DA_R1 != 0x4a);
    regs->DA_R6 = 0x2c;
    while (regs->DA_R6 != 0x2c);
    regs->DA_R2 = 0x40;
    while (regs->DA_R2 != 0x40);
    // regs->DA_R4 = 0x02;
    // while (regs->DA_R4 != 0x02);
    // regs->DA_R0 = 0x08;
    // while (regs->DA_R0 != 0x08);
    // regs->DA_R0 = 0x28;
    // while (regs->DA_R0 != 0x28);
    regs->DA_R0 = 0x38;
    while (regs->DA_R0 != 0x38);
    // regs->DA_R4 = 0x16;
    // while (regs->DA_R4 != 0x16);
    regs->DA_R4 = 0x7e;
    while (regs->DA_R4 != 0x7e);
    // regs->DA_R1 = 0x42;
    // while (regs->DA_R1 != 0x42);
    regs->DA_R1 = 0x02;
    while (regs->DA_R1 != 0x02);
    // regs->DA_R2 = 0x41;
    // while (regs->DA_R2 != 0x41);
    // regs->DA_R2 = 0x45;
    // while (regs->DA_R2 != 0x45);
    regs->DA_R2 = 0x47;
    while (regs->DA_R2 != 0x47);
    regs->DA_R0 = 0x3c;
    while (regs->DA_R0 != 0x3c);
    regs->DA_R2 = 0x40;
    while (regs->DA_R2 != 0x40);
    regs->DA_R1 = 0x4A;
    while (regs->DA_R1 != 0x4A);
    ...
    while (regs->DA_R4 != 0x16);
    regs->DA_R0 = 0x3f;
    while (regs->DA_R0 != 0x3f);
    regs->DA_R1 = 0x02;
    while (regs->DA_R1 != 0x02);
    regs->DA_R4 = 0x00;
    while (regs->DA_R4 != 0x00);
#endif
