...
int32_t silan_adev_write_clear(void *pdev)
{
    ADEV_CFG *adev_cfg = (ADEV_CFG *)pdev;

    return silan_buf_clear(adev_cfg->tx_dma_req->buf_t);
}

int32_t silan_adev_read_clear(void *pdev)
{
    ADEV_CFG *adev_cfg = (ADEV_CFG *)pdev;

    return silan_buf_clear(adev_cfg->rx_dma_req->buf_t);
}
