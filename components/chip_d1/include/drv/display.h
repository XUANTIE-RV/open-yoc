...
struct disp_tv_func {
	int (*tv_enable)(uint32_t sel);
	int (*tv_disable)(uint32_t sel);
	int (*tv_suspend)(uint32_t sel);
	int (*tv_resume)(uint32_t sel);
	int (*tv_get_mode)(uint32_t sel);
	int (*tv_set_mode)(uint32_t sel, enum disp_tv_mode tv_mod);
	int (*tv_get_input_csc)(uint32_t sel);
	int (*tv_get_video_timing_info)(uint32_t sel,
					 struct disp_video_timings **
					 video_info);
	int (*tv_mode_support)(uint32_t sel, enum disp_tv_mode mode);
	int (*tv_hot_plugging_detect)(uint32_t state);
	int (*tv_set_enhance_mode)(uint32_t sel, uint32_t mode);
	int (*tv_irq_enable)(uint32_t sel, uint32_t irq_id, uint32_t en);
	int (*tv_irq_query)(uint32_t sel);
	unsigned int (*tv_get_cur_line)(uint32_t sel);
	int (*vdpo_set_config)(uint32_t sel, struct disp_vdpo_config *p_cfg);
	int (*tv_get_startdelay)(uint32_t sel);
	void (*tv_show_builtin_patten)(uint32_t sel, uint32_t patten);
};
...
struct disp_device_func {
	...
	int (*mode_support)(uint32_t mode);
	...
	/*for hdmi cec*/
	int32_t (*cec_standby_request)(void);
	int32_t (*cec_send_one_touch_play)(void);
};
