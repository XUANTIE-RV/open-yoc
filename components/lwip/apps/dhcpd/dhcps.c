...
int dhcps_get_client_number()
{
    ...
    for (i = 0; i < 256; i++) {
        ...
        if (p[0]+p[1]+p[2] != 0) {
            client_number++;
            printf("DHCP Client[%d] %02x:%02x:%02x:%02x:%02x:%02x\n",
                i, p[0], p[1], p[2], p[3], p[4], p[5]);
        }
    }
...
#if (debug_dhcps)
    printf("\r\n[%s] dst=0x%x src=0x%x option_length=%d \r\n", __func__, option_length);
#endif
...
static void dhcps_send_offer(struct pbuf *packet_buffer)
{
    ...
    LOGD(TAG, "assign ip = %d.%d.%d.%d", ip4_addr1(&dhcps_network_id), ip4_addr2(&dhcps_network_id), ip4_addr3(&dhcps_network_id), temp_ip);
...
#endif
                break;
            case DHCP_OPTION_CODE_HOST_NAME: {
                    char *buffer = aos_zalloc(option_start[1] + 1);
                    if (buffer) {
                        memcpy(buffer, option_start + 2, option_start[1] );
...
void dhcps_init(struct netif *pnetif)
{
    ...
    LOGD(TAG, "DHCP Server init on %c%c", pnetif->name[0], pnetif->name[1]);
