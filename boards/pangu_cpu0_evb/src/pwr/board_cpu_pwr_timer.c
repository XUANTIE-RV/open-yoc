...
extern int32_t drv_get_timer_freq(int32_t idx);
...
#ifdef CONFIG_CSI_V2
void timer_event_cb(csi_timer_t *timer, void *arg)
{
#ifdef TIM_DBG
    printf("time out\n");
#endif
...
kstat_t tim_timer_init(void)
{
    ...
    krhino_spin_lock_init(&tim_spin);
#ifdef CONFIG_CSI_V1
...
#ifdef CONFIG_CSI_V2
    csi_timer_init(&wakeup_timer, CONFIG_LPM_TICKLESS_SYSTIM);
    csi_timer_attach_callback(&wakeup_timer, timer_event_cb, NULL);
#endif
...
)
{
    ...
    krhino_spin_lock_irq_save(&tim_spin, flags_cpsr);
#ifdef TIM_DBG
...
#ifdef CONFIG_CSI_V2
    csi_timer_start(&wakeup_timer, planUs);
#endif
    krhino_spin_unlock_irq_restore(&tim_spin, flags_cpsr);
    return RHINO_SUCCESS;
...
#ifdef CONFIG_CSI_V2
    load_val = csi_timer_get_load_value(&wakeup_timer);
    current_val = csi_timer_get_remaining_value(&wakeup_timer);
#endif
...
#endif
#ifdef CONFIG_CSI_V2
    csi_timer_detach_callback(&wakeup_timer);
    csi_timer_stop(&wakeup_timer);
#endif
    krhino_spin_unlock_irq_restore(&tim_spin, flags_cpsr);
    return RHINO_SUCCESS;
