...
#ifdef CONFIG_HAL_UART_0_RINGBUF_LEN
static char uart_recv_buffer_0[CONFIG_HAL_UART_0_RINGBUF_LEN];
#endif

#ifdef CONFIG_HAL_UART_1_RINGBUF_LEN
static char uart_recv_buffer_1[CONFIG_HAL_UART_1_RINGBUF_LEN];
#endif

#ifdef CONFIG_HAL_UART_2_RINGBUF_LEN
static char uart_recv_buffer_2[CONFIG_HAL_UART_2_RINGBUF_LEN];
#endif

#ifdef CONFIG_HAL_UART_3_RINGBUF_LEN
static char uart_recv_buffer_3[CONFIG_HAL_UART_3_RINGBUF_LEN];
#endif

#ifdef CONFIG_HAL_UART_4_RINGBUF_LEN
static char uart_recv_buffer_4[CONFIG_HAL_UART_4_RINGBUF_LEN];
#endif
...
static int set_uart_recv_buffer(uint8_t port)
{
  switch (port) {
    case 0 :
#ifdef CONFIG_HAL_UART_0_RINGBUF_LEN
    uart_list[port].recv_buf = uart_recv_buffer_0;
    return 0;
#endif
        break;
		case 1 :
#ifdef CONFIG_HAL_UART_1_RINGBUF_LEN
		uart_list[port].recv_buf = uart_recv_buffer_1;
		return 0;
#endif
	  break;
	  case 2 :
#ifdef CONFIG_HAL_UART_2_RINGBUF_LEN
	  uart_list[port].recv_buf = uart_recv_buffer_2;
	  return 0;
#endif
	  break;
	  case 3 :
#ifdef CONFIG_HAL_UART_3_RINGBUF_LEN
	  uart_list[port].recv_buf = uart_recv_buffer_3;
	  return 0;
#endif
	  break;
	  case 4 :
#ifdef CONFIG_HAL_UART_4_RINGBUF_LEN
	  uart_list[port].recv_buf = uart_recv_buffer_4;
	  return 0;
#endif
      default:
      break;
  }
   uart_list[port].recv_buf = (char *)aos_malloc(HAL_UART_RINGBUF_LEN);
   if(!uart_list[port].recv_buf) {
      return -1;
   }
   return 0;
}

static int clear_uart_recv_buffer(uint8_t port)
{
  switch (port) {
    case 0 :
#ifdef CONFIG_HAL_UART_0_RINGBUF_LEN
   ...
   aos_free(uart_list[port].recv_buf);
...
int32_t hal_uart_init(uart_dev_t *uart)
{
	...
	ret = set_uart_recv_buffer(uart->port);
	if(ret != 0 ) {
       return -1;
	}
...
int32_t hal_uart_recv_poll(uart_dev_t *uart, void *data, uint32_t expect_size)
{
    ...
    while (recv_num < expect_size){
        csi_usart_getchar(uart->priv, temp_data);
            recv_num++;
            temp_data++;
    }
...
int32_t hal_uart_finalize(uart_dev_t *uart)
{
	...
	clear_uart_recv_buffer(uart->port);
