...
#ifndef __AOS_AUDIO_PCM_H__
#define __AOS_AUDIO_PCM_H__

#define AOS_AUDIO_PCM_DIR_OUT 0
#define AOS_AUDIO_PCM_DIR_IN  1
...
typedef struct {
    ...
    int period_size;
    int period_count;
    aos_audio_pcm_format_t format;
} aos_audio_pcm_config_t;

typedef struct {
    aos_audio_pcm_config_t config;
    unsigned char   dir:1;
    unsigned char   card:2;
    unsigned char   state:3;
    void *private_data;
} aos_audio_pcm_device_t;
...
typedef enum {
    ...
    AOS_SND_DEVICE_MAX,
} aos_audio_snd_device_t;

typedef struct {
    aos_audio_snd_device_t out_device;
    int external_pa;
    int external_pa_pin;
 ...
 *  -1 -- failed
 */
int aos_audio_set_path(aos_audio_pcm_device_t *pcm, aos_audio_snd_device_t device);
