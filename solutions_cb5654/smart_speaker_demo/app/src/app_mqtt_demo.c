...
// for demo only
// #define PRODUCT_KEY             "a1Jne1b3r08"
// #define PRODUCT_SECRET          "MGSHz7yKGz2RlPQ4"
// #define DEVICE_NAME             "tPp7LhWw3OJHvMFnev0w"
// #define DEVICE_SECRET           "sWnQ0aP8r92UcEmVf7YKDh5n863vKVX9"
...
{
    printf("Down Raw Message, Devid: %d, Payload Length: %d", devid, payload_len);
    return 0;
}
        ...
        char **response, int *response_len)
{
    ...
    printf("Service Request Received, Devid: %d, Service ID: %.*s, Payload: %s", devid, serviceid_len,
                  serviceid,
                  request);
...
static int user_property_set_event_handler(const int devid, const char *request, const int request_len)
{
    ...
    printf("Post Property Message ID: %d", res);
        ...
        int *response_len)
{
    ...
    for (index = 0; index < cJSON_GetArraySize(request_root); index++) {
        ...
        printf("Property ID, index: %d, Value: %s", index, item_propertyid->valuestring);
    ...
    *response_len = strlen(*response);
    ...
    printf("Property Get Response: %s", *response);
        ...
        const int reply_len)
{
    ...
    printf("Message Post Reply Received, Devid: %d, Message ID: %d, Code: %d, Reply: %.*s", devid, msgid, code,
                  reply_value_len,
                  reply_value);
    return 0;
}
...
{
    printf("Trigger Event Reply Received, Devid: %d, Message ID: %d, Code: %d, EventID: %.*s, Message: %.*s", devid,
                  msgid, code,
                  eventid_len,
...
void user_post_property(void)
{
    ...
    if (example_index % 3== 0) {
        /* Normal Example */
        property_payload = "{\"LightSwitch\":1, \"LightMode\":0}";
        example_index++;
    } else if (example_index % 3 == 1) {
        /* Wrong Property ID */
        property_payload = "{\"LightSwitch\":0, \"LightMode\":1}";
        example_index++;
    } else if (example_index % 3 == 2) {
        /* Wrong Value Format */
        property_payload = "{\"LightSwitch\":1, \"LightMode\":2}";
        example_index++;
    }
...
void user_post_event(void)
{
    ...
    printf("Post Event Message ID: %d", res);
}
...
void user_deviceinfo_delete(void)
{
    ...
    printf("Device Info Delete Message ID: %d", res);
}
...
void user_post_raw_data(void)
{
    ...
    res = IOT_Linkkit_Report(user_example_ctx->master_devid, ITM_MSG_POST_RAW_DATA,
                             raw_data, 7);
    printf("Post Raw Data Message ID: %d", res);
}
...
static void linkkit_thread(void *paras)
{
    ...
    while (1) {
        ...
        if (max_running_seconds && (time_now_sec - time_begin_sec > max_running_seconds)) {
            printf("Example Run for Over %d Seconds, Break Loop!\n", max_running_seconds);
            break;
        }
...
{
    aos_task_t tsk;
    aos_task_new_ext(&tsk, "mqttkit", linkkit_thread, NULL, 1024*8, AOS_DEFAULT_APP_PRI);
    return 0;
}
