...
extern int32_t target_get_aes(int32_t idx, uint32_t *base, uint32_t *irq, void **handler);
...
static int32_t aes_set_key(void *context, uint8_t *key, aes_key_len_bits_e keylen, uint32_t enc, uint32_t endian)
{
    uint8_t keynum = 0;
...
static void aes_get_iv(uint32_t mode, uint32_t endian, uint8_t *iv)
{
    uint32_t temp;
    uint32_t i;
    ...
    for (i = 0; i < len; i = i + 16) {
        for (j = 0; j < 4; j++) {
            if (endian == AES_ENDIAN_BIG) {
                temp[j] = in[0] << 24 | in[1] << 16 | in[2] << 8 | in[3];
            } else {
        ...
        while (aes_get_intstatus(AES_IT_BUSY)) {
            if (!(timeout--)) {
                return ERR_AES(DRV_ERROR_TIMEOUT);
            }
        }
        ...
        for (j = 0; j < 4; j++) {
            temp[0] = aes_reg->DATAOUT[3 - j];
            if (endian == AES_ENDIAN_BIG) {
                out[0] = (temp[0] >> 24) & 0xff;
                out[1] = (temp[0] >> 16) & 0xff;
                out[2] = (temp[0] >>  8) & 0xff;
                out[3] = (temp[0] >>  0) & 0xff;
            } else {
                out[3] = (temp[0] >> 24) & 0xff;
                out[2] = (temp[0] >> 16) & 0xff;
                out[1] = (temp[0] >>  8) & 0xff;
                out[0] = (temp[0] >>  0) & 0xff;
            }
            out += 4;
...
#ifdef CONFIG_LPM
static void manage_clock(aes_handle_t handle, uint8_t enable)
{
    drv_clock_manager_config(CLOCK_MANAGER_AES, enable);
}

static void do_prepare_sleep_action(aes_handle_t handle)
{
...
static void do_wakeup_sleep_action(aes_handle_t handle)
{
    ck_aes_priv_t *aes_priv = handle;
    uint32_t *abase = (uint32_t *)(aes_priv->base);
...
aes_handle_t csi_aes_initialize(int32_t idx, aes_event_cb_t cb_event)
{
    ...
    int32_t real_idx = target_get_aes(idx, &base, &irq, &handler);
...
#ifdef CONFIG_LPM
    csi_aes_power_control(aes_priv, DRV_POWER_FULL);
#endif
#if 0 // FIXME:
    aes_enable_interrupt();             /* enable the aes interrupt */

    drv_irq_register(aes_priv->irq, handler);
    drv_irq_enable(aes_priv->irq); /* enable the aes bit in vic */
#endif
...
int32_t csi_aes_uninitialize(aes_handle_t handle)
{
    ...
    drv_irq_disable(aes_priv->irq);
    drv_irq_unregister(aes_priv->irq);

#ifdef CONFIG_LPM
    csi_aes_power_control(aes_priv, DRV_POWER_OFF);
#endif
