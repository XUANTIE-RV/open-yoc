...
static int user_connected_event_handler(void)
{
    ...
    if (gateway_ctx->master_initialized == 1) { // initialized
        app_gateway_ut_update_subdev(GW_TOPO_GET_REASON_CONNECT_CLOUD);
    }
...
static int user_service_request_event_handler(const int devid, const char *serviceid, const int serviceid_len,
                                              ...
                                              int *response_len)
{
    gateway_info("Service Request Received, Devid: %d, Service ID: %.*s, Payload: %s", devid, serviceid_len, serviceid,
                 request);
...
{
    return app_gateway_ut_handle_property_set(devid, request, request_len);
}
...
{
    gateway_info("Trigger Event Reply Received, Devid: %d, Message ID: %d, Code: %d, EventID: %.*s, Message: %.*s",
...
static int user_initialized(const int devid)
{
    ...
    if (devid > 0) { // subdev
        app_gateway_subdev_registered_handle(devid);
    }
    ...
    if (!strcmp(item->valuestring, "awss.BindNotify")) {
        ...
        if (op != NULL && cJSON_IsString(op)) {
            if (!strcmp(op->valuestring, "Bind")) {
                ...
                app_gateway_subdev_binded_handle(devid);
                ...
                if (devid > 0) { // User unbind subdev
                    ...
                    app_gateway_ut_send_msg(&gw_msg, sizeof(gateway_msg_t));
                }
...
static int user_event_notify_handler(const int devid, const char *request, const int request_len)
{
    ...
    res = IOT_Linkkit_Report(devid, ITM_MSG_EVENT_NOTIFY_REPLY, (unsigned char *)request, request_len);
...
// get topo list reply
static int user_topolist_reply_handler(const int devid, const int id, const int code, const char *payload,
                                       const int payload_len)
{
    gateway_info("Receive topolist reply, code:%d, payload:%s", code, payload);

    if (code == 200 && payload && payload_len > 0) {
        app_gateway_ut_handle_topolist_reply(payload, payload_len);
    }
...
static int user_permit_join_event_handler(const char *product_key, const int time)
{
    ...
    app_gateway_ut_handle_permit_join();
...
static int user_subdev_misc_reply_handler(const int devid, const int event_id, const int code, const char *payload,
                                          const int payload_len)
{
...
static int user_fota_event_handler(int type, const char *version)
{
    ...
    if (type == 0) {
        ...
        extern void app_smartliving_ota_prepare(const char *version);
        app_smartliving_ota_prepare(version);

        memset(p_fota_buffer, 0, GATEWAY_OTA_BUFFER_LEN);
        IOT_Linkkit_Query(gateway_ctx->master_devid, ITM_MSG_QUERY_FOTA_DATA, (unsigned char *)p_fota_buffer,
                          GATEWAY_OTA_BUFFER_LEN);
    }
                                   ...
                                   const char *sign, const char *sign_method, const char *url)
{
    ...
    if (type == 0) {
        ...
        IOT_Linkkit_Query(gateway_ctx->master_devid, ITM_MSG_QUERY_COTA_DATA, (unsigned char *)p_cota_buffer,
                          GATEWAY_OTA_BUFFER_LEN);
    }
...
static int user_unified_service_post_reply_handler(const int devid, const int id, const int code, const char *payload,
                                                   const int payload_len)
{
...
void *user_dispatch_yield(void *args)
{
    ...
    while ((gateway_ctx->g_user_dispatch_thread_running) && (gateway_ctx->started)) {
...
static iotx_linkkit_dev_meta_info_t *gateway_main_init(gateway_ctx_t *gateway_ctx)
{
    ...
    if ((0 == strlen(p_master_meta->product_key)) || (0 == strlen(p_master_meta->device_name))
        || (0 == dynamic_register && (0 == strlen(p_master_meta->device_secret)))
        || (0 == strlen(p_master_meta->product_secret)))
    {
        while (1) {
...
int gateway_main(void *paras)
{
    ...
    } while ((gateway_ctx->master_devid < 0) && (gateway_ctx->started == 1));
    ...
    } while ((res < 0) && (gateway_ctx->started == 1));
    ...
    while (gateway_ctx->started == 1) {
        ...
        if (user_master_dev_available()) {
            app_gateway_ut_msg_process(gateway_ctx->master_devid, GATEWAY_YIELD_TIMEOUT_MS / 2);
        }
    ...
    aos_msleep(1000); // wait for IOT_Linkkit_Yield finished before release IOT sources
...
static int user_get_iot_info(iotx_linkkit_dev_meta_info_t *iot_info)
{
    ...
    if (!HAL_GetProductKey(master_meta_info.product_key) || !HAL_GetDeviceName(master_meta_info.device_name)
...
int smartliving_client_control(const int start)
{
    ...
    if (start) {
        if (gateway_ctx->started == 1) {
            gateway_warn("gateway has started");
            return 0;
        }

        if (user_get_iot_info(NULL) < 0) {
            gateway_err("missing iot devinfo\n");
            return -1;
        }

        ret = aos_task_new("gateway_main", (void (*)(void *))gateway_main, NULL, 8 * 1024);
        return ret;
    } else {
        if (gateway_ctx->started == 0) {
            gateway_warn("gateway not started");
            return 0;
        }

        gateway_ctx->started = 0;
    }
    return ret;
}
...
int user_at_post_property(int device_id, char *message, int len)
{
    ...
    return IOT_Linkkit_Report(device_id, ITM_MSG_POST_PROPERTY, (unsigned char *)message, len);
}
...
int user_at_post_event(int device_id, char *evtid, int evtid_len, char *evt_payload, int len)
{
    ...
    return IOT_Linkkit_TriggerEvent(gateway_get_ctx()->master_devid, evtid, evtid_len, evt_payload, len);
}
