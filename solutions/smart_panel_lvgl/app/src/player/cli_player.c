...
{   
    ...
    while (1) {
        ...
        if (state == SMTAUDIO_STATE_STOP) {
            //printf("====####test\r\n");
            LOGD(TAG, "loop play url %s", (char *)arg);
           smtaudio_start(MEDIA_MUSIC, (char *)arg, 0, 0);
        }
        aos_msleep(1000);
    }
}
...
void play_sintest(int sec)
{
    ...
    aos_task_new_ext(&task_handle, "sin", task_play_sin, (void *)second, 4096,
                        AOS_DEFAULT_APP_PRI);
}
...
static int cli_player_proc(int argc, char **argv)
{
    ...
    //smta action pause/resume/next/previous/play
    if (strcmp(argv[1], "action") == 0){
        if(argc >= 3){
            ...
            if(strcmp(action,"pause")==0){
                ...
                printf("smta action pause/resume/next/previous/play\n");
            }
        }
        else{
            printf("smta action pause/resume/next/previous/play\n");
        ...
        printf("set out gain %d\r\n", gain);
        board_audio_out_set_gain(id, gain);
        ...
        static char * loop_url = NULL;

        if (argc == 3) {
            if (loop_url != NULL) {
                free(loop_url);
            }
            loop_url = strdup(argv[2]);
            printf("loop play url=%s\n", loop_url);
        ...
        if (0 == g_task_loop_play_is_run) {
            ...
            if (SMTAUDIO_STATE_MUTE != smtaudio_get_state()) {
                aos_task_new_ext(&g_loop_task, "loop_play", audio_loop_proc_task, loop_url, 4096, AOS_DEFAULT_APP_PRI + 4);
            } else {
                printf("Device is mute\n");
            }
        } else {
