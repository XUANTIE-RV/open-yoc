...
#define APP_WIFI_USER_SCAN_SSID_MAX 64

rvm_hal_wifi_ap_record_t g_app_wifi_user_scan_table[APP_WIFI_USER_SCAN_SSID_MAX];
...
static void app_user_scan_compeleted(rvm_dev_t *dev, uint16_t number, rvm_hal_wifi_ap_record_t *ap_records)
{
    rvm_hal_wifi_ap_record_t wifiApRecord;

    memset(g_app_wifi_user_scan_table, 0, sizeof(g_app_wifi_user_scan_table));
    ...
    LOGI(TAG, "number of ssid %d", number);

    if(number <= APP_WIFI_USER_SCAN_SSID_MAX)
    {
        memcpy(g_app_wifi_user_scan_table, ap_records, (number * sizeof(rvm_hal_wifi_ap_record_t)));   
    }
    else
    {
        memcpy(g_app_wifi_user_scan_table, ap_records, (APP_WIFI_USER_SCAN_SSID_MAX * sizeof(rvm_hal_wifi_ap_record_t)));      
    }

    aos_sem_signal(&g_wifi_config_sem);

    app_event_update(EVENT_GUI_USER_WIFI_SCAN_FINISH);
}

static rvm_hal_wifi_event_func app_user_wifi_event = {NULL, NULL, app_user_scan_compeleted, NULL};
void app_user_scan_wifi_network_list()
{
    LOGI(TAG, "User Start wifi network");
    rvm_dev_t *dev = netmgr_get_dev(app_netmgr_hdl);
    //rvm_hal_net_set_hostname(dev, "T-head");

    rvm_hal_wifi_install_event_cb(dev, &app_user_wifi_event);
    aos_check(!aos_sem_new(&g_wifi_config_sem, 0), EIO);
