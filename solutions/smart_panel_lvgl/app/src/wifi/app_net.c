...
static int       net_reset_delay_time = 3;
    ...
    case EVENT_NETMGR_NET_DISCON:
        ...
        else 
        {
            ...
            else 
            {
                ...
                {
                    app_event_update(EVENT_GUI_USER_WIFI_NOEXIST);
                    LOGD(TAG, "AP not found");
                } 
                else if (reason == NET_DISCON_REASON_WIFI_PSK_ERR) 
                {
                    app_event_update(EVENT_GUI_USER_WIFI_PSK_ERR);
                    LOGD(TAG, "password error");
                }
                //LOGD(TAG, "[[[[[======wifi_is_pairing value:[%d]======]]]]]\r\n", wifi_is_pairing());
                if (!wifi_is_pairing()) 
                {
                    char *ssid, *psk;
                    net_is_linkup  = 1;
...
//#endif
                    ...
                    printf("@@@====smartliving_client_control===@@@@");
#endif
            }
        } else {
