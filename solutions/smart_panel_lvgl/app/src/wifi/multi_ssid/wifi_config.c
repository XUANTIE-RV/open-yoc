...
static void wifi_config_add(app_wifi_config_t *config, char *ssid, char *psk)
{
    ...
    // memset(config->ssid[(config->cur_used_ssid + 1) % config->max_num], 0, sizeof(config->ssid[(config->cur_used_ssid + 1) % config->max_num]));
    // memset(config->psk[(config->cur_used_ssid + 1) % config->max_num], 0 ,sizeof(config->psk[(config->cur_used_ssid + 1) % config->max_num]));

    // strncpy(config->ssid[(config->cur_used_ssid + 1) % config->max_num], ssid, strlen(ssid));
    // strncpy(config->psk[(config->cur_used_ssid + 1) % config->max_num], psk, strlen(psk));
