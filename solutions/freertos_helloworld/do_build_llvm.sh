#!/bin/bash
###attention: do_build.sh in demo mcu_rtthread_xxx should keep same with solutions/rtthread_helloworld/do_build.sh for xt-rtos sdk
...
make -j${nproc} cpu=$1 board=$2 toolchain=llvm libc=$3
