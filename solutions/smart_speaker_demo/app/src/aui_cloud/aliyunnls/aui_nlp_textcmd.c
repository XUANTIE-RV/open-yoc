...
int aui_nlp_proc_textcmd(const char *asr_text)
{
    ...
    if (strstr(cmd, "media_close") != NULL) {
        ...
        switch(id){
            case 1:
                app_player_play(MEDIA_MUSIC, URL_BASE"/"MP3_TEST1, 0);
                break;
            case 2:
                app_player_play(MEDIA_MUSIC, URL_BASE"/"MP3_TEST2, 0);
                break;
            default:
                return -1;
        }
    } else if (strstr(cmd, "media_1khz") != NULL) {
        app_player_play(MEDIA_MUSIC, URL_BASE"/"MP3_HZ1K, 0);
    } else if (strstr(cmd, "media_300hz") != NULL) {
        app_player_play(MEDIA_MUSIC, URL_BASE"/"MP3_HZ300, 0);
    } else {
