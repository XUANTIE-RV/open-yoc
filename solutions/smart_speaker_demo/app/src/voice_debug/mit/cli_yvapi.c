...
#define VAD_DATA_FILE   "/fatfs0/yvrec/VAD%d.RAW"
...
{
    char filename[32], vad_filename[32];
...
#ifdef CONFIG_MIT_DSP
    sprintf(vad_filename, VAD_DATA_FILE, g_rec_count);

    LOGI(TAG,"Vad %s", vad_filename);
    g_vad_fd = aos_open(vad_filename, O_WRONLY | O_CREAT);

    if (g_vad_fd < 0) {
        LOGE(TAG,"Create %s error", vad_filename);
    }
#endif
...
static int vad_write_file(uint8_t * buf, int size)
{
    if (g_vad_fd < 0) {
        return -1;
    }
    ...
    if (size % 2) {
        LOGE(TAG,"write vad %d", size);
        return -1;
    }

    int wsize = aos_write(g_vad_fd, buf, size);
    if (wsize != size) {
        LOGE(TAG,"vad write error %d", wsize);
    }
...
static int vad_write(void *context, rpc_t *rpc);
...
static int vad_write(void *context, rpc_t *rpc)
{
    ...
    vad_write_file(param->buf, param->size);
...
int vad_call_write(uint8_t * buf, int size)
{
    int ret = -1;
    sdrec_write_param_t param;
    param.buf = buf;
    ...
    ret = uservice_call_async(g_sdrec_usrv, VAD_WRITE, (void*) &param, sizeof(param));
    return ret;
}
...
#define MIC_CTRL_DEBUG_CLOSE (11)
...
static void yv_event_cb(void *priv, mic_event_id_t evt_id, void *data, int len)
{
    ...
    case YV_VAD_DAT_EVT:
        ...
        if (g_vad_fd > 0) {
            /* SD卡连续录制 */
            uint8_t * rec_data = aos_malloc_check(len);
            int ret;
            memcpy(rec_data, data, len);
            ret = vad_call_write(rec_data, len);
            if (ret < 0) {
                aos_free(rec_data);
                LOGE(TAG, "sdrec_call_write");
            }
