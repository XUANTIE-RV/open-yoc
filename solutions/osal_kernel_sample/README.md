...
`osal_kernel_sample` 是一个玄铁RTOS SDK中使用osal封装的示例，可运行在玄铁QEMU环境。该demo提供了基于osal接口下事件(event)、消息队列(message queue)、互斥量(mutex)、信号量(semaphore)、任务(task)、时间(time)和定时器(timer)的使用范例。
