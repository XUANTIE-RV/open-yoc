...
`rtthread_kernel`   是一个玄铁RTOS SDK中的demo，该demo基于rtthread原生接口实现，可运行在QEMU环境。这个demo提供了rtthread中事件(event)、消息队列(message queue)、互斥量(mutex)、信号量(semaphore)、任务(task)、时间(time)和定时器(timer)的使用范例。
