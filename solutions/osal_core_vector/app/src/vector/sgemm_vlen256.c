...
void shl_c908_gemm_8x16_fp32_v256(float *sc, const float *sa, const float *sb, int m, int k, int n, int ldc)
{
    ...
    for (; i + 7 < m; i += 8) {
        ...
        for (; j + 7 < n; j += 8) {
            ...
            vfloat32m1_t _acc0 = vle32_v_f32m1(out_ptr0, vl);
            vfloat32m1_t _acc1 = vle32_v_f32m1(out_ptr1, vl);
            vfloat32m1_t _acc2 = vle32_v_f32m1(out_ptr2, vl);
            vfloat32m1_t _acc3 = vle32_v_f32m1(out_ptr3, vl);
            vfloat32m1_t _acc4 = vle32_v_f32m1(out_ptr4, vl);
            vfloat32m1_t _acc5 = vle32_v_f32m1(out_ptr5, vl);
            vfloat32m1_t _acc6 = vle32_v_f32m1(out_ptr6, vl);
            vfloat32m1_t _acc7 = vle32_v_f32m1(out_ptr7, vl);  // init acc with bias_data
    ...
    for (; i + 3 < m; i += 4) {
        ...
        for (; j < n; j++) {
            ...
            float acc1 = out_ptr1[0];
            float acc2 = out_ptr2[0];
            float acc3 = out_ptr3[0];
            for (int c = 0; c < k; c++) {
