...
int test_spi_slaveReceive(char *args)
{
    ...
    TEST_CASE_TIPS("config spi's mode SLAVE");
	...
	/*spi config*/	
    ret = csi_spi_config(spi, td.baud, SPI_MODE_SLAVE, td.cp_format, td.bit_order, SPI_SS_MASTER_HW_OUTPUT, td.frame_len);
	...
	receive_data = (char *)malloc(SPI_TRANSFER_LEN);
	...
	TEST_CASE_READY();
	spi_cb_rx_flag = 1;
	ret = csi_spi_receive(spi, receive_data, SPI_TRANSFER_LEN);
	TEST_CASE_ASSERT(ret == NULL, "csi_spi_receive interface test fail");
	while (spi_cb_rx_flag);

	TEST_CASE_ASSERT(memcmp(send_data, receive_data,SPI_TRANSFER_LEN) == 0, "receive data err");
