...
int test_iic_slaveReceive(char *args)
{
	...
	ret = csi_iic_slave_receive(iic, receive_data, td.trans_size);
	TEST_CASE_ASSERT(ret == 0, "csi_iic_slave_receive fail");
	while (transfer_done_flag);

	TEST_CASE_ASSERT(memcmp(send_data, receive_data,td.trans_size) == 0, "receive data err");
