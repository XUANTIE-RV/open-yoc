...
static void nlp_handle(void *data, int len, void *priv)
{
    ...
    if (req_ack_info->data_type == AUI_AUDIO_REQ_TYPE_WWV) {
        ...
        cJSON *js = cJSON_Parse(req_ack_info->data);
        //fix me
        cJSON *name = cJSON_GetObjectItemByPath(js, "nlp");
        //nlp only
        if( cJSON_IsString(name) && strcmp(name->valuestring, "nlp_result") == 0 ){
...
int app_aui_cloud_init(void)
{
    ...
    aui_us_nlp_register(g_aui_handler, nlp_handle, g_aui_handler);
    aui_us_tts_register(g_aui_handler, tts_handle, g_aui_handler);
