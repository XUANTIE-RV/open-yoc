...
static int g_app_record_start = 0;
...
static int g_mic_rec_test_rpos[MIC_REC_COUNT];
...
{
    if (g_app_record_start == 1) {
        LOGW(TAG, "app rec start yet, please stop before starting");
        return;
    }
    g_app_record_start = 1;
#ifdef MIC_REC_DEBUG
    mic_rec_test_init();
#endif
    for (int i = 0; i < MIC_REC_COUNT; i++) {
