...
#if MAIN_HAS_NOARGC
MAIN_RETURN_TYPE
benchmark_coremark_main(void)
{
    int   argc = 0;
    char *argv[1];
#else
MAIN_RETURN_TYPE
benchmark_coremark_main(int argc, char *argv[])
{
#endif
...
#endif
            ee_printf("\n");
            extern uint32_t soc_get_cur_cpu_freq(void);
            printf("Score (Coremarks/MHz): %6.2f\n", default_num_contexts*results[0].iterations/time_in_secs(total_time) / (soc_get_cur_cpu_freq() / 1000000));
        }
#endif
    }
    if (total_errors > 0)
