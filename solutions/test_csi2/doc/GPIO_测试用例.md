...
| 示例 | 把A板的PA0和B板的PA1这两个引脚使用杜邦线连接, 然后按照下列步骤进行测试<br>步骤1:配置A板的PA0引脚作为GPIO的功能,向A板发送下列命令<br>    AT+PINMUX_CONFIG=0,0,4<br>步骤2:配置B板的PA1引脚作为GPIO的功能,向B板发送下列命令<br>    AT+PINMUX_CONFIG=0,1,4<br>步骤3:向A板发送测试GPIO写的命令<br>    AT+GPIO_PIN_WRITE=0,0,1,2000<br>步骤4:向B板发送GPIO读的命令<br>    AT+GPIO_PIN_READ=1,0,1 |
...
#### AT+GPIO_PINS_WRITE
| 命令格式 | AT+GPIO_PINS_WRITE=pin_mask,gpio_mode, pin_value,delay_ms |
|----------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 命令功能 | 配置pin_mask描述的多个引脚为gpio_mode模式, 当配置完成后输出一个pin_value 状态的电平并维持后delay_ms毫秒 |
| 可搭配的辅助测试命令 | AT+GPIO_PINS_READ |
| 示例 | 把A板的PA0和B板的PA1这两个引脚使用杜邦线连接, 把A板的PA2和B板的PA3 互接,然后按照下列步骤进行测试<br>步骤1:配置A板的PA0和PA2引脚作为GPIO的功能,向A板发送下列命令<br/>    AT+PINMUX_CONFIG=0,0,4<br/>    AT+PINMUX_CONFIG=0,2,4<br/>步骤2:配置B板的PA1和PA3引脚作为GPIO的功能,向B板发送下列命令<br/>    AT+PINMUX_CONFIG=0,1,4<br/>    AT+PINMUX_CONFIG=0,3,4<br/>步骤3:向A板发送测试GPIO写的命令<br/>    AT+GPIO_PINS_WRITE=3,0,1,2000<br/>步骤4:向B板发送GPIO读的命令<br/>    AT+GPIO_PINS_READ=12,0,1 |
| 备注 | 由于步骤3是持续输出2秒的电平状态,所以步骤4中GPIO读的命令必须在步骤3描述的命令发出后的2秒之内发出 |


#### AT+GPIO_PINS_READ
| 命令格式 | AT+GPIO_PINS_READ=pin_mask,gpio_mode,pin_value |
|----------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 命令功能 | 配置pin_mask这些引脚为gpio_mode模式, 当配置完成后读取这些引脚的输入状态值,并同pin_value做比较,如果相同则向PC机返回OK |
| 可搭配的辅助测试命令 | AT+GPIO_PINS_WRITE |
| 示例 | 参考AT+GPIO_PINS_WRITE示例 |

#### AT+GPIO_PIN_DEBONCE_WRITE
| 命令格式 | AT+GPIO_PIN_DEBONCE_WRITE=pin,gpio_mode, pin_value,delay_ms |
|----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 命令功能 | 配置pin这个引脚为gpio_mode模式,开启引脚输出防抖功能 ,当配置完成后输出一个pin_value 状态的电平并维持后delay_ms毫秒 |
| 可搭配的辅助测试命令 | AT+GPIO_PIN_READ<br>AT+GPIO_DEBONCE_PIN_READ |
| 示例 | 把A板的PA0和B板的PA1这两个引脚使用杜邦线连接, 然后按照下列步骤进行测试<br/>步骤1:配置A板的PA0引脚作为GPIO的功能,向A板发送下列命令<br/>    AT+PINMUX_CONFIG=0,0,4<br/>步骤2:配置B板的PA1引脚作为GPIO的功能,向B板发送下列命令<br/>    AT+PINMUX_CONFIG=0,1,4<br/>步骤3:向A板发送测试GPIO写的命令<br/>    AT+GPIO_PIN_DEBONCE_WRITE=0,0,1,2000<br/>步骤4:向B板发送GPIO读的命令<br/>    AT+GPIO_DEBONCE_PIN_READ=1,0,1 |
| 备注 | 由于步骤3是持续输出2秒的电平状态,所以步骤4中GPIO读的命令必须在步骤3描述的命令发出后的2秒之内发出 |
