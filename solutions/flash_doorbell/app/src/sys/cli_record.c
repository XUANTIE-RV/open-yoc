...
cx_preview_hdl_t preview_handle = NULL;
static bool lv_is_streaming = false;
...
static void cmd_preview_test(char *wbuf, int wbuf_len, int argc, char **argv)
{
    if(argc < 2) {
        printf("param err\n");
        return;
...
static void cmd_gui_test(char *wbuf, int wbuf_len, int argc, char **argv)
{
    if(strcmp(argv[1], "enable") == 0) {
#ifdef CONFIG_RGBIR_SENSOR_SWITCH
        GUI_IR_Display_Enable(atoi(argv[2]));   //显示GUI
#endif
    } else if(strcmp(argv[1], "play") == 0) {
        GUI_Show_FaceRecog_Result(atoi(argv[2]));   //显示成功
    }
}
...
void cli_reg_cmd_lv(void)
{
    ...
    static const struct cli_command cli_preview_test = {"preview", "preview test", cmd_preview_test};
    aos_cli_register_command(&cli_preview_test);

    static const struct cli_command cli_gui_test = {"gui", "gui test", cmd_gui_test};
    aos_cli_register_command(&cli_gui_test);
}
/*
record video start
record video stop
...
int app_preview_init(preview_mode_t mode)
{
    SENSOR_TOGGLE_LOCK(&sensor_toggle_lock);
#ifdef CONFIG_RGBIR_SENSOR_SWITCH
    if(mode == PREVIEW_MODE_RGB) {
        MediaVideoRGBInit();
    } else if(mode == PREVIEW_MODE_IR) {
        MediaVideoIRInit();
    }
#endif
    preview_handle = cx_preview_start("preview", mode);
    SENSOR_TOGGLE_UNLOCK(&sensor_toggle_lock);
    return 0;
}