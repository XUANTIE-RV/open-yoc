...
`bare_drv_uart` 是一个玄铁RTOS SDK中面向baremetal领域的串口驱动裸驱示例(基于csi2.0规范)，可运行在QEMU环境
...
- cpu: <br />
        `smartm/smartl/xiaohui`平台的cpu参考 [`components/xuantie_cpu_sdk/xt_rtos_sdk.csv`](../../components/xuantie_cpu_sdk/xt_rtos_sdk.csv) 的 `cpu_list`
- platform: <br />
...
正常运行串口输出内容参考如下
```
bare_drv_uart demo start
==uart_test_sync_mode start==
ABCDEFGHIJKLMN
ABCDEFGHIJKLMN
==uart_test_sync_mode end==
==uart_test_async_mode start==
ABCDEFGHIJKLMN
==uart_test_async_mode end==
bare_drv_uart success
```
