...
class PageFota extends $falcon.Page {
  /**
   * 构造函数,页面生命周期内只执行一次
   */
  constructor() {
  ...
  onNewOptions(options) {
    ...
    console.log("fota onNewOptions:"+"enter fota again");
  }
  ...
  onShow() {
    ...
    // this.$root.sayHello();
    console.log("fota onShow");
    // let res = await fota.start();
    // /*异步函数，需要await等待返回值*/
    //console.log("fota onShow:"+"StartFota res" + res);
  }
   ...
   */
  async onUnload() {
    console.log("fota onUnload");
    let res = await fota.stop();
    /*异步函数，需要await等待返回值*/
    console.log("fota onUnload:"+"StopFota res=" + res);
  }

}
export default PageFota;