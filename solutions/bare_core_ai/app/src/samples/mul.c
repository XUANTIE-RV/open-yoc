...
int mul_main(int argc, char **argv)
{
	...
	memcpy(metadata_buffer, (int32_t *)&mul_test_data_f32[1 * 4],
		   3 * sizeof(int32_t));
	...
	memcpy(shape_in0, (int32_t *)&mul_test_data_f32[(1 + 3) * 4],
		   dim_in0 * sizeof(int32_t));
	memcpy(shape_in1, (int32_t *)&mul_test_data_f32[(1 + 3 + dim_in0) * 4],
		   dim_in1 * sizeof(int32_t));
	memcpy(shape_out,
		   (int32_t *)&mul_test_data_f32[(1 + 3 + dim_in0 + dim_in1) * 4],
	...
	float *data_ptr = &mul_test_data_f32[data_start_index];
	...
	int32_t infer_dim_o;
	int32_t infer_size_o = xnnl_mul_infer_shape(
