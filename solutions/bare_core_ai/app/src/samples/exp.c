...
int exp_main(int argc, char **argv)
{
	...
	memcpy(metadata_buffer, (int32_t *)&exp_test_data_f32[1 * 4],
		   5 * sizeof(int32_t));
	...
	float *data_ptr = &exp_test_data_f32[data_start_index];
	...
	int32_t infer_size_o =
		xnnl_exp_infer_shape(&infer_shape_o, &infer_dim_o, shape, dim);
	check_infer_shape(shape_o, dim_o, infer_shape_o, infer_dim_o);
	align_free(infer_shape_o);
	free(shape);

#if (TYPE == 32)
	test_unary_op_fp32(input_buffer, ref_buffer, size, xnnl_exp_fp32);
#elif (TYPE == 16)
	test_unary_op_fp16(input_buffer, ref_buffer, size, xnnl_exp_fp16);
#endif
