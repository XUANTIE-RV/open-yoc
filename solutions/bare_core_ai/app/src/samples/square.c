...
#include "app_main.h"
#include "square_test_data_f32.h"
...
int square_main(int argc, char **argv)
{
    ...
    memcpy(metadata_buffer, (int32_t *)&square_test_data_f32[1 * 4],
           5 * sizeof(int32_t));
    ...
    float *data_ptr = &square_test_data_f32[data_start_index];
    ...
    int32_t *shape_o = shape;
    int32_t dim_o = dim;
    int32_t *infer_shape_o;
    int32_t infer_dim_o;
    int32_t infer_size_o =
        xnnl_square_infer_shape(&infer_shape_o, &infer_dim_o, shape, dim);
    check_infer_shape(shape_o, dim_o, infer_shape_o, infer_dim_o);
    align_free(infer_shape_o);
    free(shape);

#if (TYPE == 32)
    test_unary_op_fp32(input_buffer, ref_buffer, size, xnnl_square_fp32);
#elif (TYPE == 16)
    test_unary_op_fp16(input_buffer, ref_buffer, size, xnnl_square_fp16);
#endif
