...
int neg_main(int argc, char **argv)
{

    int32_t *metadata_buffer = (int32_t *)fastmalloc(5 * sizeof(int32_t));
    memcpy(metadata_buffer, (int32_t *)&neg_test_data_f32[1 * 4],
           5 * sizeof(int32_t));
    ...
    float *data_ptr = &neg_test_data_f32[data_start_index];
...
#if (TYPE == 32)
    test_unary_op_fp32(input_buffer, ref_buffer, size, xnnl_neg_fp32);
#elif (TYPE == 16)
    test_unary_op_fp16(input_buffer, ref_buffer, size, xnnl_neg_fp16);
#endif
