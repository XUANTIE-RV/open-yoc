...
int sqrt_main(int argc, char **argv)
{
    ...
    memcpy(metadata_buffer, (int32_t *)&sqrt_test_data_f32[1 * 4],
           1 * sizeof(int32_t));
    ...
    memcpy(shape_buffer, (int32_t *)&sqrt_test_data_f32[2 * 4],
           dim * sizeof(int32_t));
    ...
    int32_t data_start_index = (2 + dim) * 4;
    float *data_ptr = &sqrt_test_data_f32[data_start_index];
...
#if (TYPE == 32)
    test_unary_op_fp32(input_buffer, ref_buffer, size, xnnl_sqrt_fp32);
#elif (TYPE == 16)
    test_unary_op_fp16(input_buffer, ref_buffer, size, xnnl_sqrt_fp16);
#endif
