...
uint8_t uart_baud_rate[3] = {115200,9600,51200};

void hal_uart_api_test(baud,width,parity,stop)
{
    uint32_t count   = 0;
    uint32_t ret     = -1;
    uint32_t i    = 0;
...
int aos_hal_uart_master_test(baud,width,parity,stop,size,timeout)
{
    uint32_t count   = 0;
    uint32_t ret     = -1;
    uint32_t i    = 0;
