...
{
	uint32_t i = 0;
	uint32_t max_fun_num = NELEMS(mars_pin_func);
	char v;

	for (i = 0; i < max_fun_num; i++) {
		if (strcmp(mars_pin_func[i].func, func) == 0) {
			if (strncmp(mars_pin_func[i].name, pin, strlen(pin)) == 0) {
				v = mars_pin_func[i].name[strlen(mars_pin_func[i].name) - 1];
				break;
			}
		}
	}
...
void print_fun(char *name, uint32_t value)
{
	...
	printf("%s function:\n", name);
	for (i = 0; i < max_fun_num; i++) {
		if (strncmp(pinname, mars_pin_func[i].name, strlen(name)) == 0) {
			if (strcmp(pinname, mars_pin_func[i].name) == 0)
				printf("[v] %s\n", mars_pin_func[i].func);
			else
				printf("[ ] %s\n", mars_pin_func[i].func);
			// break;
		}
...
int cvi_pinmux_func(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "hplr:w:")) != -1) {
		switch (opt) {
		case 'r':
			for (i = 0; i < NELEMS(mars_pin); i++) {
				if (strcmp(optarg, mars_pin[i].name) == 0)
					break;
			}
			if (i != NELEMS(mars_pin)) {
				value = mmio_read_32(PINMUX_BASE + mars_pin[i].offset);
				// printf("value %d\n", value);
				print_fun(optarg, value);

				printf("register: 0x%x\n", PINMUX_BASE + mars_pin[i].offset);
				printf("value: %d\n", value);
			} else {
		...
		case 'w':
			...
			for (i = 0; i < NELEMS(mars_pin); i++) {
				if (strcmp(pin, mars_pin[i].name) == 0)
					break;
			}
			...
			if (i != NELEMS(mars_pin)) {
				...
				mmio_write_32(PINMUX_BASE + mars_pin[i].offset, f_val);
			} else {
				printf("\nInvalid option: %s\n", optarg);
			}
			break;
		...
		case 'l':
			for (i = 0; i < NELEMS(mars_pin); i++) {
				value = mmio_read_32(PINMUX_BASE + mars_pin[i].offset);
				// printf("value %d\n", value);
				print_fun(mars_pin[i].name, value);
				aos_msleep(10);
			}
			break;
