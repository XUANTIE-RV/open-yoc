...
int user_combo_get_dev_status_handler(uint8_t *buffer, uint32_t length)
{
    ...
    if (ble_ops == COMBO_BLE_SET) {
        ...
        cJSON *item = cJSON_GetObjectItem(params, "powerstate");
        ...
        item = cJSON_GetObjectItem(params, "brightness");
        if (item && cJSON_IsNumber(item)) {
...
void user_post_property(void)
{
    ...
    int light_switch = app_pwm_led_get_status();
    int brightness = app_pwm_led_get_brightness();
    snprintf(property_payload, sizeof(property_payload),
             "{\"powerstate\":%d,\"brightness\":%d}", light_switch, brightness);
...
int smartliving_client_control(const int start)
{
    if (start) {
        ...
        aos_task_t tsk;
        int ret = aos_task_new_ext(&tsk, "mqttkit", linkkit_thread, NULL, 1024*6, AOS_DEFAULT_APP_PRI);
        if (ret == 0) {
