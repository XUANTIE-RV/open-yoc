...
static void input_event_key_check_timer_cb(void *timer, void *arg)
{
    ...
    if (((key->input_io.config == INPUT_PULL_UP) && !input_io_status_get(key->input_io)) || ((key->input_io.config == INPUT_PULL_DOWN) && !input_io_status_get(key->input_io)))
    {
        key_pressed = 1;
    }
    if (key_pressed)
    ...
    {
        if (key->key_pressd_time < INPUT_EVENT_KEY_LONG_PRESS)
        { //short press release
            input_event_send(key->input_io.port, SWITCH_PRESS_ONCE);
        }
        else if (key->key_pressd_time > INPUT_EVENT_KEY_LONG_PRESS)
        { // long press release
...
__attribute__((section(".__sram.code"))) void input_event_check_key_state(uint8_t port)
{
...
__attribute__((section(".__sram.code"))) void io_irq_handler(void *arg)
{
    ...
    input_event_check_key_state(io->port);
}
...
int32_t input_event_init(void)
{
    ...
    if (ret != 0)
    {
        GENIE_LOG_ERR("input event gpio init fail");
        return ret;
    }
