...
#define TAG "BODY_SENSOR_INPUT"

typedef struct _input_key_s {
    uint8_t key_last_status;
    uint8_t key_status;//0 released 1 inputed
    uint8_t key_inputd_time;
    uint8_t input_check_timer_status;
    gpio_dev_t input_io;
    aos_timer_t input_check_timer;
} _input_key_t;

static sb_input_event_t input_event;
static _input_key_t g_key[INPUT_IO_NUM];

static int input_event_send(uint8_t port, body_sensor_input_type_e input_type)
{
    queue_mesg_t mesg;

    mesg.type = QUEUE_MESG_TYPE_INPUT;
    mesg.data = (port & 0x0F) | ((input_type & 0x0F) << 4);
...
static void input_event_key_check_timer_cb(void *timer, void *arg)
{
    ...
    if (key_inputed) {
        key->key_inputd_time++;
        aos_timer_start(&key->input_check_timer);
    } else {
        if(key->key_inputd_time < INPUT_EVENT_KEY_LONG_PRESS) { //short input release
            input_event_send(key->input_io.port,BODY_SENSOR_INPUT_ONCE);
        } else if (key->key_inputd_time > INPUT_EVENT_KEY_LONG_PRESS) {// long input release
            input_event_send(key->input_io.port,BODY_SENSOR_INPUT_LONG);
        }
        aos_timer_stop(&key->input_check_timer);
        key->input_check_timer_status = 0;
    }

}

__attribute__((section(".__sram.code")))  void _key_input_process(uint8_t port)
{
    for(int index =0 ; index < INPUT_IO_NUM ; index++) {
        if(g_key[index].input_io.port == port && g_key[index].input_check_timer_status == 0) {
            aos_timer_start(&g_key[index].input_check_timer);
            g_key[index].input_check_timer_status = 1;
            g_key[index].key_inputd_time = 0;
        }
    }
}
...
int32_t input_event_init(void)
{
    ...
    for (int index = 0; index < INPUT_IO_NUM; index++) {
        aos_timer_new(&g_key[index].input_check_timer, input_event_key_check_timer_cb, &g_key[index], INPUT_EVENT_KEY_CHECK_TIMEROUT, 1);
        aos_timer_stop(&g_key[index].input_check_timer);
        g_key[index].key_status = KEY_RELEASED;
        g_key[index].key_last_status = KEY_RELEASED;
        g_key[index].key_inputd_time = 0;
        g_key[index].input_check_timer_status = 0;
    }
