...
perf_stat_args_t* perf_stat_parse_cmd(char* input) {
    ...
    if (token == NULL || strcmp(token, "perf") != 0) {
        perf_printf("Input does not start with 'perf'.\n");
    ...
    if (event_values) {
        ...
        configASSERT(event_values_copy != NULL);
    ...
    if (cpu_values) {
        ...
        configASSERT(cpu_values_copy != NULL);
    ...
    if (thread_values) {
        ...
        configASSERT(thread_values_copy != NULL);
        ...
        cmd->tids = (perf_thread_id*)perf_malloc(thread_count * sizeof(perf_thread_id));
        ...
        while ((temp_token = strtok_r(temp_rest, ",", &temp_rest))) {
            ...
            char *endptr;
            perf_thread_id tid = (perf_thread_id)strtol(temp_token, &endptr, 0);
            if (endptr == temp_token || *endptr != '\0')
            {
                perf_printf("Conversion error: invalid hexadecimal string\n");
                return NULL;
            }
            cmd->tids[i++] = tid;
        }
